...
package ARM_HTML is
   ...
   type HTML_Type is
     (HTML_3, -- Use only HTML 3 elements.
...
private
   ...
   type HTML_Output_Type is new ARM_Output.Output_Type with record
      ...
      Had_Prefix       : Boolean :=
        False; -- If in paragraph, value of not No_Prefix.
      ...
      Disp_Large_Char_Count : Natural                 :=
        0; -- Displayed large characters on current line (others are "small" characters).
      -- Large characters are capitals, 'm', 'w', and numbers.
      Any_Nonspace : Boolean :=
        False; -- Have we output any non-space on this line?
      ...
      Can_Emulate_Tabs : Boolean                          :=
        False; -- Can we emulate tabs in the current style?

      Is_In_Table : Boolean := False; -- Are we processing a table?
      In_Header   : Boolean :=
        False; -- If Is_In_Table, are we processing the header?
      Table_Column_Alignment : ARM_Output
        .Column_Text_Alignment; -- If Is_In_Table, specifies the column alignment.
      Table_Has_Small_Text : Boolean :=
        False; -- If Is_In_Table, specifies the text size.

      Current_Column : Natural :=
        0; -- When processing 4-column+ text, the current column number.
      Current_Item : Natural :=
        0; -- When processing 4-column+ text, the current item within the column.
