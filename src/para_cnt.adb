...
package body Para_Cnt_Pkg is
   ...
   begin
      Ada.Text_IO.Put_Line
        ("  Counting file " & Ada.Directories.Simple_Name (Item));

      Ada.Text_IO.Open
        (The_File, Ada.Text_IO.In_File, Ada.Directories.Full_Name (Item));
      ...
      begin
         loop
            ...
            Ada.Strings.Fixed.Translate
              (Buffer (1 .. BLen), Ada.Strings.Maps.Constants.Lower_Case_Map);

            PStart :=
              Ada.Strings.Fixed.Index
                (Buffer (1 .. BLen), "<div class=""paranum"">");
            if PStart /= 0 then -- A paragraph number found.
               PEnd :=
                 Ada.Strings.Fixed.Index (Buffer (PStart .. BLen), "</div>");
               if PEnd = 0 then
                  Ada.Text_IO.Put_Line
                    ("** Unable to find paranum end on line" & LCnt'Image);
               elsif Ada.Strings.Fixed.Index
                   (Buffer (PEnd .. BLen), "<div class=""paranum"">") /=
                 0
               then
                  Ada.Text_IO.Put_Line
                    ("** Multiple paranums on line" & LCnt'Image);
               else
                  Total := Total + 1;
                  if Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), ".") /=
                    0
                  then
                     Inserted := Inserted + 1;
                     -- else not inserted.
                  ...
                  if Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/1<") /=
                    0
                  then
                     Cnts ('1') := Cnts ('1') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/2<") /=
                    0
                  then
                     Cnts ('2') := Cnts ('2') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/3<") /=
                    0
                  then
                     Cnts ('3') := Cnts ('3') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/4<") /=
                    0
                  then
                     Cnts ('4') := Cnts ('4') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/5<") /=
                    0
                  then
                     Cnts ('5') := Cnts ('5') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/6<") /=
                    0
                  then
                     Cnts ('6') := Cnts ('6') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/7<") /=
                    0
                  then
                     Cnts ('7') := Cnts ('7') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/8<") /=
                    0
                  then
                     Cnts ('8') := Cnts ('8') + 1;
                  elsif Ada.Strings.Fixed.Index
                      (Buffer (PStart + 22 .. PEnd + 4), "/9<") /=
                    0
                  then
                     Cnts ('9') := Cnts ('9') + 1;
      ...
      Ada.Text_IO.Put_Line
        ("    Saw" & Total'Image & " paragraphs, with" & Inserted'Image &
         " inserted paragraphs");
   end Count_File;
...
begin
   ...
   Ada.Text_IO.Put_Line
     ("  Write results to " & Ada.Command_Line.Argument (1) & ".csv");

   Ada.Text_IO.Create
     (Para_Cnt_Pkg.Result_File, Ada.Text_IO.Out_File,
      Ada.Command_Line.Argument (1) & ".csv");

   -- Put a CVS header:
   Ada.Text_IO.Put_Line
     (Para_Cnt_Pkg.Result_File,
      "File, Total, Inserted, Org, '/1, '/2, '/3, '/4, '/5, '/6, '/7, '/8, '/9");

   Ada.Directories.Search
     (Directory => ".", Pattern => "RM-*.html",
