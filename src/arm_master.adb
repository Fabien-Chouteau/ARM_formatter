with Ada.Text_IO, Ada.Characters.Handling, Ada.Strings.Fixed, Ada.Strings
  .Unbounded;
with ARM_Input, ARM_File, ARM_Format, ARM_Output, ARM_Text, ARM_Texinfo, ARM_HTML, ARM_RTF, ARM_Corr, ARM_Master, ARM_Contents;
package body ARM_Master is
   ...
   Source, TOC,
   -- Global properties:
   Show_Index_Entries, Hide_Index_Entries, Show_Annotations,
      Hide_Annotations, Show_ISO, Hide_ISO, Link_Non_Terminals,
      Number_Paragraphs, Title, File_Prefix, Example_Font, Body_Font,
      Note_Format, Contents_Format, List_Format, Subdivision_Names,

   -- HTML properties:

      Single_HTML_Output_File, Use_MS_DOS_Names, HTML_Kind_Command,
      HTML_Nav_Bar, HTML_Tabs, HTML_Script, HTML_Header, HTML_Footer,
      HTML_Color, HTML_New_Revision_Colors,
   -- RTF properties:
   Single_RTF_Output_File,
      RTF_Header_Prefix, RTF_Footer_Text, RTF_Footer, RTF_Page_Size, RTF_Fonts,
      RTF_Version_Name,
   -- Other commands:
   Comment, Unknown);
   ...
   type Source_Item (Kind : Source_Kind := Empty) is record
      case Kind is
         when Empty =>
            null;         -- This item includes no source item.
         when TOC =>
            null;           -- No info for this, it just holds a
   ...
   Display_Index_Entries : Boolean :=
     False; -- Should Index entries be displayed?
   Document_Title     : ARM_Contents.Versioned_String; -- Document title.
   Output_File_Prefix : Ada.Strings.Unbounded
     .Unbounded_String; -- Output file prefix.
   Include_Annotations : Boolean :=
     False; -- Should annotations be included in the output?
   Include_ISO_Text : Boolean :=
     False; -- Should ISO text be included in the output?
   Should_Link_Non_Terminals : Boolean :=
     False; -- Should non-terminals be linked?
   Should_Number_Paragraphs : Boolean :=
     False; -- Should paragraphs be numbered?
   ...
   -- HTML properties:
   Use_Large_HTML_Files : Boolean :=
     False; -- Use small output files by default.
   ...
   HTML_Script_Text : Ada.Strings.Unbounded
     .Unbounded_String; -- Empty by default.
   HTML_Header_Text : Ada.Strings.Unbounded
     .Unbounded_String; -- Empty by default.
   HTML_Footer_Text : Ada.Strings.Unbounded
     .Unbounded_String; -- Empty by default.
   ...
   Footer_Use_Clause_Name : Boolean :=
     True; -- Use the clause name rather than the text above by default.
   ...
   Page_Size : ARM_RTF.Page_Size :=
     ARM_RTF.Letter; -- Use Letter size by default.
   Serif_Font : ARM_RTF.Serif_Fonts :=
     ARM_RTF.Times_New_Roman; -- Use Times by default.
   Sans_Serif_Font : ARM_RTF.Sans_Serif_Fonts :=
     ARM_RTF.Arial; -- Use Arial by default.
   ...
   function Decode_Command
     (Name : in ARM_Input.Command_Name_Type) return Command_Type
   is
   ...
   function Get_Versioned_String
     (Item_Details : in ARM_Contents.Versioned_String;
      For_Version  : in ARM_Contents.Change_Version_Type) return String
   is
   ...
   is
      ...
      procedure Process_Command is
         ...
         begin
            ...
            if ARM_Input.Is_Open_Char (Ch) then -- Start parameter:
               ...
               Ada.Text_IO.Put_Line
                 ("** Parameter missing for command on line" &
         ...
         begin
            ...
            if Close_Ch /= ' ' then
               ...
               ARM_Input.Copy_to_String_until_Close_Char
                 (Input_Object, Close_Ch, Item, ILen);
               return Item (1 .. ILen);
         ...
         procedure Get_Boolean
           (Param_Name : in ARM_Input.Command_Name_Type; Result : out Boolean;
            Is_First   : in Boolean := False)
         is
            -- Get a boolean value from a parameter named Param_Name.
            Ch, Close_Ch : Character;
         begin
            ARM_Input.Check_Parameter_Name
              (Input_Object, Param_Name => Param_Name, Is_First => Is_First,
...
--Ada.Text_IO.Put_Line("  Bool=" & Ch);
               case Ch is
                  ...
                  when others =>
                     Ada.Text_IO.Put_Line
                       ("  ** Bad value for boolean parameter " &
                        Ada.Strings.Fixed.Trim
                          (Param_Name, Ada.Strings.Right) &
                        " on line " & ARM_Input.Line_String (Input_Object));
               end case;
               ARM_Input.Get_Char (Input_Object, Ch);
               if Ch /= Close_Ch then
                  Ada.Text_IO.Put_Line
                    ("  ** Bad close for boolean parameter " &
         ...
         procedure Process_Versioned_String
           (Item : in out ARM_Contents.Versioned_String)
         is
...
--Ada.Text_IO.Put_Line("Process versioned item command");
            ...
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "Version" & (8 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => True, Param_Close_Bracket => Param_Close_Ch);
...
--Ada.Text_IO.Put_Line("  Version=" & Version);
               ...
               if Ch /= Param_Close_Ch then
                  Ada.Text_IO.Put_Line
                    ("  ** Bad close for change version on line " &
                     ARM_Input.Line_String (Input_Object));
                  ARM_Input.Replace_Char (Input_Object);
               end if;
               -- else no parameter. Weird.
            ...
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "Text" & (5 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Param_Close_Bracket => Param_Close_Ch);
            if Param_Close_Ch /= ' ' then
               -- Copy over the term:
               ARM_Input.Copy_to_String_until_Close_Char
                 (Input_Object, Param_Close_Ch, Text, TLen);
--Ada.Text_IO.Put_Line("  Text=" & Text(1..TLen));
            ...
            if Ch = Close_Ch then
               ...
               Ada.Text_IO.Put_Line
                 ("** Missing closing character for command on line" &
                  ARM_Input.Line_String (Input_Object));
               ARM_Input.Replace_Char (Input_Object);
            end if;
...
--Ada.Text_IO.Put_Line("Process RTF Fonts");
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "Serif" & (6 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => True, Param_Close_Bracket => Param_Close_Ch);
            if Param_Close_Ch /= ' ' then
               -- Copy over the font name:
               ARM_Input.Copy_to_String_until_Close_Char
                 (Input_Object, Param_Close_Ch, Item, ILen);
               ...
               begin
                  if Name = "times" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown serif font name: " & Name & " on line" &
                        ARM_Input.Line_String (Input_Object));
                  end if;
               end;
               -- else no parameter, error already produced.
            ...
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "SansSerif" & (10 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Param_Close_Bracket => Param_Close_Ch);
            if Param_Close_Ch /= ' ' then
               -- Copy over the font name:
               ARM_Input.Copy_to_String_until_Close_Char
                 (Input_Object, Param_Close_Ch, Item, ILen);
               Ada.Text_IO.Put_Line
                 ("RTF Sans_Serif Font=" & Item (1 .. ILen));
               declare
                  Name : constant String :=
                    Ada.Characters.Handling.To_Lower (Item (1 .. ILen));
               begin
...
--Ada.Text_IO.Put_Line("Process HTML Kind");
            ...
            if Param_Close_Ch /= ' ' then
               ...
               ARM_Input.Copy_to_String_until_Close_Char
                 (Input_Object, Param_Close_Ch, Item, ILen);
               Ada.Text_IO.Put_Line
                 ("HTML version and kind=" & Item (1 .. ILen));
               declare
               ...
               begin
                  if Kind = "3" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown HTML version: " & Kind & " on line" &
                        ARM_Input.Line_String (Input_Object));
                  end if;
               end;
               -- else no parameter, error already produced.
            ...
            Get_Boolean
              ("Unicode" & (8 .. ARM_Input.Command_Name_Type'Last => ' '),
               HTML_Use_Unicode);
            if HTML_Use_Unicode then
               ...
               Ada.Text_IO.Put_Line
                 ("HTML will use Unicode characters only when explicitly requested");
            end if;
...
--Ada.Text_IO.Put_Line("Process HTML Nav Bar");
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "RefName" & (8 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => True, Param_Close_Bracket => Param_Close_Ch);
            if Param_Close_Ch /= ' ' then
               -- Copy over the version:
               ARM_Input.Copy_to_String_until_Close_Char
            ...
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "SrchName" & (9 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Param_Close_Bracket => Param_Close_Ch);
            if Param_Close_Ch /= ' ' then
               -- Copy over the version:
               ARM_Input.Copy_to_String_until_Close_Char
            ...
            ARM_Input.Check_Parameter_Name
              (Input_Object,
               Param_Name =>
                 "IndexName" & (10 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Param_Close_Bracket => Param_Close_Ch);
            if Param_Close_Ch /= ' ' then
               -- Copy over the version:
               ARM_Input.Copy_to_String_until_Close_Char
            ...
            Get_Boolean
              ("UseButtons" & (11 .. ARM_Input.Command_Name_Type'Last => ' '),
               HTML_Use_Buttons);
            ...
            Get_Boolean
              ("OnTop" & (6 .. ARM_Input.Command_Name_Type'Last => ' '),
               HTML_Nav_On_Top);
            ...
            Get_Boolean
              ("OnBottom" & (9 .. ARM_Input.Command_Name_Type'Last => ' '),
               HTML_Nav_On_Bottom);
         ...
         procedure Process_HTML_Color is
            ...
            procedure Get_Color
              (Param_Name : in ARM_Input.Command_Name_Type;
               Is_First   : in Boolean; Result : out ARM_HTML.Color_String)
            is
...
--Ada.Text_IO.Put_Line("Process HTML Color");
            ...
            Get_Color
              ("Background" & (11 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Result => HTML_Background_Color);
            Ada.Text_IO.Put_Line
              ("HTML background color is " & HTML_Background_Color);

            Get_Color
              ("Link" & (5 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Result => HTML_Link_Color);
            Ada.Text_IO.Put_Line ("HTML link color is " & HTML_Link_Color);

            Get_Color
              ("VLink" & (6 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Result => HTML_VLink_Color);
            Ada.Text_IO.Put_Line
              ("HTML visited link color is " & HTML_VLink_Color);

            Get_Color
              ("ALink" & (6 .. ARM_Input.Command_Name_Type'Last => ' '),
               Is_First => False, Result => HTML_ALink_Color);
            Ada.Text_IO.Put_Line
              ("HTML active link color is " & HTML_ALink_Color);
...
--Ada.Text_IO.Put_Line("Process source command - Length =" & Source_Count'Image(Source_Length));
               Source_Data (Source_Length) :=
                 (Kind               => A_File, File_Name => (others => ' '),
                  File_Name_Len      => 0, Section_Name => (others => ' '),
                  Section_Name_Len   => 0, Section_Number => 0,
                  Starts_New_Section => True);
               ARM_Input.Check_Parameter_Name
                 (Input_Object,
                  Param_Name =>
                    "Name" & (5 .. ARM_Input.Command_Name_Type'Last => ' '),
                  Is_First => True, Param_Close_Bracket => Param_Close_Ch);
               if Param_Close_Ch /= ' ' then
                  -- Copy over the term:
                  ARM_Input.Copy_to_String_until_Close_Char
                  ...
                  if ILen = 1 and then Item (1) in 'A' .. 'Z' then
                     ...
                     exception
                        when Constraint_Error =>
                           Ada.Text_IO.Put_Line
                             ("** Illegal section number " & Item (1 .. ILen) &
                              " on line" &
                              ARM_Input.Line_String (Input_Object));
                     end;
                  end if;
...
--Ada.Text_IO.Put_Line("Process command=" & Command_Type'Image(Command) & " Name=" & Command_Name);
         ...
         case Command is
            ...
            when File_Prefix =>
               ...
               Ada.Text_IO.Put_Line
                 ("File Prefix is " & (+Output_File_Prefix));
            ...
            when Example_Font =>
               ...
               begin
                  if Font_Name = "swiss" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown example font name: " & Font_Name &
                        " on line" & ARM_Input.Line_String (Input_Object));
                  end if;
               end;
            ...
            when Body_Font =>
               ...
               begin
                  if Font_Name = "swiss" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown body font name: " & Font_Name &
                        " on line" & ARM_Input.Line_String (Input_Object));
                  end if;
               end;
            ...
            when Note_Format =>
               ...
               begin
                  if Format_Name = "ada95" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("Notes in ISO 2004 standard format");
                  else
                     Ada.Text_IO.Put_Line
                       ("** Unknown note format name: " & Format_Name &
                        " on line" & ARM_Input.Line_String (Input_Object));
                  end if;
               end;
            ...
            when Contents_Format =>
               ...
               begin
                  if Format_Name = "ada95" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("Table of Contents in ISO 2004 standard format");
                  else
                     Ada.Text_IO.Put_Line
                       ("** Unknown contents format name: " & Format_Name &
                        " on line" & ARM_Input.Line_String (Input_Object));
                  end if;
               end;
            ...
            when List_Format =>
               ...
               begin
                  if Format_Name = "ada95" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("Lists in ISO 2004 standard format");
                  else
                     Ada.Text_IO.Put_Line
                       ("** Unknown list format name: " & Format_Name &
                        " on line" & ARM_Input.Line_String (Input_Object));
                  end if;
               end;
            ...
            when Subdivision_Names =>
               ...
               begin
                  if SD_Name = "chapter" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("Top-level subdivisions known as Chapters");
                  elsif SD_Name = "section" then
                     Subdivision_Name_Kind := ARM_Output.Section;
                     Ada.Text_IO.Put_Line
                       ("Top-level subdivisions known as Sections");
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown subdivision name: " & SD_Name &
                        " on line" & ARM_Input.Line_String (Input_Object));
                  end if;
               end;
            ...
            when Use_MS_DOS_Names =>
               ...
               Ada.Text_IO.Put_Line
                 ("Use MS-DOS (8.3) file names for HTML output files");
            ...
            when HTML_Tabs =>
               ...
               begin
                  if Tabs = "singlespace" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown tab emulation name: " & Tabs & " on line" &
                        ARM_Input.Line_String (Input_Object));
                  end if;
                  Ada.Text_IO.Put_Line
                    ("HTML Tab Emulation is " &
            ...
            when HTML_New_Revision_Colors =>
               ...
               Ada.Text_IO.Put_Line
                 ("Unconditionally use new revision colors in HTML");
...
--Ada.Text_IO.Put_Line("Process RTF Footer");
                  ...
                  if Footer_Use_Clause_Name then
                     ...
                     Ada.Text_IO.Put_Line
                       ("RTF footer will include the fixed footer text");
                  end if;
                  ...
                  if Footer_Use_ISO_Format then
                     Ada.Text_IO.Put_Line
                       ("RTF footer will use the ISO format (swiss font, multiple sizes)");
                  else
                     Ada.Text_IO.Put_Line
                       ("RTF footer will use the normal format (body font, one size)");
                  end if;
            ...
            when RTF_Page_Size =>
               ...
               begin
                  if Size = "letter" then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Unknown page size name: " & Size & " on line" &
                        ARM_Input.Line_String (Input_Object));
                  end if;
                  Ada.Text_IO.Put_Line
                    ("RTF Page Size is " &
            ...
            when TOC =>
               ...
               if Source_Length = Source_Count'Last then
                  ...
                  Source_Length               := Source_Length + 1;
                  Source_Data (Source_Length) := (Kind => TOC);
               end if;
   ...
   begin
      ...
      loop
         ...
         begin
            ...
            case Char is
               ...
               when others =>
                  Ada.Text_IO.Put_Line
                    ("** Character(s) not in a command in the master file on line" &
                     ARM_Input.Line_String (Input_Object));
                  while
                    (Char /= ' ' and then Char /= '@' and then Char /= ASCII.LF
                     and then Char /= ASCII.SUB)
                  loop
                     ARM_Input.Get_Char (Input_Object, Char);
                  end loop;
                  ARM_Input.Replace_Char (Input_Object);
      ...
      if ARM_Output."=" (Font_of_Examples, ARM_Output.Roman)
        and then ARM_Output."=" (Font_of_Body, ARM_Output.Swiss)
      then
         -- This doesn't work, since "Roman" examples use the
         -- body styles, and if they are Swiss...
         Ada.Text_IO.Put_Line
           ("  ** Examples cannot be Roman if Body is Swiss!! (They will appear in the Swiss font)");
      end if;
   ...
   begin
      ARM_Format.Create
        (Format_Object, Change_Kind, Change_Version,
      ...
      for Source_Index in Source_Data'First .. Source_Length loop
         case Source_Data (Source_Index).Kind is
            ...
            when A_File =>
               ARM_Format.Scan
                 (Format_Object,
                  Source_Data (Source_Index).File_Name
                    (1 .. Source_Data (Source_Index).File_Name_Len),
   ...
   begin
      ...
      for Source_Index in Source_Data'First .. Source_Length loop
         case Source_Data (Source_Index).Kind is
            when TOC =>
               ARM_Format.Write_Table_of_Contents
            ...
            when A_File =>
               ARM_Format.Process
                 (Format_Object,
                  ...
                  Output_Object,
                  Section_Name =>
                    Source_Data (Source_Index).Section_Name
                      (1 .. Source_Data (Source_Index).Section_Name_Len),
                  Section_Number => Source_Data (Source_Index).Section_Number,
                  Starts_New_Section =>
                    Source_Data (Source_Index).Starts_New_Section);
            when Empty =>
   ...
   procedure Read_and_Process_Master_File
     (File_Name               : in String;
      ...
      The_Base_Change_Version :    ARM_Contents
        .Change_Version_Type; -- Base change version.
      Output_Format : in Output_Format_Type; Output_Path : in String)
   is
   ...
   begin
      ...
      exception
         when others =>
            Ada.Text_IO.Put_Line
              ("** Unable to open master file " & File_Name);
            return;
      end;
      ...
      Ada.Text_IO.Put_Line
        ("  Lines processed: " & ARM_File.Line_String (Input_Object));
      ...
      case Output_Format is
         when HTML =>
            ...
            begin
               ARM_HTML.Create
                 (Output, Big_Files => Use_Large_HTML_Files,
                  File_Prefix       => +Output_File_Prefix,
                  DOS_Filenames     => Use_MS_DOS_Filenames,
                  Output_Path       => Output_Path, HTML_Kind => HTML_Kind,
                  Use_Unicode       => HTML_Use_Unicode,
                  Number_Paragraphs => Should_Number_Paragraphs,
                  Ref_URL => +HTML_Ref_URL, Srch_URL => +HTML_Srch_URL,
                  ...
                  Force_New_Revision_Colors =>
                    HTML_Use_New_Revision_Colors
                    and then Change_Version >= '5',
         ...
         when RTF =>
            ...
            begin
               if ARM_Format."=" (Change_Kind, ARM_Format.Old_Only)
                 or else Change_Version = '0'
               then
                  ARM_RTF.Create
                    (Output, Page_Size => Page_Size, Includes_Changes => False,
                     ...
                     Footer_Text            =>
                       Get_Versioned_String (Footer_Text, Change_Version),
                     Body_Font => Font_of_Body, Version_Names => Version_Name);
               else
                  ARM_RTF.Create
                    (Output, Page_Size => Page_Size, Includes_Changes => True,
                     Big_Files               => Use_Large_RTF_Files,
                     Output_Path             => Output_Path,
                     Primary_Serif_Font      => Serif_Font,
                     Primary_Sans_Serif_Font => Sans_Serif_Font,
