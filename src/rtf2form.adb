with Ada.Text_IO, Ada.Strings.Fixed, Ada.Strings.Unbounded, Ada.Characters
  .Handling, Ada.Command_Line, Ada.Unchecked_Deallocation;
procedure Rtf2Form is
   ...
   type Line_Count is
     range 0 .. 1_000_000; -- 1 million lines ought to be enough.
   type Line_Type;
   ...
   procedure Load_File (File_Name : in String; Data : in out Line_Access) is
      ...
      Count         : Line_Count  := 0;
      Buffer        : String
        (1 .. 1_000); -- Since line endings aren't significant, the
...
--Ada.Text_IO.Put_Line("Line" & Line_Count'Image(Count) & ": read=" & Buffer(1..BLength));
         ...
         while Break_Loc <= BLength - 4 loop
            ...
            exit when Buffer (Break_Loc .. Break_Loc + 3) = "\par"
              and then
              (Buffer (Break_Loc + 4) = ' '
               or else Buffer (Break_Loc + 4) = '\');
            -- Found it.
...
--Ada.Text_IO.Put_Line("Line" & Line_Count'Image(Count) & ": initial=" & Buffer(1..Break_Loc-1));
         elsif Length (Last.Text) > 30_000 and then Break_Loc > BLength then
            -- Break at the original place, as this is too long.
            -- This is fine so long as the length of Buffer < 2768/2 (1386)
            Last.Text :=
              Last.Text & To_Unbounded_String (Buffer (1 .. Break_Loc - 1));
...
--Ada.Text_IO.Put_Line("Start_Loc = " & Natural'Image(Start_Loc) & " Blength=" & Natural'Image(BLength));
         while Start_Loc <= BLength + 1
         loop -- +1 so we handle end of line "\par" properly.
            -- Create a new line:
            Working        := new Line_Type;
            Working.Number := Count;
            Working.Next   := null;
            ...
            Working.Text :=
              To_Unbounded_String (Buffer (Start_Loc .. Break_Loc - 1));
--Ada.Text_IO.Put_Line("Line" & Line_Count'Image(Count) & ": startp=" & Buffer(Start_Loc..Break_Loc-1));
   ...
   type RTF_Group_Kind is
     (Unknown, Empty, Bold, Italic, Tab, Line_Break, Exam, ExamCom,
      Key); -- Type of RTF command.
   type Brace_Info is record
      Kind : RTF_Group_Kind;
   end record;
   Brace_Info_Stack : array (1 .. 1_000) of Brace_Info;
   ...
   procedure Process_and_Write_File
     (File_Name : in String; Data : in out Line_Access)
   is
      File          : Ada.Text_IO.File_Type;
   ...
   begin
      ...
      Ada.Text_IO.Put_Line
        (File, "@comment($Source: e:\\cvsroot/ARM/Progs/rtf2form.ada,v $)");
      Ada.Text_IO.Put_Line
        (File, "@comment($Revision: 1.4 $ $Date: 2012/05/19 01:00:26 $)");
      Ada.Text_IO.Put_Line (File, "");
      ...
      while Cursor /= null loop
         declare
            ...
            begin
               Ada.Text_IO.Put_Line
                 (File, Our_Line (Last_Output + 1 .. Working - 1));
...
--Ada.Text_IO.Put_Line ("  NO - plain text found");
                  ...
                  elsif Our_Line (I) in 'a' .. 'z'
                    or else Our_Line (I) in '0' .. '9'
                    or else Our_Line (I) = '-' or else Our_Line (I) = '\'
                  then
                     null; -- OK.
                  else
...
--Ada.Text_IO.Put_Line ("  OK");
            ...
            begin
               ...
               if Our_Line (Style_Code_Loc .. Style_Code_Loc + 4) = "\s15\"
               then
...
--Ada.Text_IO.Put_Line("Open Brace on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Working-1));
                  Chars_on_Line :=
                    Chars_on_Line + ((Working - 1) - (Last_Output + 1) + 1);
                  ...
                  if Working + 1 > Our_Line'Last
                    or else Our_Line (Working + 1) /= '\'
                  then
...
--Ada.Text_IO.Put_Line("Empty Open Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  elsif Working + 3 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 3) = "{\b "
                  then
...
--Ada.Text_IO.Put_Line("Bold Open Brace on Line" & Line_Count'Image(Cursor.Number));
                    ...
                    and then Our_Line (Working .. Working + 3) = "{\i "
                  then
...
--Ada.Text_IO.Put_Line("Italic Open Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  elsif Working + 6 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 6) = "{\line "
                  then
...
--Ada.Text_IO.Put_Line("Line_Break Open Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  elsif Working + 5 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 5) = "{\tab "
                  then
...
--Ada.Text_IO.Put_Line("Tab Open Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  elsif Working + 14 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 14) =
                      "{\cs16\f1\fs20 "
                  then
...
--Ada.Text_IO.Put_Line("Exam Open Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  elsif Working + 8 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 8) = "{\cs20\b "
                  then
...
--Ada.Text_IO.Put_Line("Key (in example) Open Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  elsif Working + 16 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 16) =
                      "{\cs20\b\f1\fs20 "
                  then
...
--Ada.Text_IO.Put_Line("Key (in text) Open Brace on Line" & Line_Count'Image(Cursor.Number));
                    ...
                    and then Our_Line (Working .. Working + 8) = "{\cs24\i "
                  then
...
--Ada.Text_IO.Put_Line("Key Close Brace on Line" & Line_Count'Image(Cursor.Number));
                  ...
                  then
                     ...
                     Ada.Text_IO.Put_Line
                       (File, Our_Line (Last_Output + 1 .. Working));
--Ada.Text_IO.Put_Line("Soft line break on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Working));
                  ...
                  -- interesting.
                  if Working + 5 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 5) = "\line "
                  then
...
--Ada.Text_IO.Put_Line("Line break on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Working));
                  ...
                  elsif Working + 4 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 4) = "\tab "
                  then
...
--Ada.Text_IO.Put_Line("Escaped char on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Working));
      ...
      -- We leave the literal character in the output buffer for later writing.
                  ...
                  elsif Working + 7 <= Our_Line'Last
                    and then Our_Line (Working .. Working + 7) = "\endash "
                  then
...
--Ada.Text_IO.Put_Line("Endash on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Working));
                    ...
                    and then Our_Line (Working .. Working + 7) = "\emdash "
                  then
                     -- Line break in a paragraph. Output a New_line.
                     Ada.Text_IO.Put
                       (File, Our_Line (Last_Output + 1 .. Working - 1));
--Ada.Text_IO.Put_Line("Emdash on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Working));
                    ...
                    and then Our_Line (Working .. Working + 5) = "\pard\"
                  then
...
--Ada.Text_IO.Put_Line("Paragraph style on Line" & Line_Count'Image(Cursor.Number));
                     declare
                        ...
                        Style_Code : Natural :=
                          Ada.Strings.Fixed.Index
                            (Our_Line (Working .. Our_Line'Last), "\s");
...
--                     "; End_of_Style=" & Natural'Image(End_of_Style));
                        if Style_Code = 0 or else End_of_Style = 13
                          or else Style_Code > End_of_Style
                          or else (not Possible_Style (Working, End_of_Style))
                        then
...
--Ada.Text_IO.Put_Line("Short paragraph style on Line" & Line_Count'Image(Cursor.Number));
                     ...
                     begin
                        ...
                        if Style_Code = 0 or else End_of_Style = 0
                          or else Style_Code > End_of_Style
                          or else (not Possible_Style (Working, End_of_Style))
                        then
                  -- No style code, no end of style, the style
            ...
            Ada.Text_IO.Put
              (File, Our_Line (Last_Output + 1 .. Our_Line'Last));
--Ada.Text_IO.Put_Line("Remaining text on Line" & Line_Count'Image(Cursor.Number) & ": output=" & Our_Line(Last_Output+1..Our_Line'Last));
      ...
      Ada.Text_IO.Put_Line
        ("  Total paragraphs:" & Natural'Image (Paragraphs));
      Ada.Text_IO.Put_Line
        ("  Deepest nesting:" & Natural'Image (Deepest_Brace_Nesting));
      for I in RTF_Group_Kind loop
         Ada.Text_IO.Put_Line
           ("  Kind " & RTF_Group_Kind'Image (I) & ":" &
            Natural'Image (Brace_Kinds_Found (I)));
      end loop;
      Ada.Text_IO.Put_Line
        ("  Escaped characters found:" & Natural'Image (Escaped_Chars));
      Ada.Text_IO.Put_Line ("  EM Dashes found:" & Natural'Image (EM_Dashes));
      Ada.Text_IO.Put_Line ("  EN Dashes found:" & Natural'Image (EN_Dashes));
      Ada.Text_IO.Put_Line
        ("  Line breaks inside of paragraphs:" &
         Natural'Image (Intraparagraph_Line_Breaks));
      Ada.Text_IO.Put_Line
        ("  Tabs inside of paragraphs:" & Natural'Image (Intraparagraph_Tabs));
      for I in Paragraph_Styles loop
         Ada.Text_IO.Put_Line
           ("  Style " & Paragraph_Styles'Image (I) & ":" &
