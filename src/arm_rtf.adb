with ARM_Output, ARM_Contents, Ada.Text_IO, Ada.Exceptions, Ada.Streams
  .Stream_IO, Ada.Strings.Maps, Ada.Strings.Fixed, Ada.Characters.Handling, Ada
  .Calendar, Ada.Unchecked_Conversion;
package body ARM_RTF is
   ...
   procedure Set_Style
     (Into                : in out Format_Info_Type;
      Font : in ARM_Output.Font_Family_Type; -- Default means use Body_Font.
      Body_Font : in     ARM_Output.Font_Family_Type; Font_Size : in Natural;
      Style_Indent        : in     Natural; Style_Hang_Width : in Natural := 0;
      Style_Before        : in     Natural; Style_After : in Natural;
      Style_Justified     : in     Boolean; Style_String_Prefix : in String;
      Style_String_Suffix : in     String)
   is
      -- Internal routine.
   ...
   begin
      ...
      declare
         Style : constant String :=
           Style_String_Prefix & " " & Font_String & Make ("fs", Into.Size) &
           Make ("li", Into.Indent) & Make ("fi-", Into.Hang_Width) &
           Make ("sb", Into.Before) & Make ("sa", Into.After) &
           Style_String_Suffix;
      begin
         Ada.Strings.Fixed.Move
           (Source => Style, Target => Into.Format_String);
         Into.Format_Len := Style'Length;
      end;
   end Set_Style;

   procedure Write_Style
     (Fyle : in Ada.Text_IO.File_Type; Style : in Format_Info_Type)
   is
   -- Internal routine.
   ...
   begin
      ...
      Ada.Text_IO.Put
        (Fyle, "{" & Style.Format_String (1 .. Style.Format_Len));
   end Write_Style;

   procedure Write_Style_for_Paragraph
     (Fyle  : in     Ada.Text_IO.File_Type; Style : in Format_Info_Type;
      Count :    out Natural)
   is
   -- Internal routine.
   ...
   begin
      ...
      Ada.Text_IO.Put
        (Fyle, "{\pard\plain " & Style.Format_String (1 .. Style.Format_Len));
      ...
      if Ada.Strings.Unbounded.Length (Output_Object.Header_Prefix) = 0 then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "{\b\f1 " & Ada.Strings.Unbounded.To_String (Output_Object.Title) &
            "\par}}}");
         ...
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            ...
            Ada.Strings.Unbounded.To_String (Output_Object.Header_Prefix) &
            " \emdash  " &
            Ada.Strings.Unbounded.To_String (Output_Object.Title) & "\par}}}");
      end if;
...
--      Ada.Text_IO.Put_Line (Output_Object.Output_File, "\par}}}");
   ...
   procedure Start_RTF_File
     (Output_Object : in out RTF_Output_Type; File_Name : in String;
      Title         : in     String; Name : in String)
   is
      -- Internal routine.
   ...
   begin
      Ada.Text_IO.Create
        (Output_Object.Output_File, Ada.Text_IO.Out_File,
      ...
      case Output_Object.Primary_Sans_Serif_Font is
         when Helvetica =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "{\f1\fswiss\fcharset0 Helvetica{\*\falt Arial};}"); -- Usually Arial on most Windows machines.
         when Arial =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "{\f1\fswiss\fcharset0 Arial{\*\falt Helvetica};}"); -- Give Arial preference, because otherwise it screwed up Jim Moore's machine.
      end case;
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "{\f2\fmodern\fcharset0\fprq1 Courier New;}");
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "{\f3\ftech\fcharset0 Symbol;}}");
      -- f=Font number;
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red0\green0\blue0;"); -- Color 1
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red0\green0\blue255;"); -- Color 2
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red0\green255\blue255;"); -- Color 3
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red0\green255\blue0;"); -- Color 4
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red255\green0\blue255;"); -- Color 5
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red255\green0\blue0;"); -- Color 6
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red255\green255\blue0;"); -- Color 7
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "\red255\green255\blue255;"); -- Color 8
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red0\green0\blue128;"); -- Color 9
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "\red0\green128\blue128;"); -- Color 10
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red0\green128\blue0;"); -- Color 11
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "\red128\green0\blue128;"); -- Color 12
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\red128\green0\blue0;"); -- Color 13
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
      ...
      if Output_Object.Page_Size = ARM_RTF.Ada95
        or else Output_Object.Page_Size = ARM_RTF.Half_Letter
      then
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 0), Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent => 0, Style_Before => 0, Style_After => 120,
            Style_Justified                             => True,
            Style_String_Prefix => "\s0\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-220\slmult0 \snext0 ");
         Set_Style
           (Heading_1_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 28,
            Style_Indent         => 0, Style_Before => 0, Style_After => 210,
            Style_Justified      => False,
            Style_String_Prefix  =>
              "\s1\keepn\widctlpar\outlinelevel0\adjustright",
            Style_String_Suffix =>
              "\b\kerning28\qc\cgrid \sbasedon0 \snext0 ");
         Set_Style
           (Heading_2_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 24,
            Style_Indent         => 0, Style_Before => 240, Style_After => 120,
            Style_Justified      => False,
            Style_String_Prefix  =>
              "\s2\keepn\widctlpar\outlinelevel1\adjustright",
            Style_String_Suffix => "\b\ql\cgrid \sbasedon0 \snext0 ");
         Set_Style
           (Heading_3_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 23,
            Style_Indent         => 0, Style_Before => 210, Style_After => 90,
            Style_Justified      => False,
            Style_String_Prefix  =>
              "\s3\keepn\widctlpar\outlinelevel2\adjustright",
            Style_String_Suffix => "\b\ql\cgrid \sbasedon0 \snext0 ");
         Set_Style
           (Category_Header_Info, Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 14,
            Style_Indent => 0, Style_Before => 100, Style_After => 60,
            Style_Justified            => False,
            Style_String_Prefix        => "\s4\keepn\adjustright",
            Style_String_Suffix        => "\cgrid\qc\i \snext0 ");
         Set_Style
           (Normal_Paragraph_Number_Info, Font => ARM_Output.Swiss,
            Body_Font => Output_Object.Body_Font, Font_Size => 12,
            Style_Indent => 0, Style_Before => 0, Style_After => 0,
            Style_Justified                    => False,
            Style_String_Prefix                =>
              ...
              "\cgrid\qc",
            Style_String_Suffix =>
              "\snext0 "); -- Note: We adjust the space before on use.
         Set_Style
           (Wide_Paragraph_Number_Info, Font => ARM_Output.Swiss,
            Body_Font => Output_Object.Body_Font, Font_Size => 12,
            Style_Indent => 0, Style_Before => 0, Style_After => 0,
            Style_Justified                  => False,
            Style_String_Prefix              =>
            ...
            Style_String_Suffix =>
              "\snext0 "); -- Note: We adjust the space before on use.
         Set_Style
           (Paragraph_Info (ARM_Output.Small, 1), -- Notes
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent        => INDENT_UNIT * 1, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
            Style_String_Prefix => "\s6\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext6 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small, 2), -- Annotations
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent        => INDENT_UNIT * 2, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
            Style_String_Prefix => "\s7\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext7 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Examples, 1), Font => ARM_Output.Fixed,
            Body_Font => Output_Object.Body_Font, Font_Size => 16,
            Style_Indent => INDENT_UNIT * 1, Style_Before => 0,
            Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s8\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\sl-160\ql \snext8 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Examples, 3),
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 3,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s9\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\sl-140\ql \snext9 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 1), -- Syntax indent
            Font         => ARM_Output.Default,
            Body_Font    => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent => INDENT_UNIT * 1, Style_Before => 0,
            Style_After  =>
              120, --80, previously different than normal, unknown why.
         ...
         --now returned to Ada 95 format.
            Style_String_Prefix => "\s10\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 3), -- Regular indent
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent        => INDENT_UNIT * 3, Style_Before => 0,
            Style_After         => 120, Style_Justified => True,
            Style_String_Prefix => "\s11\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-220\slmult0 \snext11 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small, 5), -- Regular annotation indent
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent        => INDENT_UNIT * 5, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
            Style_String_Prefix => "\s12\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext12 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Wide_Hanging, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width => 1_080, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s13\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Narrow_Hanging, 1),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 1,
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s14\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-200\slmult0 \snext14 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Wide_Hanging, 5),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 1_080, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s15\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-170\slmult0 \snext15 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Narrow_Hanging, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s16\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Bulleted, 1),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 1,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s17\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Nested_Bulleted, 2),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s18\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Bulleted, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s19\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Nested_Bulleted, 4),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 200, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s20\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-170\slmult0\tx1440 \snext20 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Bulleted, 4), -- Indented bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s21\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-200\slmult0\tx1080 \snext21 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Bulleted, 2), -- Syntax indent bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s22\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-200\slmult0\tx720 \snext22 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Bulleted, 3), -- Code indented bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s23\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-200\slmult0\tx1080 \snext23 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 2), -- Code indented
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent        => INDENT_UNIT * 2, Style_Before => 0,
            Style_After         => 120, Style_Justified => True,
            Style_String_Prefix => "\s24\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-220\slmult0 \snext24 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small, 4), -- Code indented annotations.
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent        => INDENT_UNIT * 4, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
            Style_String_Prefix => "\s25\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext25 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Examples, 4), -- Indented examples
            Font                => ARM_Output.Fixed,
            Body_Font           => Output_Object.Body_Font, Font_Size => 16,
            Style_Indent        => INDENT_UNIT * 4, Style_Before => 0,
            Style_After         => 80, Style_Justified => False,
            Style_String_Prefix => "\s26\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext26 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples, 6), -- Indented annotation examples.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 6,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s27\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-140 \snext27 ");

         Set_Style
           (Header_Info, Font   => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 17,
            Style_Indent        => 0, Style_Before => 0, Style_After => 0,
            Style_Justified     => False,
            Style_String_Prefix =>
              "\s28\widctlpar\tqc\tx" & Half_Paper_Width & "\tqr\tx" &
              Paper_Width & "\adjustright",
            Style_String_Suffix => "\cgrid \sbasedon0 \snext28 ");
         Set_Style
           (Footer_Info, Font   => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 17,
            Style_Indent        => 0, Style_Before => 0, Style_After => 0,
            Style_Justified     => False,
            Style_String_Prefix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Index, 0), Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent => 225, Style_Hang_Width => 225, Style_Before => 0,
            Style_After => 0, Style_Justified => False,
            Style_String_Prefix => "\s31\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-180\slmult0 \snext31 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Wide_Above, 0),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => 0, Style_Before => 120,
            Style_After         => 120, Style_Justified => True,
            Style_String_Prefix => "\s32\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-220\slmult0 \snext0 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Wide_Above, 2), -- Wide above annotations.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 2,
            Style_Before => 90, Style_After => 90, Style_Justified => True,
            Style_String_Prefix => "\s33\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext7 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Header, 1), -- Notes header
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 0, Style_Justified => True,
            Style_String_Prefix => "\s34\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext6 ");
         -- Note: No extra space afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Bulleted, 2), -- Bullets in notes
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 60,
            Style_Justified     => True,
            Style_String_Prefix => "\s35\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-170\slmult0\tx720 \snext35 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Nested_Bulleted, 3), -- Nested bullets in notes
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 230, Style_Before => 0, Style_After => 60,
            Style_Justified     => True,
            Style_String_Prefix => "\s36\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Hanging_in_Bulleted, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s37\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Hanging_in_Bulleted, 5),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s38\widctlpar\adjustright",
            Style_String_Suffix => "\ri360\cgrid\qj\sl-170\slmult0 \snext16 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Nested_Bulleted, 4), -- Code indent nested bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s39\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Syntax_Summary, 1),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 65, Style_Justified => False,
            Style_String_Prefix => "\s40\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Enumerated, 1),
            ...
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s41\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Enumerated, 3),
            ...
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s42\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Enumerated, 2),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 260, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s43\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Enumerated, 4),
            ...
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s44\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-170\slmult0\tx1080 \snext44 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Nested_Bulleted, 3), -- Doubly nested bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s45\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-200\slmult0\tx720 \snext45 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Nested_Bulleted, 5), -- Doubly nested bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 200, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s46\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-170\slmult0\tx1440 \snext46 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Nested_Bulleted, 5), -- Indented nested bullets.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s47\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-200\slmult0\tx1080 \snext47 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 4), -- Inner indented
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent        => INDENT_UNIT * 4, Style_Before => 0,
            Style_After         => 120, Style_Justified => True,
            Style_String_Prefix => "\s48\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-220\slmult0 \snext48 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small, 6), -- Inner indented
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent        => INDENT_UNIT * 6, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
            Style_String_Prefix => "\s49\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext49 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small, 3), -- Annotations in syntax indentation
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent        => INDENT_UNIT * 3, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
            Style_String_Prefix => "\s50\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-180\slmult0 \snext50 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Swiss_Examples, 1),
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s51\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Swiss_Examples, 3),
            ...
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 3,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s52\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\sl-160\ql \snext52 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Swiss_Examples, 4), -- Indented
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 4,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s53\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-180 \snext53 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Swiss_Examples, 6), -- Indented
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 6,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s54\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext54 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Enumerated, 3), -- Doubly nested enumerations
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 260, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s55\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-200\slmult0\tx360 \snext43 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Enumerated, 5), -- Doubly nested enumerations
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s56\widctlpar\adjustright",
            Style_String_Suffix =>
              "\ri360\cgrid\qj\sl-170\slmult0\tx1080 \snext56 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info
              (ARM_Output.Examples, 2), -- Syntax Indented examples
            Font                => ARM_Output.Fixed,
            Body_Font           => Output_Object.Body_Font, Font_Size => 16,
            Style_Indent        => INDENT_UNIT * 2, Style_Before => 0,
            Style_After         => 80, Style_Justified => False,
            Style_String_Prefix => "\s57\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext57 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               4), -- Syntax Indented annotation examples.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 4,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s58\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-140 \snext58 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Swiss_Examples, 2), -- Syntax Indented
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 2,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s59\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-180 \snext59 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Swiss_Examples, 4), -- Syntax Indented
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 4,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s60\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext60 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Examples, 3), -- Code Indented examples
            Font                => ARM_Output.Fixed,
            Body_Font           => Output_Object.Body_Font, Font_Size => 16,
            Style_Indent        => INDENT_UNIT * 3, Style_Before => 0,
            Style_After         => 80, Style_Justified => False,
            Style_String_Prefix => "\s61\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext61 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               5), -- Code Indented annotation examples.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 5,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s62\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-140 \snext62 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Swiss_Examples, 3), -- Code Indented
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 3,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s63\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-180 \snext63 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Swiss_Examples, 5), -- Code Indented
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 5,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s64\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext64 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Title, 0), Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font,
            Font_Size => 34, -- Slightly less than double.
            Style_Indent => 0, Style_Before => 120, Style_After => 120,
            Style_Justified                            => True,
            Style_String_Prefix => "\s65\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-360\slmult0 \snext0 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Giant_Hanging, 4),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width => 1_440, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s66\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Medium_Hanging, 2),
            ...
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s67\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-200\slmult0 \snext67 ");
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Giant_Hanging, 6),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 15, Style_Indent => INDENT_UNIT * 6,
            Style_Hang_Width    => 1_440, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s68\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Medium_Hanging, 4),
            ...
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s69\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Narrow_Hanging, 2),
            ...
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s70\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Medium_Hanging, 3),
            ...
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s71\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Wide_Hanging, 4),
            ...
            Style_Hang_Width => 1_080, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s72\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Narrow_Hanging, 4),
            ...
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s73\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Medium_Hanging, 5),
            ...
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s74\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Wide_Hanging, 6),
            ...
            Style_Hang_Width    => 1_080, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s75\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Narrow_Hanging, 3),
            ...
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s76\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Medium_Hanging, 4),
            ...
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s77\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Narrow_Hanging, 5),
            ...
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s78\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Medium_Hanging, 6),
            ...
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 80,
            Style_Justified     => True,
            Style_String_Prefix => "\s79\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-170\slmult0 \snext79 ");
         -- Note: Narrower space between and afterwards.

         Set_Style
           (Paragraph_Info (ARM_Output.Small_Examples, 2), -- Notes indent
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 2,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s80\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\sl-140\ql \snext80 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Swiss_Examples, 1),
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s81\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Swiss_Examples, 2),
            ...
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s82\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Examples, 1),
            ...
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s83\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\sl-140\ql \snext83 ");
         Set_Style
           (Paragraph_Info (ARM_Output.Examples, 5), -- Child Indented examples
            Font                => ARM_Output.Fixed,
            Body_Font           => Output_Object.Body_Font, Font_Size => 16,
            Style_Indent        => INDENT_UNIT * 5, Style_Before => 0,
            Style_After         => 80, Style_Justified => False,
            Style_String_Prefix => "\s84\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-160 \snext84 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               7), -- Child Indented annotation examples.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 14, Style_Indent => INDENT_UNIT * 7,
            Style_Before => 0, Style_After => 70, Style_Justified => False,
            Style_String_Prefix => "\s85\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-140 \snext85 ");

         -- The following are indented three extra levels:
         Set_Style
           (Paragraph_Info (ARM_Output.Narrow_Hanging, 4),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 100,
            Style_Justified     => True,
            Style_String_Prefix => "\s86\widctlpar\adjustright",
   ...
   -- New styles should be added here, following numbers will need adjustments.
         Set_Style
           (Heading_4_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 22,
            Style_Indent         => 0, Style_Before => 210, Style_After => 90,
            Style_Justified      => False,
            Style_String_Prefix  =>
         ...
         if Output_Object.Big_Files then
            ...
            Set_Style
              (TOC_1_Info, Font    => ARM_Output.Swiss,
               Body_Font           => Output_Object.Body_Font, Font_Size => 20,
               Style_Indent        => 0, Style_Before => 45, Style_After => 45,
               Style_Justified     => False,
               Style_String_Prefix =>
                 "\s88\widctlpar\tqr\tldot\tx" & Paper_Width & "\adjustright",
               Style_String_Suffix => "\b\cgrid \sbasedon0 \snext0 ");
            Set_Style
              (TOC_2_Info, Font    => ARM_Output.Swiss,
               Body_Font           => Output_Object.Body_Font, Font_Size => 17,
               Style_Indent        => 200, Style_Before => 0, Style_After => 0,
               Style_Justified     => False,
               Style_String_Prefix =>
                 "\s89\widctlpar\tqr\tldot\tx" & Paper_Width & "\adjustright",
               Style_String_Suffix => "\b\cgrid \sbasedon0 \snext0 ");
            Set_Style
              (TOC_3_Info, Font    => ARM_Output.Swiss,
               Body_Font           => Output_Object.Body_Font, Font_Size => 17,
               Style_Indent        => 400, Style_Before => 0, Style_After => 0,
               Style_Justified     => False,
               Style_String_Prefix =>
                 "\s90\widctlpar\tqr\tldot\tx" & Paper_Width & "\adjustright",
               Style_String_Suffix => "\b\cgrid \sbasedon0 \snext0 ");
            Set_Style
              (TOC_4_Info, Font    => ARM_Output.Swiss,
               Body_Font           => Output_Object.Body_Font, Font_Size => 17,
               Style_Indent        => 600, Style_Before => 0, Style_After => 0,
               Style_Justified     => False,
               Style_String_Prefix =>
         ...
         Set_Style
           (Table_C_Text_Info, Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent => 0, Style_Before => 20, Style_After => 20,
            Style_Justified         => False, Style_String_Prefix => "",
            Style_String_Suffix     => "\qc ");
         -- We use a bit of space above and below to avoid overrunning
         -- the borders of the cells.
         Set_Style
           (Table_L_Text_Info, Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent => 0, Style_Before => 20, Style_After => 20,
            Style_Justified         => False, Style_String_Prefix => "",
            Style_String_Suffix     => "\ql ");
         Set_Style
           (Table_C_Sml_Text_Info, Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent => 0, Style_Before => 20, Style_After => 20,
            Style_Justified             => False, Style_String_Prefix => "",
            Style_String_Suffix         => "\qc ");
         Set_Style
           (Table_L_Sml_Text_Info, Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 15,
            Style_Indent => 0, Style_Before => 20, Style_After => 20,
            Style_Justified             => False, Style_String_Prefix => "",
            Style_String_Suffix         => "\ql ");
         ...
         Set_Style
           (Heading_1_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 36,
            Style_Indent         => 0, Style_Before => 0, Style_After => 210,
            Style_Justified      => False,
            Style_String_Prefix  =>
              "\s1\keepn\widctlpar\outlinelevel0\adjustright",
            Style_String_Suffix =>
              "\b\kerning36\qc\cgrid \sbasedon0 \snext0 ");
         Set_Style
           (Heading_2_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 28,
            Style_Indent         => 0, Style_Before => 240, Style_After => 120,
         ...
         Set_Style
           (Heading_3_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 28,
            Style_Indent         => 0, Style_Before => 210, Style_After => 100,
            Style_Justified      => False,
            Style_String_Prefix  =>
              "\s3\keepn\widctlpar\outlinelevel2\adjustright",
            Style_String_Suffix => "\b\ql\cgrid \sbasedon0 \snext0 ");
         ...
         Set_Style
           (Normal_Paragraph_Number_Info, Font => ARM_Output.Swiss,
            ...
            Style_String_Prefix                =>
              ...
              "\cgrid\qc",
            Style_String_Suffix =>
              "\snext0 "); -- We adjust the space before for each number.
         -- Frame commands:
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Examples, 1), Font => ARM_Output.Fixed,
            Body_Font => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent => INDENT_UNIT * 1, Style_Before => 0,
            Style_After => 100, Style_Justified => False,
            Style_String_Prefix => "\s8\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 1), -- Syntax indented
            Font         => ARM_Output.Default,
            Body_Font    => Output_Object.Body_Font, Font_Size => 22,
            Style_Indent => INDENT_UNIT * 1, Style_Before => 0,
            Style_After  =>
              120, --100, was different from other normals, unknown why
            Style_Justified => True, --FALSE, Not justified 2000-2020,
         --but was in Ada 95. Unknown why the difference,
         --now returned to Ada 95 format.
            Style_String_Prefix => "\s10\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 3), -- Indented
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 22,
            Style_Indent        => INDENT_UNIT * 3, Style_Before => 0,
            Style_After         => 120, Style_Justified => True,
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small, 5), -- Indented annotations
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent        => INDENT_UNIT * 5, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Wide_Hanging, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width => 1_080, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s13\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Narrow_Hanging, 1),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 1,
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 120,
            Style_Justified     => True,
            Style_String_Prefix => "\s14\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Wide_Hanging,
               5), -- Indented two levels for use in indented text.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 1_080, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s15\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Narrow_Hanging,
               3), -- Indented two levels for use in indented text.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 360, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s16\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Bulleted, 1),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 1,
            Style_Hang_Width    => 250, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s17\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Nested_Bulleted, 2),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s18\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Bulleted, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s19\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Nested_Bulleted, 4),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 200, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s20\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         -- Note: Narrower space between.
         Set_Style
           (Paragraph_Info (ARM_Output.Bulleted, 4), -- Indented bulleted
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 250, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s21\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Bulleted, 2), -- Bullets in syntax indenting
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 250, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Bulleted, 3), -- Bullets in syntax indenting
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 250, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
         ...
         -- Note: Narrower space between and afterwards.
         Set_Style
           (Paragraph_Info (ARM_Output.Normal, 2), -- Code indenting
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 22,
            Style_Indent        => INDENT_UNIT * 2, Style_Before => 0,
            Style_After         => 120, Style_Justified => True,
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small, 4), -- Annotations in code indenting
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent        => INDENT_UNIT * 4, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               6), -- Indented examples in annotations.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 6,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s27\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Index, 0), Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent => 270, Style_Hang_Width => 270, Style_Before => 0,
            Style_After => 0, Style_Justified => False,
            Style_String_Prefix => "\s31\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Wide_Above, 0),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => 0, Style_Before => 120,
            Style_After         => 120, Style_Justified => True,
            Style_String_Prefix => "\s32\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\qj\sl-260\slmult0 \snext0 ");
         Set_Style
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Bulleted, 2), -- Bullets in notes
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 250, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s35\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Hanging_in_Bulleted, 3),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 3,
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s37\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Hanging_in_Bulleted, 5),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s38\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Nested_Bulleted,
               4), -- Nested bullets in code indenting
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 220, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Syntax_Summary, 1),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 90, Style_Justified => False,
            Style_String_Prefix => "\s40\widctlpar\adjustright",
         ...
         -- Note: Narrower space between.
         Set_Style
           (Paragraph_Info (ARM_Output.Enumerated, 2), -- Nested enumerations.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 270, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s43\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Enumerated, 4), -- Small nested enumerations.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s44\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Nested_Bulleted, 5), -- Indented nested bullets
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 250, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s47\widctlpar\adjustright",
            Style_String_Suffix =>
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small, 3), -- Annotations in syntax indenting
            Font                => ARM_Output.Default,
            Body_Font           => Output_Object.Body_Font, Font_Size => 18,
            Style_Indent        => INDENT_UNIT * 4, Style_Before => 0,
            Style_After         => 90, Style_Justified => True,
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Swiss_Examples, 1),
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 20, Style_Indent => INDENT_UNIT * 1,
            Style_Before => 0, Style_After => 110, Style_Justified => False,
            Style_String_Prefix => "\s51\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Swiss_Examples, 4), -- Indented examples
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 20, Style_Indent => INDENT_UNIT * 4,
            Style_Before => 0, Style_After => 110, Style_Justified => False,
            Style_String_Prefix => "\s53\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-240\slmult0 \snext53 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Swiss_Examples, 6), -- Indented examples
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 6,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s54\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Enumerated,
               5), -- Small doubly nested enumerations.
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 5,
            Style_Hang_Width    => 240, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               4), -- Syntax Indented examples in annotations.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 4,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s58\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Swiss_Examples, 2), -- Syntax Indented examples
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 20, Style_Indent => INDENT_UNIT * 2,
            Style_Before => 0, Style_After => 110, Style_Justified => False,
            Style_String_Prefix => "\s59\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-240\slmult0 \snext59 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Swiss_Examples, 4), -- Syntax Indented examples
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 4,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s60\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               5), -- Code Indented examples in annotations.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 5,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s62\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-170\slmult0 \snext62 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Swiss_Examples, 3), -- Code Indented examples
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 20, Style_Indent => INDENT_UNIT * 3,
            Style_Before => 0, Style_After => 110, Style_Justified => False,
            Style_String_Prefix => "\s63\widctlpar\adjustright",
            Style_String_Suffix => "\cgrid\ql\sl-240\slmult0 \snext63 ");
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Swiss_Examples, 5), -- Code Indented examples
            Font => ARM_Output.Swiss, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 5,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s64\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Title, 0), Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font,
            Font_Size                                  =>
              42, -- Slight less than double (3 pts larger than Heading_1).
            Style_Indent        => 0, Style_Before => 120, Style_After => 120,
            Style_Justified     => True,
            Style_String_Prefix => "\s65\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Giant_Hanging, 4),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width => 1_440, Style_Before => 0, Style_After => 110,
            Style_Justified     => True,
            Style_String_Prefix => "\s66\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Medium_Hanging, 2),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 2,
            Style_Hang_Width    => 720, Style_Before => 0, Style_After => 120,
            Style_Justified     => True,
            Style_String_Prefix => "\s67\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Small_Giant_Hanging, 6),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 18, Style_Indent => INDENT_UNIT * 6,
            Style_Hang_Width    => 1_440, Style_Before => 0, Style_After => 90,
            Style_Justified     => True,
            Style_String_Prefix => "\s68\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info (ARM_Output.Wide_Hanging, 4),
            Font => ARM_Output.Default, Body_Font => Output_Object.Body_Font,
            Font_Size           => 22, Style_Indent => INDENT_UNIT * 4,
            Style_Hang_Width => 1_080, Style_Before => 0, Style_After => 120,
            Style_Justified     => True,
            Style_String_Prefix => "\s72\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples, 2), -- Indented for notes.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 2,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s80\widctlpar\adjustright",
         ...
         Set_Style
           (Paragraph_Info
              (ARM_Output.Small_Examples,
               7), -- Child Indented examples in annotations.
            Font => ARM_Output.Fixed, Body_Font => Output_Object.Body_Font,
            Font_Size           => 16, Style_Indent => INDENT_UNIT * 7,
            Style_Before => 0, Style_After => 80, Style_Justified => False,
            Style_String_Prefix => "\s85\widctlpar\adjustright",
   ...
   -- New styles should be added here, following numbers will need adjustments.
         Set_Style
           (Heading_4_Info, Font => ARM_Output.Swiss,
            Body_Font            => Output_Object.Body_Font, Font_Size => 27,
            Style_Indent         => 0, Style_Before => 210, Style_After => 100,
            Style_Justified      => False,
            Style_String_Prefix  =>
              "\s87\keepn\widctlpar\outlinelevel3\adjustright",
         ...
         if Output_Object.Big_Files then
            ...
            Set_Style
              (TOC_1_Info, Font    => ARM_Output.Swiss,
               Body_Font           => Output_Object.Body_Font, Font_Size => 24,
               Style_Indent        => 0, Style_Before => 60, Style_After => 60,
               Style_Justified     => False,
               Style_String_Prefix =>
                 "\s88\widctlpar\tqr\tldot\tx" & Paper_Width & "\adjustright",
               Style_String_Suffix => "\b\cgrid \sbasedon0 \snext0 ");
         ...
         Set_Style
           (Table_C_Text_Info, Font => ARM_Output.Default,
            Body_Font => Output_Object.Body_Font, Font_Size => 22,
            Style_Indent => 0, Style_Before => 10, Style_After => 10,
            Style_Justified         => False, Style_String_Prefix => "",
            Style_String_Suffix     => "\qc ");
         -- We use a bit of space above and below to avoid overrunning
         -- the borders of the cells.
      ...
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Normal, 0));
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Large Paragraph Number;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Small, 1));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Notes;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Small, 2));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Annotations;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Examples, 1));
      ...
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Normal, 1));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Syntax Indented;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Normal, 3));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Normal Indented;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Small, 5));
      ...
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Narrow_Hanging, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Narrow Hanging;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Bulleted, 1));
      ...
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Nested_Bulleted, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Nested Bulleted;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Bulleted, 4));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Indented Bulleted;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Bulleted, 2));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Syntax Indented Bulleted;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Bulleted, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Code Indented Bulleted;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Normal, 2));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Code Indented;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Small, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Code Indented;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Examples, 4));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Examples, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Indented Examples;}");
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "{\*\cs30 \additive \sbasedon30 page number;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Index, 0));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Index;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Wide_Above, 0));
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Notes Nested Bulleted;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Hanging_in_Bulleted, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Hanging in Bulleted;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Hanging_in_Bulleted, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Hanging in Bulleted;}");
      Write_Style
        (Output_Object.Output_File,
      ...
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Enumerated, 1));
      ...
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Enumerated, 2));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Nested Enumerated;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Enumerated, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Nested Enumerated;}");
      Write_Style
        (Output_Object.Output_File,
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Nested X2 Bulleted;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Nested_Bulleted, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Indented Nested Bulleted;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Normal, 4));
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "Inner Indented;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Small, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Inner Indented;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Small, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Syntax Indented;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Swiss_Examples, 1));
      ...
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Swiss_Examples, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Swiss Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Swiss_Examples, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Swiss Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Swiss_Examples, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Swiss Indented Examples;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Enumerated, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Nested X2 Enumerated;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Enumerated, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Nested X2 Enumerated;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Examples, 2));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Syntax Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Examples, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Syntax Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Swiss_Examples, 2));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Swiss Syntax Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Swiss_Examples, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Swiss Syntax Indented Examples;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Examples, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Code Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Examples, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Code Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Swiss_Examples, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Swiss Code Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Swiss_Examples, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Swiss Code Indented Examples;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Title, 0));
      ...
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Giant_Hanging, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Giant Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Medium_Hanging, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Medium Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Narrow_Hanging, 2));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Indented Narrow Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Medium_Hanging, 3));
      Ada.Text_IO.Put_Line
      ...
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Narrow_Hanging, 4));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Indented Narrow Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Medium_Hanging, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Indented Medium Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Wide_Hanging, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Indented Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Narrow_Hanging, 3));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Dbl Indented Narrow Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Medium_Hanging, 4));
      Ada.Text_IO.Put_Line
      ...
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Narrow_Hanging, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Dbl Indented Narrow Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Medium_Hanging, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Dbl Indented Medium Hanging;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Examples, 2));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Notes Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Swiss_Examples, 1));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Swiss Basic Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Swiss_Examples, 2));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Swiss Indented Basic Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Examples, 1));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Basic Examples;}");
      Write_Style
        (Output_Object.Output_File, Paragraph_Info (ARM_Output.Examples, 5));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Child Indented Examples;}");
      Write_Style
        (Output_Object.Output_File,
         Paragraph_Info (ARM_Output.Small_Examples, 6));
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "Small Child Indented Examples;}");
      if Output_Object.Big_Files then
      ...
      for Version in Character range '1' .. '9' loop
         ...
         then
            ...
            exit; -- No more after the first empty one.
         end if;
      end loop;
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "{\author AXE Consultants}"); -- Working.
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "{\operator Randall Brukardt, Principle Technician}}");
      ...
      if Output_Object.Includes_Changes then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "\revisions\revprop3 \revbar0 ");
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "\pgbrdrhead\pgbrdrfoot\fet0");
   ...
   procedure Create
     (Output_Object : in out RTF_Output_Type; Page_Size : in ARM_RTF.Page_Size;
      Includes_Changes        : in     Boolean; Big_Files : in Boolean;
      File_Prefix             : in     String; Output_Path : in String;
      ...
      Header_Prefix           : in String := ""; Footer_Use_Date : in Boolean;
      Footer_Use_Clause_Name  : in Boolean; Footer_Use_ISO_Format : in Boolean;
   ...
   begin
      ...
      Ada.Strings.Fixed.Move
        (Target => Output_Object.File_Prefix, Source => File_Prefix);
         ...
         Start_RTF_File
           (Output_Object,
            Ada.Strings.Fixed.Trim
              (Output_Object.File_Prefix, Ada.Strings.Right),
            Ada.Strings.Unbounded.To_String (Output_Object.Title), "All");
   ...
   procedure Section
     (Output_Object : in out RTF_Output_Type; Section_Title : in String;
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Section in paragraph");
      end if;
      if not Output_Object.Big_Files then
         ...
         -- Just a new section header (odd page break) and page number setup:
            if Section_Name = "TOC" then
               ...
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "\sect\sbkodd\pgnlcrm\pgncont ");
               ...
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "\sect\sbkodd\pgndec\pgncont ");
   ...
   begin
      ...
      case Number_of_Columns is
         when 1 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols1 ");
         when 2 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols2\colsx0 ");
            -- Two columns, no space between. (Paragraph formatting
            -- will take care of that.)
         when 3 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols3\colsx0 ");
         when 4 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols4\colsx0 ");
         when 5 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols5\colsx0 ");
         when 6 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols6\colsx0 ");
         when 7 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols7\colsx0 ");
         when 8 =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cols8\colsx0 ");
      end case;
   ...
   begin
      case Style is
         when ARM_Output.Normal | ARM_Output.Wide_Above | ARM_Output.Small |
           ARM_Output.Small_Wide_Above | ARM_Output.Header |
           ARM_Output.Small_Header | ARM_Output.Index |
           ARM_Output.Syntax_Summary | ARM_Output.Title | ARM_Output.Examples |
           ARM_Output.Small_Examples | ARM_Output.Swiss_Examples |
           ARM_Output.Small_Swiss_Examples =>
            if Output_Object.Tab_Stops.Number /= 0 then
               ...
               declare
                  function Stop_in_Twips
                    (Stop : ARM_Output.Tab_Stop_Type) return Natural
                  is
               ...
               begin
                  for I in 1 .. Output_Object.Tab_Stops.Number loop
                     ...
                     begin
                        ...
                        Ada.Text_IO.Put
                          (Output_Object.Output_File, Num (2 .. Num'Length));
         ...
         when ARM_Output.Bulleted | ARM_Output.Nested_Bulleted |
           ...
           ARM_Output.Hanging_in_Bulleted | ARM_Output.Small_Giant_Hanging |
           ARM_Output.Small_Wide_Hanging | ARM_Output.Small_Medium_Hanging |
           ARM_Output.Small_Narrow_Hanging |
           ARM_Output.Small_Hanging_in_Bulleted | ARM_Output.Enumerated |
           ARM_Output.Small_Enumerated =>
            if Output_Object.Tab_Stops.Number /= 0 then
               Ada.Exceptions.Raise_Exception
                 (ARM_Output.Not_Valid_Error'Identity,
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Already in paragraph");
      end if;
      if not Paragraph_Info (Style, Indent).Defined then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity,
            "Undefined Style " &
            ARM_Output.Paragraph_Style_Type'Image (Style) & " and Indent:" &
      ...
      if Number /= "" then -- We have a paragraph number.
         ...
         if Number'Length > 7 then
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Wide_Paragraph_Number_Info,
               Output_Object.Char_Count);
         else
            Write_Style_for_Paragraph
         ...
         declare
            Diff : Natural :=
              (Paragraph_Info (Style, Indent).Size -
         ...
         begin
            if Diff >= 30 then
               Ada.Text_IO.Put
                 (Output_Object.Output_File,
                  "\sb3" & Character'Val (Diff mod 10 + Character'Pos ('0')) &
                  "0 ");
            elsif Diff >= 20 then
               Ada.Text_IO.Put
                 (Output_Object.Output_File,
                  "\sb2" & Character'Val (Diff mod 10 + Character'Pos ('0')) &
                  "0 ");
            elsif Diff >= 10 then
               Ada.Text_IO.Put
                 (Output_Object.Output_File,
                  "\sb1" & Character'Val (Diff mod 10 + Character'Pos ('0')) &
                  "0 ");
            else
               Ada.Text_IO.Put
                 (Output_Object.Output_File,
                  "\sb" & Character'Val (Diff + Character'Pos ('0')) & "0 ");
            end if;
         end;
--*** Box width??
      ...
      case Style is
         when ARM_Output.Normal | ARM_Output.Wide_Above | ARM_Output.Small |
           ...
           ARM_Output.Small_Swiss_Examples =>
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Paragraph_Info (Style, Indent),
               Output_Object.Char_Count);
...
--Ada.Text_IO.Put_Line ("Start paragraph - full style (hang)");
      ...
      if No_Breaks or Keep_with_Next then
         if Output_Object.Char_Count + 13 > LINE_LENGTH then
            Ada.Text_IO.New_Line (Output_Object.Output_File);
            Output_Object.Char_Count := 0;
         end if;
         if No_Breaks then
      ...
      if ARM_Output."=" (Space_After, ARM_Output.Narrow) then
         ...
         declare
            SA : constant String :=
              Natural'Image
                ((Paragraph_Info (Style, Indent).After *
                  (LEADING_PERCENT / 10)) /
                 10);
         begin
            if Output_Object.Char_Count + 4 + SA'Length - 1 > LINE_LENGTH then
               Ada.Text_IO.New_Line (Output_Object.Output_File);
               Output_Object.Char_Count := 0;
            end if;
      ...
      if ARM_Output."/=" (Justification, ARM_Output.Default) then
         if Output_Object.Char_Count + 4 > LINE_LENGTH then
            Ada.Text_IO.New_Line (Output_Object.Output_File);
            Output_Object.Char_Count := 0;
         end if;
...
--Ada.Text_Io.Put ("Start Hang:");
   ...
   begin
      ...
      if not Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Not in paragraph");
      end if;
...
--Ada.Text_IO.Put_Line ("End paragraph '}'");
   ...
   procedure Category_Header
     (Output_Object : in out RTF_Output_Type; Header_Text : String)
   is
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Header in paragraph");
      end if;
      Ada.Text_IO.New_Line (Output_Object.Output_File);
      Write_Style_for_Paragraph
        (Output_Object.Output_File, Category_Header_Info, Count);
   ...
   begin
      case Month is
         when 1 =>
            return Day (2 .. Day'Last) & " January" & Year;
         when 2 =>
            return Day (2 .. Day'Last) & " February" & Year;
         when 3 =>
            return Day (2 .. Day'Last) & " March" & Year;
         when 4 =>
            return Day (2 .. Day'Last) & " April" & Year;
         when 5 =>
            return Day (2 .. Day'Last) & " May" & Year;
         when 6 =>
            return Day (2 .. Day'Last) & " June" & Year;
         when 7 =>
            return Day (2 .. Day'Last) & " July" & Year;
         when 8 =>
            return Day (2 .. Day'Last) & " August" & Year;
         when 9 =>
            return Day (2 .. Day'Last) & " September" & Year;
         when 10 =>
            return Day (2 .. Day'Last) & " October" & Year;
         when 11 =>
            return Day (2 .. Day'Last) & " November" & Year;
         when 12 =>
            return Day (2 .. Day'Last) & " December" & Year;
      end case;
   end Current_Date;

   procedure Clause_Footer
     (Output_Object : in out RTF_Output_Type; Header_Text : in String;
   ...
   begin
      ...
      if Output_Object.Wrote_into_Section then
         -- Start a new section:
         if No_Page_Break or else ARM_Contents."=" (Level, ARM_Contents.Clause)
           or else ARM_Contents."=" (Level, ARM_Contents.Subclause)
           or else ARM_Contents."=" (Level, ARM_Contents.Subsubclause)
         then
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "\sect\sbknone\pgncont ");
         else
            -- Start sections on a new page. (Should only happen in the
            -- introduction).
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "\sect\sbkpage\pgncont ");
         end if;
         -- else just use the existing section.
      end if;
      if Clause_Number = ""
        or else ARM_Contents."=" (Level, ARM_Contents.Unnumbered_Section)
      then
         Ada.Text_IO.Put (Output_Object.Output_File, "{\footerl ");
         Write_Style_for_Paragraph
           (Output_Object.Output_File, Footer_Info, Count);
         ...
         if Output_Object.Footer_Use_ISO_Format then
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "\~\~\~\~\~\~{\f1\fs22\b {\field{\*\fldinst { PAGE }}{\fldrslt {\lang1024 x}}}}\par}}}");
         else
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
         ...
         if Output_Object.Footer_Use_ISO_Format then
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "{\f1\fs22\b {\field{\*\fldinst { PAGE }}{\fldrslt {\lang1024 x}}}}\~\~\~\~\~\~");
         elsif ARM_Output."=" (Output_Object.Body_Font, ARM_Output.Swiss) then
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "{\f1 {\field{\*\fldinst { PAGE }}{\fldrslt {\lang1024 x}}}}\~\~\~\~\~\~");
         else
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "{\f0 {\field{\*\fldinst { PAGE }}{\fldrslt {\lang1024 x}}}}\~\~\~\~\~\~");
         end if;
         if Output_Object.Footer_Use_Date then
            Ada.Text_IO.Put (Output_Object.Output_File, Current_Date);
            -- else no date.
         ...
         if Output_Object.Footer_Use_Clause_Name then
            Ada.Text_IO.Put (Output_Object.Output_File, "{\b\f1 ");
            if Level in
                ARM_Contents.Plain_Annex .. ARM_Contents.Normative_Annex
            then
            ...
            if Output_Object.Footer_Use_ISO_Format then
               Ada.Text_IO.Put
                 (Output_Object.Output_File, "}\~\~\~{\f1\fs16 ");
   ...
   begin
      ...
      -- Adjust footer.
      Clause_Footer
        (Output_Object, Header_Text, Level, Clause_Number, No_Page_Break);

      -- Special for table of contents:
      if Clause_Number = ""
        and then
        (Header_Text =
         "Table of Contents" or else -- Ada 95 format

           Header_Text = "Contents")
      then -- ISO 2004 format.
         if Header_Text = "Table of Contents" then
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "{\pard\plain \s1\sb240\sa60\keepn\widctlpar\outlinelevel0\adjustright \b\f1\fs36\kerning36\qc\cgrid Table of Contents\par}");
         else
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "{\pard\plain \s1\sb240\sa60\keepn\widctlpar\outlinelevel0\adjustright \b\f1\fs36\kerning36\qc\cgrid Contents\par}");
         end if;
         Output_Object.Char_Count := 0;
         return;
      end if;
      ...
      case Level is
         when ARM_Contents.Plain_Annex =>
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Heading_1_Info, Count);
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
         ...
         when ARM_Contents.Normative_Annex =>
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Heading_1_Info, Count);
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, Clause_Number & "\line ");
            -- Note: Clause_Number includes "Annex"
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "{\b0 (normative)}\line ");
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, Header_Text & "\par}");
         when ARM_Contents.Informative_Annex =>
            ...
            -- Note: Clause_Number includes "Annex"
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "{\b0 (informative)}\line ");
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, Header_Text & "\par}");
         ...
         when ARM_Contents.Clause =>
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Heading_2_Info, Count);
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               Clause_Number & " " & Header_Text & "\par}");
         when ARM_Contents.Subclause =>
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Heading_3_Info, Count);
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               Clause_Number & " " & Header_Text & "\par}");
         when ARM_Contents.Subsubclause =>
            Write_Style_for_Paragraph
              (Output_Object.Output_File, Heading_4_Info, Count);
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               Clause_Number & " " & Header_Text & "\par}");
   ...
   procedure Revised_Clause_Header
     (Output_Object : in out RTF_Output_Type; New_Header_Text : in String;
   ...
   is
      ...
      begin
         if Old_Version = '0' then -- Old is original text
            return
              "{\revised\revauth" & Version & " " & New_Header_Text &
              "}{\deleted\revauthdel" & Version & " " & Old_Header_Text & "}";
         else
            return
              "{\revised\revauth" & Version & " " & New_Header_Text &
              "}{\deleted\revauthdel" & Version & "\revised\revauth" &
              Old_Version & " " & Old_Header_Text & "}";
         end if;
      end Header_Text;
   begin
      if not Output_Object.Is_Valid then
      ...
      -- Adjust footer.
      Clause_Footer
        (Output_Object, New_Header_Text, Level, Clause_Number, No_Page_Break);
   ...
   procedure TOC_Marker
     (Output_Object : in out RTF_Output_Type; For_Start : in Boolean)
   is
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "TOC in paragraph");
      end if;
      if Output_Object.Big_Files then
         if For_Start then
   ...
   begin
      ...
      case Kind is
         when ARM_Output.Any_Page =>
            if Output_Object.Is_In_Paragraph then
               Ada.Exceptions.Raise_Exception
                 (ARM_Output.Not_Valid_Error'Identity, "Page in paragraph");
            end if;
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "\sect\sbkpage\pgncont ");
      ...
      -- item is a clause header (as it usually is).
         ...
         when ARM_Output.Odd_Page_Only =>
            ...
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "\sect\sbkodd\pgncont ");
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "New Column in paragraph");
      end if;
      ...
      declare
         FS : constant String :=
           Natural'Image
             (Paragraph_Info
                (Output_Object.Paragraph_Style, Output_Object.Paragraph_Indent)
                .Size);
      begin
   ...
   procedure Separator_Line
     (Output_Object : in out RTF_Output_Type; Is_Thin : Boolean := True)
   is
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Separator in paragraph");
      end if;
      Ada.Text_IO.New_Line (Output_Object.Output_File);
      if Is_Thin then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "{\pard \widctlpar\brdrb\brdrs\brdrw15\brsp20 \adjustright \fs4\par }");
         ...
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "{\pard \widctlpar\brdrb\brdrs\brdrw30\brsp20 \adjustright \fs4\par }");
      end if;
      Ada.Text_IO.New_Line (Output_Object.Output_File);
   end Separator_Line;
   ...
   type Table_Info_Kind is
     (Caption, Header, Header_no_Caption, First_Row, Row, Last_Row);

   procedure RTF_Table_Info
     (Output_Object : in out RTF_Output_Type; Kind : in Table_Info_Kind)
   is
   ...
   begin
      ...
      if Output_Object.Table_No_Page_Break then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "\trkeep\trkeepfollow ");
      ...
      case Kind is
         when Caption =>
            ...
            if Output_Object.Table_Has_Border then
               ...
               Ada.Text_IO.Put_Line (Output_Object.Output_File, "\clvertalc ");
            end if;
            ...
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "\cltxlrtb\cellx" &
               Format_Value
                 (Output_Object.Table_Indent + Output_Object.Table_Width) &
               " ");
            ...
            if Output_Object.Table_Has_Small_Text then
               Write_Style_for_Paragraph
                 (Output_Object.Output_File, Table_C_Sml_Text_Info,
                  Output_Object.Char_Count);
               Output_Object.Real_Size := Table_C_Sml_Text_Info.Size;
               Output_Object.Size      := 0;
            else
               Write_Style_for_Paragraph
                 (Output_Object.Output_File, Table_C_Text_Info,
                  Output_Object.Char_Count);
         ...
         when Header | Header_no_Caption =>
            ...
            for I in 1 .. Output_Object.Column_Count loop
               if Output_Object.Table_Has_Border then
                  ...
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File, "\clvertalc ");
               end if;
...
--      " Count:" & Natural'Image(I+Output_Object.Table_First_Column_Mult-1));
               if I /= Output_Object.Column_Count then
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
                     "\cltxlrtb\cellx" &
                     Format_Value
                       (Output_Object.Table_Indent +
                        Output_Object.Table_Column_Width *
                          (Integer
                             (I + Output_Object.Table_First_Column_Mult -
                              1))) &
                     " ");
            ...
            if Output_Object.Table_Alignment = ARM_Output.Center_All then
               ...
               if Output_Object.Table_Has_Small_Text then
                  Write_Style_for_Paragraph
                    (Output_Object.Output_File, Table_L_Sml_Text_Info,
                     Output_Object.Char_Count);
                  Output_Object.Real_Size := Table_L_Sml_Text_Info.Size;
                  Output_Object.Size      := 0;
               else
                  Write_Style_for_Paragraph
                    (Output_Object.Output_File, Table_L_Text_Info,
                     Output_Object.Char_Count);
   ...
   begin
      ...
      if Output_Object.Is_In_Paragraph then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Table in paragraph");
      end if;
      ...
      Output_Object.Table_Column_Width :=
        Output_Object.Table_Width / (Column_Units);
...
--      " Column width:" & Natural'Image(Output_Object.Table_Column_Width));
   ...
   begin
      ...
      if (not Output_Object.Is_In_Paragraph) or (not Output_Object.Is_In_Table)
      then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Table marker not in table");
      end if;
      case Marker is
         when ARM_Output.End_Item =>
            if Output_Object.Table_Alignment = ARM_Output.Center_except_First
            then
         ...
         when ARM_Output.End_Caption =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cell }");
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "\row "); -- End row.

            -- Start header row:
            RTF_Table_Info
              (Output_Object, Kind => Header); -- Repeat table definition.
         ...
         when ARM_Output.End_Header =>
            ...
            -- Start 1st body row:
            RTF_Table_Info
              (Output_Object, Kind => First_Row); -- Repeat table definition.
         ...
         when ARM_Output.End_Row =>
            ...
            RTF_Table_Info
              (Output_Object, Kind => Row); -- Repeat table definition.
         ...
         when ARM_Output.End_Row_Next_Is_Last =>
            ...
            -- Start other body rows (no top border!):
            RTF_Table_Info
              (Output_Object, Kind => Last_Row); -- Repeat table definition.

         when ARM_Output.End_Table =>
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "\cell }");
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "\row "); -- End last row of table.
   ...
   procedure Ordinary_Text
     (Output_Object : in out RTF_Output_Type; Text : in String)
   is
   ...
   begin
      ...
      if Ada.Strings.Fixed.Count (Text, Special_Set) = 0
        and then
        (Output_Object.Paragraph_Style not in
           ARM_Output.Text_Prefixed_Style_Subtype
         or else Output_Object.Saw_Hang_End)
      then
   ...
   procedure Ordinary_Character
     (Output_Object : in out RTF_Output_Type; Char : in Character)
   is
   ...
   begin
      ...
      if Char = ' ' and then Output_Object.Char_Count >= LINE_LENGTH - 5 then
         ...
         if Char = '\' then
            ...
            Ada.Text_IO.Put (Output_Object.Output_File, "\}");
            Output_Object.Char_Count := Output_Object.Char_Count + 2;
         elsif Char >= Character'Val (126)
         then -- All higher Latin-1 characters.
            declare
                  ...
                  if Code / 16 >= 10 then
                     Ada.Text_IO.Put
                       (Output_Object.Output_File,
                        "\'" &
                        Character'Val
                          ((Code / 16 - 10) + Character'Pos ('a')) &
                        Character'Val
                          ((Code mod 16 - 10) + Character'Pos ('a')));
                  else
                     Ada.Text_IO.Put
                       (Output_Object.Output_File,
                        "\'" &
            ...
            if Output_Object.Paragraph_Style in
                ARM_Output.Text_Prefixed_Style_Subtype
              and then (not Output_Object.Saw_Hang_End)
            then
...
--Ada.Text_Io.Put (Char);
               if Char in 'A' .. 'H' or else Char in 'J' .. 'Z'
                 or else -- Capital 'I' is narrow.
               Char in '0' .. '9' or else Char = 'm'
                 or else
                   Char =
                   'w'
                 or else -- Small 'm' and 'w' are extra wide for lower-case characters.

                   Char = '+' or else Char = '_'
                 or else Char = '@' or else Char = '#' or else Char = '$'
                 or else Char = '%' or else Char = '&' or else Char = '*'
   ...
   begin
      ...
      then
         ...
         begin
            if ARM_Output."="
                (Output_Object.Current_Space_After, ARM_Output.Narrow)
            then
               SA_Width := SA_Width * (LEADING_PERCENT / 10) / 10;
            elsif ARM_Output."="
                (Output_Object.Current_Space_After, ARM_Output.Wide)
            then
         ...
         if
           (Output_Object.Paragraph_Style in
              ARM_Output.Bulleted .. ARM_Output.Small_Hanging_in_Bulleted)
         then -- Always "NoPrefix" here.
            Ada.Text_IO.Put (Output_Object.Output_File, "\tab ");
            Output_Object.Char_Count := Output_Object.Char_Count + 5;
         end if;
      end if;
   ...
   begin
      ...
      if ARM_Output."/=" (Output_Object.Paragraph_Style, ARM_Output.Index)
        or else ARM_Output."/=" (Output_Object.Paragraph_Indent, 0)
      then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Not in index paragraph");
      end if;
      ...
      if Clear_Keep_with_Next then
         ...
         Start_Paragraph
           (Output_Object, ARM_Output.Index, Indent => 0, Number => "",
...
-- a hyphen will be added to the text.
   ...
   begin
      ...
      if ARM_Output."=" (Output_Object.Tab_Stops, ARM_Output.NO_TABS) then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Tab, but none set");
      end if;
      Ada.Text_IO.Put (Output_Object.Output_File, "\tab ");
      Output_Object.Char_Count := Output_Object.Char_Count + 5;
   end Tab;
   ...
   begin
      ...
      Ada.Text_IO.Put
        (Output_Object.Output_File,
         "\uc" & Len (2 .. Len'Last) & "\u" & Char_Code (2 .. Char_Code'Last) &
         " <U" & Char_Code (2 .. Char_Code'Last) & ">");
      Output_Object.Char_Count :=
        Output_Object.Char_Count + 9 + Len'Last - 1 + (Char_Code'Last - 1) * 2;
      if Output_Object.Paragraph_Style in
          ARM_Output.Text_Prefixed_Style_Subtype
        and then (not Output_Object.Saw_Hang_End)
      then
   ...
   begin
      ...
      then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Not a hanging paragraph");
      end if;
...
--Ada.Text_Io.Put_Line (" No_Break");
         ...
         Current_Format :=
           (Bold => Output_Object.Is_Bold, Italic => Output_Object.Is_Italic,
            Font          => Output_Object.Font, Size => Output_Object.Size,
            Color => Output_Object.Color, Change => Output_Object.Change,
         ...
         Write_Style_for_Paragraph
           (Output_Object.Output_File,
            ...
            Output_Object.Char_Count);
         Set_Tabs
           (Output_Object, Output_Object.Paragraph_Style,
...
--Ada.Text_Io.Put_Line (" Break");
   ...
   is
         ...
         if Output_Object.Real_Size >= 50 then
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "\fs5" &
               Character'Val
                 (Output_Object.Real_Size mod 10 + Character'Pos ('0')));
      ...
      begin
         if (not Output_Object.Is_Bold) and (not Output_Object.Is_Italic) and
           ...
           Output_Object.Color = ARM_Output.Default and Output_Object.Size = 0
         then
         ...
         Ada.Text_IO.Put (Output_Object.Output_File, "}");
         Output_Object.Char_Count := Output_Object.Char_Count + 1;
         Output_Object.Real_Size  :=
           Output_Object.Real_Size - (Integer (Output_Object.Size) * 2);
         Output_Object.Size      := 0;
         Output_Object.Is_Bold   := False;
         Output_Object.Is_Italic := False;
         Output_Object.Color     := ARM_Output.Default;
         case Output_Object.Font is
            ...
            when ARM_Output.Swiss =>
               ...
               if ARM_Output."/=" (Output_Object.Body_Font, ARM_Output.Swiss)
               then
      ...
      begin
         if (not Format.Bold) and (not Format.Italic) and
           ...
           Format.Color = ARM_Output.Default and Format.Size = 0
         then
         ...
         if Format.Size /= 0 then
            if TRACE_TF then
               Ada.Text_IO.Put
                 (" Change size " & ARM_Output.Size_Type'Image (Format.Size));
            end if;
            Output_Object.Real_Size :=
              Output_Object.Real_Size + Integer (Format.Size) * 2;
            if ARM_Output."/=" (Format.Font, ARM_Output.Swiss)
              or else ARM_Output."="
                (Output_Object.Body_Font, ARM_Output.Swiss)
            then
   ...
   begin
      ...
      if Format.Change /= Output_Object.Change
        or else Format.Version /= Output_Object.Version
        or else Format.Added_Version /= Output_Object.Added_Version
      then
         Close_Basic_Format;
         Close_Location;
         Close_Revision;
         ...
         Make_Location;
         Make_Basic_Format;
      elsif Format.Color /= Output_Object.Color
        or else Format.Size /= Output_Object.Size
        or else ARM_Output."/=" (Format.Font, Output_Object.Font)
        or else Format.Bold /= Output_Object.Is_Bold
        or else Format.Italic /= Output_Object.Is_Italic
      then
         Close_Basic_Format;
         Make_Basic_Format;
   ...
   procedure Index_Target
     (Output_Object : in out RTF_Output_Type; Index_Key : in Natural)
   is
   ...
   procedure Local_Link_Start
     (Output_Object : in out RTF_Output_Type; Target : in String;
      Clause_Number : in     String)
   is
   -- Generate a local link to the target and clause given.
   ...
   procedure Picture
     (Output_Object : in out RTF_Output_Type; Name : in String;
   ...
   is
      ...
      procedure Get_Picture_Dimensions is
         ...
         function Convert is new Ada.Unchecked_Conversion
           (Source => PNG_Stream, Target => PNG_Header);
         Buffer : PNG_Stream;
      ...
      begin
         begin
            Ada.Streams.Stream_IO.Open
              (Fyle, Mode => Ada.Streams.Stream_IO.In_File,
               Name       =>
                 Ada.Strings.Unbounded.To_String (Output_Object.Output_Path) &
                 Name);
         exception
         ...
         if Last /= 24 then
            Ada.Exceptions.Raise_Exception
              (ARM_Output.Not_Valid_Error'Identity, "Picture file too short");
         end if;
         if Convert (Buffer).Signature_1 = 16#89_50_4e_47#
           and then Convert (Buffer).Signature_2 = 16#0d_0a_1a_0a#
         then
            -- This is a PNG file:
            Picture_Kind   := PNG;
            ...
            Ada.Text_IO.Put_Line
              ("Forward PNG: Width=" & DWord'Image (Picture_Width) &
               " Height=" & DWord'Image (Picture_Height));
         elsif Convert (Buffer).Signature_1 = 16#47_4e_50_89#
           and then Convert (Buffer).Signature_2 = 16#0a_1a_0a_0d#
         then
            ...
            Picture_Height := Convert (Buffer).Height;
            Ada.Text_IO.Put_Line
              ("Reversed PNG: Width=" & DWord'Image (Picture_Width) &
               " Height=" & DWord'Image (Picture_Height));
...
--              Picture_Kind := JPEG;
            ...
            Ada.Text_IO.Put_Line
              ("** Unimplemented picture formatting: File type");
            Ada.Exceptions.Raise_Exception
              (ARM_Output.Not_Valid_Error'Identity,
   ...
   begin
      ...
      begin
         Width_Scale :=
           Float (Width) / Float (Picture_Width) * 100.0 * Word_Scaling;
         Height_Scale :=
           Float (Height) / Float (Picture_Height) * 100.0 * Word_Scaling;
         ...
         Ada.Text_IO.Put_Line
           ("Picture scaling (%):" & Natural'Image (Picture_Scaling));
         Ada.Text_IO.Put_Line
           ("Box width=" & Natural'Image (Width) & " Height=" &
         ...
         Ada.Text_IO.Put_Line
           ("Word 2003 scaling: Width=" &
            Natural'Image
              (Natural
                 (Float (Width) / Float (Picture_Width) * 100.0 / 0.75)) &
            " Height=" &
            Natural'Image
              (Natural
                 (Float (Height) / Float (Picture_Height) * 100.0 / 0.75)));
      ...
      Ada.Text_IO.Put
        (Output_Object.Output_File, "{\shp{\*\shpinst"); -- Start a shape.
      Ada.Text_IO.Put
        (Output_Object.Output_File,
      ...
      Ada.Text_IO.Put
        (Output_Object.Output_File,
         "\shpright" &
         Format_Twips
           (Width * HORIZONTAL_TWIPS_PER_PIXEL)); -- Right edge in twips.
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "\shpbottom" &
         Format_Twips
           (Height * VERTICAL_TWIPS_PER_PIXEL)); -- Bottom edge in twips.
      Ada.Text_IO.Put
        (Output_Object.Output_File,
      ...
      Ada.Text_IO.Put
        (Output_Object.Output_File, "\shpfblwtxt0"); -- Text is below shape.
      Ada.Text_IO.Put
        (Output_Object.Output_File,
      ...
      Ada.Text_IO.Put
        (Output_Object.Output_File,
         "\picw" &
         Format_Twips
           (Natural (Picture_Width) *
            5)); -- Raw picture width in ??? (doesn't seem to be used).
      Ada.Text_IO.Put
        (Output_Object.Output_File,
         "\pich" &
         Format_Twips
           (Natural (Picture_Height) *
            5)); -- Raw picture height in ??? (doesn't seem to be used).
      Ada.Text_IO.Put
        (Output_Object.Output_File,
         "\picwgoal" &
         Format_Twips
           (Width *
            HORIZONTAL_TWIPS_PER_PIXEL)); -- Picture width goal in twips.
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "\pichgoal" &
         Format_Twips
           (Height *
            VERTICAL_TWIPS_PER_PIXEL)); -- Picture height goal in twips.
      ...
      Ada.Text_IO.Put
        (Output_Object.Output_File, "}}}"); -- End the picture data.
      ...
      Ada.Text_IO.Put_Line (Output_Object.Output_File, "}}"); -- End the shape.
      Output_Object.Char_Count := 0;
