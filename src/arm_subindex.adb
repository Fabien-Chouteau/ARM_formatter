...
package body ARM_Subindex is
   ...
   procedure Insert
     (Subindex_Object : in out Subindex_Type; Entity : in String;
      From_Unit       : in     String                  := "";
      Kind : in Subindex_Item_Kind_Type := Top_Level; Clause : in String := "";
      Paragraph       : in     String := ""; Key : in ARM_Index.Index_Key)
   is
   ...
   begin
      ...
      Ada.Strings.Fixed.Move
        (Target => Temp_Item.Clause, Source => Clause,
         Drop   => Ada.Strings.Error, Pad => ' ');
      Temp_Item.Clause_Len := Clause'Length;
      Ada.Strings.Fixed.Move
        (Target => Temp_Item.Paragraph, Source => Paragraph,
         Drop   => Ada.Strings.Error, Pad => ' ');
      ...
      if From_Unit'Length /= 0 then
         ...
         if Kind = In_Unit or else Kind = Child_of_Parent
           or else Kind = Subtype_In_Unit or else Kind = Description_In_Unit
           or else Kind = Raised_Belonging_to_Unit
         then
   ...
   is
      ...
      begin
         ...
         case Compare (To_Lower (Left.Entity.all), To_Lower (Right.Entity.all))
         is
            when Less =>
               return True;
            when Greater =>
         ...
         if Left.From_Unit = null then
            ...
            case Compare
              (To_Lower (Left.From_Unit.all), To_Lower (Right.From_Unit.all))
            is
               when Less =>
                  return True;
               when Greater =>
      ...
      procedure New_Kind
        (Item               : Item_List; Reset_Keep : in Boolean;
         Last_Had_Same_Unit : Boolean := False)
      is
...
--Ada.Text_IO.Put_Line("  Different kind (False)");
            return False;
         elsif Item.Kind /= Top_Level
           and then Item.From_Unit.all /= Item.Next.From_Unit.all
         then
...
--Ada.Text_IO.Put_Line("  Recurse");
   ...
   begin
      ...
      while Temp /= null loop
         ...
         if Last = null
           or else To_Lower (Last.Entity.all) /= To_Lower (Temp.Entity.all)
         then
...
--" Keep_Set=" & Boolean'Image(Keep_Set));
               elsif Minimize_Lines and then Temp.Kind = In_Unit
                 and then Temp.Entity'Length + 4 + Temp.From_Unit'Length <
                   CHARS_ON_SINGLE_LINE
               then
...
--" From_Unit=" & Temp.From_Unit.all & " Keep_Set=" & Boolean'Image(Keep_Set));
            ...
            if Temp.Kind /= Subtype_In_Unit then
               ...
               declare
                  Of_Loc : Natural :=
                    Ada.Strings.Fixed.Index (Temp.Entity.all, " subtype of ");
               begin
                  if Of_Loc = 0 then
...
--" Kind=" & Subindex_Item_Kind_Type'Image(Temp.Kind) &
--" From_Unit=" & Temp.From_Unit.all & " Keep_Set=" & Boolean'Image(Keep_Set));
                  if Minimize_Lines and then Temp.Kind = In_Unit
                    and then Temp.Entity'Length + 4 + Temp.From_Unit'Length <
                      CHARS_ON_SINGLE_LINE
                  then
                     ...
                     New_Kind
                       (Temp, Reset_Keep => True, Last_Had_Same_Unit => False);
                     Keep_Set := False;
                  end if;
               else -- Leave keep set:
...
--" From_Unit=" & Temp.From_Unit.all & " Keep_Set=" & Boolean'Image(Keep_Set));
                  New_Kind
                    (Temp, Reset_Keep => False, Last_Had_Same_Unit => False);
               end if;
            end if;
         elsif Last.Kind /= Temp.Kind then
            if Last.Kind /= Top_Level and then Temp.Kind /= Top_Level
              and then Last.From_Unit.all = Temp.From_Unit.all
            then
--Ada.Text_IO.Put_Line("New Kind, same unit: Entity=" & Temp.Entity.all &
--" Kind=" & Subindex_Item_Kind_Type'Image(Temp.Kind) &
--" From_Unit=" & Temp.From_Unit.all & " Keep_Set=" & Boolean'Image(Keep_Set));
               New_Kind
                 (Temp, Reset_Keep => Keep_Set, Last_Had_Same_Unit => True);
            else
--if Temp.Kind /= Top_Level then
...
--end if;
               New_Kind
                 (Temp, Reset_Keep => Keep_Set, Last_Had_Same_Unit => False);
            end if;
            Keep_Set := False;
         elsif
           (Temp.Kind = In_Unit or else Temp.Kind = Child_of_Parent
            or else Temp.Kind = Subtype_In_Unit
            or else Temp.Kind = Description_In_Unit
            or else Temp.Kind = Raised_Belonging_to_Unit)
           and then Last.From_Unit.all /= Temp.From_Unit.all
         then
