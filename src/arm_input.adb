...
package body ARM_Input is
         ...
         if Ada.Characters.Handling.Is_Alphanumeric (Ch) then
            ...
            if Len = 0 and then (not Null_Name_Allowed) then
               Ada.Text_IO.Put_Line
                 ("  ** Failed to find command name on line " &
   ...
   procedure Copy_to_String_until_Close_Char
     (Input_Object : in out Input_Type'Class; Close_Char : in Character;
      Buffer       :    out String; Len : out Natural)
   is
   ...
   begin
      ...
      loop
         if Ch = Start_Ch then
            ...
            Ada.Text_IO.Put_Line
              ("  ** End of file when recording string on line " &
               ARM_Input.Line_String (Input_Object));
            exit;
         end if;
         if Len >= Buffer'Length then
            Ada.Text_IO.Put_Line
              ("  ** String buffer overflow on line " &
               ARM_Input.Line_String (Input_Object));
----Debug:
...
--   ARM_Input.Line_String (Input_Object));
            ...
            Ada.Text_IO.Put_Line
              ("  ** End of file when skipping to end, started on line " &
               Start_Line);
            exit;
         elsif Ch = ASCII.LF and then Exit_On_Para_End
         then -- Possible end of paragraph.
...
--   ARM_Input.Line_String (Input_Object));
   ...
   procedure Check_Parameter_Name
     (Input_Object        : in out Input_Type'Class;
      Param_Name : in     ARM_Input.Command_Name_Type; Is_First : in Boolean;
   ...
   begin
      ...
      ARM_Input.Get_Name
        (Input_Object, Our_Param_Name, Null_Name_Allowed => True);
      if Ada.Characters.Handling.To_Lower (Param_Name) =
        Ada.Characters.Handling.To_Lower (Our_Param_Name)
      then
         ARM_Input.Get_Char (Input_Object, Ch);
         if Ch /= '=' then
            Ada.Text_IO.Put_Line
              ("  ** Bad parameter character for " &
               Ada.Strings.Fixed.Trim (Our_Param_Name, Ada.Strings.Right) &
               " parameter: " & Ch & " on line " &
               ARM_Input.Line_String (Input_Object));
         end if;
         ...
         Ada.Text_IO.Put_Line
           ("  ** Bad parameter name: " &
            Ada.Strings.Fixed.Trim (Our_Param_Name, Ada.Strings.Right) & ", " &
            Ada.Strings.Fixed.Trim (Param_Name, Ada.Strings.Right) &
            " expected on line " & ARM_Input.Line_String (Input_Object));
      end if;
      ...
      if ARM_Input.Is_Open_Char (Ch) then -- Start parameter:
         ...
         ARM_Input.Replace_Char (Input_Object);
         Ada.Text_IO.Put_Line
           ("  ** Failed to find parameter text for " &
            Ada.Strings.Fixed.Trim (Our_Param_Name, Ada.Strings.Right) &
   ...
   procedure Check_One_of_Parameter_Names
     (Input_Object : in out Input_Type'Class;
      ...
      Is_First     : in     Boolean; Param_Close_Bracket : out Character;
   ...
   begin
      if not Is_First then
         ...
         if Ch /= ',' then
            Ada.Text_IO.Put_Line
              ("  ** Bad parameter separator for " &
               Ada.Strings.Fixed.Trim (Param_Name_1, Ada.Strings.Right) &
               " or " &
      ...
      ARM_Input.Get_Name
        (Input_Object, Our_Param_Name, Null_Name_Allowed => True);
      if Param_Name_1 /= ARM_Input.Command_Name_Type'(others => ' ')
        and then Ada.Characters.Handling.To_Lower (Param_Name_1) =
          Ada.Characters.Handling.To_Lower (Our_Param_Name)
      then
         ARM_Input.Get_Char (Input_Object, Ch);
         if Ch /= '=' then
         ...
         Param_Found := 1;
      elsif Param_Name_2 /= ARM_Input.Command_Name_Type'(others => ' ')
        and then Ada.Characters.Handling.To_Lower (Param_Name_2) =
          Ada.Characters.Handling.To_Lower (Our_Param_Name)
      then
         ARM_Input.Get_Char (Input_Object, Ch);
         if Ch /= '=' then
        ...
        and then Ada.Characters.Handling.To_Lower (Param_Name_3) =
          Ada.Characters.Handling.To_Lower (Our_Param_Name)
      then
         ARM_Input.Get_Char (Input_Object, Ch);
         if Ch /= '=' then
        ...
        and then Ada.Characters.Handling.To_Lower (Param_Name_4) =
          Ada.Characters.Handling.To_Lower (Our_Param_Name)
      then
         ARM_Input.Get_Char (Input_Object, Ch);
         if Ch /= '=' then
         ...
         Param_Found := 4;
      elsif Param_Name_5 /= ARM_Input.Command_Name_Type'(others => ' ')
        and then Ada.Characters.Handling.To_Lower (Param_Name_5) =
          Ada.Characters.Handling.To_Lower (Our_Param_Name)
      then
         ARM_Input.Get_Char (Input_Object, Ch);
         if Ch /= '=' then
         ...
         if Param_Name_5 /= ARM_Input.Command_Name_Type'(others => ' ') then
            Ada.Text_IO.Put_Line
              ("  ** Bad parameter name: " &
               ...
               ", " &
               Ada.Strings.Fixed.Trim (Param_Name_4, Ada.Strings.Right) &
               ", or " &
               Ada.Strings.Fixed.Trim (Param_Name_5, Ada.Strings.Right) &
               " expected on line " & ARM_Input.Line_String (Input_Object));
