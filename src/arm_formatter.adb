with Ada.Text_IO, Ada.Strings.Fixed, Ada.Strings.Unbounded, Ada.Characters
  .Handling, Ada.Command_Line;
with ARM_Master, ARM_Contents, ARM_Format;
...
-- Copyright 2000, 2002, 2004, 2005, 2006, 2011, 2012, 2016, 2017, 2019, 2021
   ...
   Master_File : Ada.Strings.Unbounded
     .Unbounded_String; -- Master file for document to generate.
   Change_Kind         : ARM_Format.Change_Kind; -- Changes to generate.
   Base_Change_Version : ARM_Contents
     .Change_Version_Type; -- Lower Change version.
   Change_Version : ARM_Contents
     .Change_Version_Type; -- (Upper) Change version.
   ...
   begin
         ...
         Output_Path :=
           Ada.Strings.Unbounded.To_Unbounded_String
             (Ada.Strings.Fixed.Trim
                (Ada.Command_Line.Argument (6), Ada.Strings.Right));
         ...
         declare
            Last : constant Character :=
              Ada.Strings.Unbounded.Element
                (Ada.Strings.Unbounded.Tail (Output_Path, 1), 1);
         begin
            if Last = '/' or else Last = '\' then
               null; -- OK; this ends with a path separator.
            else
               Ada.Text_IO.Put_Line
                 ("** Output path does not end with a path separator: " &
         ...
         declare
            Version_Arg : String :=
              Ada.Characters.Handling.To_Lower
                (Ada.Strings.Fixed.Trim
                   (Ada.Command_Line.Argument (5), Ada.Strings.Right));
         begin
            if Version_Arg'Length = 1
              and then Version_Arg (Version_Arg'First) in
                ARM_Contents.Change_Version_Type
            then
               Change_Version := Version_Arg (Version_Arg'First);
            else
               Ada.Text_IO.Put_Line
                 ("** Unrecognized change version: " & Version_Arg);
               raise No_Command_Error;
            end if;
         end;
      else
      ...
      if Ada.Command_Line.Argument_Count >= 4 then
         declare
            Version_Arg : String :=
              Ada.Characters.Handling.To_Lower
                (Ada.Strings.Fixed.Trim
                   (Ada.Command_Line.Argument (4), Ada.Strings.Right));
         begin
            if Version_Arg'Length = 1
              and then Version_Arg (Version_Arg'First) in
                ARM_Contents.Change_Version_Type
      ...
      if Ada.Command_Line.Argument_Count >= 3 then
         declare
            Changes_Arg : String :=
              Ada.Characters.Handling.To_Lower
                (Ada.Strings.Fixed.Trim
                   (Ada.Command_Line.Argument (3), Ada.Strings.Right));
         begin
            if Changes_Arg = "no-changes" then
               ...
               Ada.Text_IO.Put_Line
                 ("** Unrecognized changes: " & Changes_Arg);
               raise No_Command_Error;
            end if;
         end;
      else
      ...
      if Ada.Command_Line.Argument_Count >= 2 then
         declare
            Format_Arg : String :=
              Ada.Characters.Handling.To_Lower
                (Ada.Strings.Fixed.Trim
                   (Ada.Command_Line.Argument (2), Ada.Strings.Right));
         begin
      ...
      begin
         Master_File :=
           Ada.Strings.Unbounded.To_Unbounded_String
             (Ada.Strings.Fixed.Trim
                (Ada.Command_Line.Argument (1), Ada.Strings.Right));
   ...
   exception
      when No_Command_Error =>
         Ada.Text_IO.Put_Line
           ("  Usage: Arm_Form <Master_File> [<Format>[ <Changes>[ <BaseVers>[ <ChgVers>[ <Output Path>]]]]]}");
         Ada.Text_IO.Put_Line
           ("     where: <Master_File> is the file name (and optional path) for the master file");
         ...
         Ada.Text_IO.Put_Line
           ("                       'RTF' (RTF files for Word 97 or later),");
         Ada.Text_IO.Put_Line
           ("                       'Corr' (Corrigendum-style command files for comparisons);");
         Ada.Text_IO.Put_Line
           ("     where: <Changes> = 'No-Changes' (Original RM text),");
         Ada.Text_IO.Put_Line
           ("                        'New-Only' (Revised RM text only up to <ChgVers>),");
         Ada.Text_IO.Put_Line
           ("                        'Show-Changes' (Text with changes marked between");
         Ada.Text_IO.Put_Line
           ("                                        <BaseVers> and <ChgVers>),");
         Ada.Text_IO.Put_Line
           ("                        'New-Changes' (Text with insertions marked between");
         Ada.Text_IO.Put_Line
           ("                                        <BaseVers> and <ChgVers>);");
         Ada.Text_IO.Put_Line
           ("     where: <BaseVers> and <ChgVers> = a value in 0 .. 9 representing the");
         Ada.Text_IO.Put_Line
           ("                        base and primary change versions, respectively");
         Ada.Text_IO.Put_Line
         ...
         Ada.Text_IO.Put_Line
           ("                        4-Ada 2012 Technical Corrigendum 1");
         Ada.Text_IO.Put_Line ("                        5-Ada 202x");
         Ada.Text_IO.Put_Line
           ("     where: <Output_Path> = it the path where to write the result files. This must");
         Ada.Text_IO.Put_Line
           ("                        end with a path separator. It defaults to ./output/");
         raise No_Command_Error;
   end Get_Commands;
...
begin
   ...
   Ada.Text_IO.Put_Line
     ("  Master File = " & Ada.Strings.Unbounded.To_String (Master_File));
   Ada.Text_IO.Put_Line
     ("  Format = " & ARM_Master.Output_Format_Type'Image (Format));
   Ada.Text_IO.Put_Line
     ("  Changes = " & ARM_Format.Change_Kind'Image (Change_Kind));
   ...
   ARM_Master.Read_and_Process_Master_File
     (File_Name               => Ada.Strings.Unbounded.To_String (Master_File),
      The_Change_Kind => Change_Kind, The_Change_Version => Change_Version,
      The_Base_Change_Version => Base_Change_Version, Output_Format => Format,
