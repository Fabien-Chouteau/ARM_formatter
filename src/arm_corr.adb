with ARM_Output, ARM_Contents, Ada.Text_IO, Ada.Exceptions, Ada.Strings
  .Fixed, Ada.Strings.Maps;
package body ARM_Corr is
   ...
   procedure Create
     (Output_Object : in out Corr_Output_Type; File_Prefix : in String;
   ...
   procedure Section
     (Output_Object : in out Corr_Output_Type; Section_Title : in String;
...
--     Output_Object.Output_Buffer (1 .. Output_Object.Output_Buffer_Len));
   ...
   procedure Buffer
     (Output_Object : in out Corr_Output_Type; Char : in Character)
   is
   -- Internal:
...
--Ada.Text_IO.Put_Line("** Buffer overflow!!");
   ...
   procedure Buffer (Output_Object : in out Corr_Output_Type; Str : in String)
   is
   -- Internal:
   ...
   begin
      if Output_Object.Output_Buffer_Len + Str'Length >=
        Output_Object.Output_Buffer'Last
      then
...
--Ada.Text_IO.Put_Line("** Buffer overflow!!");
      ...
      Output_Object.Output_Buffer
        (Output_Object.Output_Buffer_Len + 1 ..
             Output_Object.Output_Buffer_Len + Str'Length) :=
        Str;
   ...
   begin
      ...
      if Output_Object.Clause_Len /= 0 and then Number /= "" then
         Ada.Text_IO.New_Line (Output_Object.Output_File);
         ...
         when ARM_Output.Wide_Hanging =>
            if Indent = 3 then
               ...
               if No_Prefix then
                  ...
                  Output_Object.Saw_Hang_End := False;
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "@xhang<@xterm<");
...
--    Natural'Image(Output_Object.Char_Count));
   ...
   procedure Category_Header
     (Output_Object : in out Corr_Output_Type; Header_Text : String)
   is
   ...
   procedure Clause_Header
     (Output_Object : in out Corr_Output_Type; Header_Text : in String;
   ...
   begin
      ...
      if Clause_Number = ""
        and then
        (Header_Text = "Table of Contents" or else Header_Text = "Contents")
      then
         Ada.Text_IO.Put (Output_Object.Output_File, "!clause ");
      ...
      case Level is
         ...
         when ARM_Contents.Normative_Annex =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               Clause_Number); -- Note: Clause_Number includes "Annex"
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "(normative)");
            Ada.Text_IO.New_Line (Output_Object.Output_File);
            Ada.Text_IO.Put_Line (Output_Object.Output_File, Header_Text);
         when ARM_Contents.Informative_Annex =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               Clause_Number); -- Note: Clause_Number includes "Annex"
            Ada.Text_IO.Put_Line (Output_Object.Output_File, "(informative)");
            Ada.Text_IO.New_Line (Output_Object.Output_File);
            Ada.Text_IO.Put_Line (Output_Object.Output_File, Header_Text);
   ...
   procedure Revised_Clause_Header
     (Output_Object : in out Corr_Output_Type; New_Header_Text : in String;
   ...
   procedure TOC_Marker
     (Output_Object : in out Corr_Output_Type; For_Start : in Boolean)
   is
   ...
   procedure Separator_Line
     (Output_Object : in out Corr_Output_Type; Is_Thin : Boolean := True)
   is
   ...
   procedure Ordinary_Text
     (Output_Object : in out Corr_Output_Type; Text : in String)
   is
...
--Ada.Text_IO.Put_Line ("Tab");
   ...
   begin
      ...
      case Char is
         ...
         when ARM_Output.GEQ =>
            Ordinary_Text
              (Output_Object,
               ">="); -- Not available in Corrigendum, use the Ada one.
         when ARM_Output.LEQ =>
            Ordinary_Text
              (Output_Object,
               "<="); -- Not available in Corrigendum, use the Ada one.
         ...
         when ARM_Output.Left_Ceiling =>
            Ordinary_Text
              (Output_Object, "Ceiling("); -- Not available in Corrigendum.
         when ARM_Output.Right_Ceiling =>
            Ordinary_Text
              (Output_Object, ")"); -- Not available in Corrigendum.
         when ARM_Output.Left_Floor =>
            Ordinary_Text
              (Output_Object, "Floor("); -- Not available in Corrigendum.
         ...
         when ARM_Output.Left_Double_Quote =>
            Ordinary_Text
              (Output_Object,
               """"); -- Not available in Corrigendum, use double quote.
         ...
         when ARM_Output.Small_Dotless_I =>
            Ordinary_Text
              (Output_Object,
               "i"); -- Not available in Corrigendum, use the nearest text.
   ...
   begin
      ...
      if Format.Size /= Output_Object.Size then
         if Format.Size < 0 then
            Buffer
              (Output_Object,
               "@s" & Character'Val (10 + Format.Size + Character'Pos ('0')) &
               '<');
         else
            Buffer
              (Output_Object,
               "@s1" & Character'Val (Format.Size + Character'Pos ('0')) &
               '<');
         end if;
   ...
   procedure Index_Target
     (Output_Object : in out Corr_Output_Type; Index_Key : in Natural)
   is
   ...
   procedure Local_Link_Start
     (Output_Object : in out Corr_Output_Type; Target : in String;
      Clause_Number : in     String)
   is
   -- Generate a local link to the target and clause given.
   ...
   procedure Picture
     (Output_Object : in out Corr_Output_Type; Name : in String;
   ...
   begin
      Ordinary_Text (Output_Object, "[Picture: " & Name & " - " & Descr & "]");
   end Picture;
