...
package body ARM_Contents is
   ...
   begin
      if Level /= Subsubclause and then Clause_Number.Subsubclause /= 0 then
         raise Bad_Clause_Error
           with "not a subsubclause but non-zero subsubclause number";
      end if;
      if Level /= Subsubclause and then Level /= Subclause
        and then Clause_Number.Subclause /= 0
      then
         raise Bad_Clause_Error
           with "not a subclause but non-zero subclause number";
      end if;
      if
        (Level /= Subsubclause and then Level /= Subclause
         and then Level /= Clause and then Level /= Unnumbered_Section
         and then Level /= Dead_Clause)
        and then Clause_Number.Clause /= 0
      then
      ...
      Title_List (Last_Title) :=
        (Title        => Title,
         ...
         Level => Level, Clause_Number => Clause_Number, Version => Version);
...
-- " Subsubclause" & Natural'Image(Clause_Number.Subsubclause));
   ...
   procedure Add_Old
     (Old_Title     : in Title_Type; Level : in Level_Type;
      Clause_Number : in Clause_Number_Type;
      Version       : in ARM_Contents.Change_Version_Type := '0')
   is
...
-- " Subsubclause" & Natural'Image(Clause_Number.Subsubclause));
   ...
   function Make_Clause_Number
     (Level : in Level_Type; Clause_Number : in Clause_Number_Type)
      return String
   is
   ...
   begin
      ...
      case Level is
         when Plain_Annex | Normative_Annex | Informative_Annex =>
            if Clause_Number.Clause /= 0 or else Clause_Number.Subclause /= 0
              or else Clause_Number.Subsubclause /= 0
              or else Clause_Number.Section <= 30
            then
            ...
            return
              "Annex " &
              Character'Val
                (Character'Pos ('A') + (Clause_Number.Section - ANNEX_START));
         when Section =>
            if Clause_Number.Clause /= 0 or else Clause_Number.Subclause /= 0
              or else Clause_Number.Section >= ANNEX_START
            then
               raise Bad_Clause_Error; -- Illegal numbers.
            end if;
            if Clause_Number.Section < 10 then
               return
                 Character'Val (Character'Pos ('0') + Clause_Number.Section) &
                 "";
            elsif Clause_Number.Section < 20 then
               return
                 "1" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Section - 10);
            elsif Clause_Number.Section < 30 then
               return
                 "2" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Section - 20);
            else
               return
                 "3" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Section - 30);
            end if;
         when Unnumbered_Section =>
            if Clause_Number.Clause = 0 or else Clause_Number.Subclause /= 0
              or else Clause_Number.Section /= 0
            then
               raise Bad_Clause_Error; -- Illegal numbers.
            end if;
            if Clause_Number.Clause < 10 then
               ...
               return
                 "0.1" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Clause - 10);
            elsif Clause_Number.Clause < 30 then
               return
                 "0.2" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Clause - 20);
            else
               return
                 "0.3" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Clause - 30);
            end if;
         when Clause =>
            ...
            if Clause_Number.Section < 10 then
               if Clause_Number.Clause < 10 then
                  return
                    Character'Val
                      (Character'Pos ('0') + Clause_Number.Section) &
                    "." &
                    Character'Val (Character'Pos ('0') + Clause_Number.Clause);
               elsif Clause_Number.Clause < 20 then
                  return
                  ...
                  return
                    ...
                    ".4" &
                    Character'Val
                      (Character'Pos ('0') + Clause_Number.Clause - 40);
                  ...
                  return
                    ...
                    ".5" &
                    Character'Val
                      (Character'Pos ('0') + Clause_Number.Clause - 50);
               else
         ...
         when Subclause =>
            if Clause_Number.Section = UNKNOWN then
               ...
               return
                 ...
                 ".1" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subclause - 10);
               ...
               return
                 ...
                 ".2" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subclause - 20);
               ...
               return
                 ...
                 ".3" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subclause - 30);
               ...
               return
                 ...
                 ".4" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subclause - 40);
            else
               raise Bad_Clause_Error; -- Out of range.
            end if;
         when Subsubclause =>
            if Clause_Number.Subsubclause < 10 then
               return
                 Make_Clause_Number
                   (Subclause,
                    (Clause_Number.Section, Clause_Number.Clause,
                     Clause_Number.Subclause, 0)) &
                 "." &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subsubclause);
            elsif Clause_Number.Subclause < 20 then
               return
                 Make_Clause_Number
                   (Subclause,
                 ...
                 ".1" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subsubclause - 10);
            elsif Clause_Number.Subclause < 30 then
               return
                 Make_Clause_Number
                   (Subclause,
                 ...
                 ".2" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subsubclause - 20);
            elsif Clause_Number.Subclause < 40 then
               return
                 Make_Clause_Number
                   (Subclause,
                 ...
                 ".3" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subsubclause - 30);
            elsif Clause_Number.Subclause < 50 then
               return
                 Make_Clause_Number
                   (Subclause,
                 ...
                 ".4" &
                 Character'Val
                   (Character'Pos ('0') + Clause_Number.Subsubclause - 40);
            else
               raise Bad_Clause_Error; -- Out of range.
            end if;
         when Dead_Clause =>
   ...
   procedure Make_Clause
     (Clause_String : in String; Clause_Number : out Clause_Number_Type)
   is
      ...
      begin
         if Clause_String'Length = 1
           or else Clause_String (Clause_String'First + 1) = '.'
         then
      ...
      -- Extract the clause:
      begin
         if Clause_String'Last - Next + 1 = 1
           or else Clause_String (Next + 1) = '.'
         then
            Next := Next + 2;
            return
              Character'Pos (Clause_String (Next - 2)) - Character'Pos ('0');
         else
            Next := Next + 3;
            return
              (Character'Pos (Clause_String (Next - 3)) -
               Character'Pos ('0')) *
              10 +
         ...
         if Clause_String'Last - Next + 1 = 1 then
            ...
            if Clause_String'Last - Next + 1 = 1 then
               ...
               then
                  Next                    := Next + 2;
                  Clause_Number.Subclause :=
                    Character'Pos (Clause_String (Next - 2)) -
                    Character'Pos ('0');
               else
                  Next                    := Next + 3;
                  Clause_Number.Subclause :=
   ...
   function Lookup_Clause_Number (Title : in Title_Type) return String is
      ...
      Lower_Title : constant Title_Type :=
   ...
   begin
      for I in 1 .. Last_Title loop
         if Lower_Title = Title_List (I).Search_Title then
            return
              Make_Clause_Number
                (Title_List (I).Level, Title_List (I).Clause_Number);
         end if;
      end loop;
   ...
   begin
      ...
      for I in 1 .. Last_Title loop
         if Title_List (I).Level = Level
           and then Title_List (I).Clause_Number = Clause_Number
         then
   ...
   begin
      ...
      for I in 1 .. Last_Old_Title loop
         if Old_Title_List (I).Level = Level
           and then Old_Title_List (I).Clause_Number = Clause_Number
         then
         ...
         if Title_List (I).Clause_Number = Clause_Number then
            for J in reverse 1 .. I - 1 loop
               if Title_List (J).Level /= Dead_Clause then
                  return
                    Make_Clause_Number
                      (Title_List (J).Level, Title_List (J).Clause_Number);
   ...
   begin
      ...
      if Clause_Number.Clause = 0 then
         ...
         if Clause_Number.Section >= ANNEX_START then
            return
              Make_Clause_Number
                (Normative_Annex, (Clause_Number.Section, 0, 0, 0));
         else
            return
              Make_Clause_Number (Section, (Clause_Number.Section, 0, 0, 0));
         end if;
   ...
   begin
      for I in 1 .. Last_Title loop
         if Title_List (I).Level /= Dead_Clause then
            Operate
              (Title_List (I).Title, Title_List (I).Level,
               Title_List (I).Clause_Number, Title_List (I).Version, Quit);
            -- else skip it.
         end if;
         if Quit then
            return;
