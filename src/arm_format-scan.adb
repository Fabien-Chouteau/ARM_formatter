...
separate (ARM_Format)
procedure Scan
  (Format_Object      : in out Format_Type; File_Name : in String;
...
is
   ...
   procedure Set_Nesting_for_Command
     (Command : in Command_Type; Param_Ch : in Character)
   is
...
--  Nesting_Stack(Nesting_Stack_Ptr).Close_Char);
         Ada.Strings.Fixed.Move
           (Target => Nesting_Stack (Nesting_Stack_Ptr).Open_Line,
            Source => ARM_File.Line_String (Input_Object),
            Drop   => Ada.Strings.Right);
      else
         Ada.Text_IO.Put_Line
           ("** Nesting stack overflow on line" &
            ARM_File.Line_String (Input_Object));
         for I in reverse Nesting_Stack'Range loop
            Ada.Text_IO.Put_Line
              ("-- Command at" & Natural'Image (I) & " has a close char of '" &
   ...
   procedure Scan_Command_with_Parameter is
      ...
      procedure Get_Change_Version
        (Is_First : in Boolean; Version : out ARM_Contents.Change_Version_Type)
      is
      ...
      begin
         ARM_Input.Check_Parameter_Name
           (Input_Object,
            ...
            Is_First => Is_First, Param_Close_Bracket => Close_Ch);
         if Close_Ch /= ' ' then
   ...
   begin
      case Nesting_Stack (Nesting_Stack_Ptr).Command is
         when Labeled_Section | Labeled_Section_No_Break | Labeled_Annex |
           ...
           Labeled_Clause | Labeled_Subclause | Labeled_Subsubclause =>
            ...
            ARM_Input.Copy_to_String_until_Close_Char
              (Input_Object, Nesting_Stack (Nesting_Stack_Ptr).Close_Char,
               Title, Title_Length);
            Title (Title_Length + 1 .. Title'Last) := (others => ' ');
            ...
            then
               ...
               Ada.Text_IO.Put_Line
                 ("  ** Multiple section headers in a file, line " &
                  ARM_File.Line_String (Input_Object));
            else
            ...
            begin
               ...
               begin
                  ...
                  Ada.Text_IO.Put_Line
                    ("  ** Title """ & Title (1 .. Title_Length) &
                     """ is multiply defined on line " &
                     ARM_File.Line_String (Input_Object));
                  Ada.Text_IO.Put_Line
                    ("     Initial use is for clause " & Ref);
               end;
            exception
               when ARM_Contents.Not_Found_Error =>
                  ...
                  then
                     ARM_Contents.Add
                       (Title, ARM_Contents.Subclause,
                        Format_Object.Clause_Number);
                  elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                    Labeled_Subsubclause
                  then
                     ARM_Contents.Add
                       (Title, ARM_Contents.Subsubclause,
                        Format_Object.Clause_Number);
                  elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                    Labeled_Clause
                  then
                     ARM_Contents.Add
                       (Title, ARM_Contents.Clause,
                        Format_Object.Clause_Number);
                  elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                    Labeled_Section
                    or else Nesting_Stack (Nesting_Stack_Ptr).Command =
                      Labeled_Section_No_Break
                  then
                     ARM_Contents.Add
                       (Title, ARM_Contents.Section,
                        Format_Object.Clause_Number);
                  elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                    Labeled_Annex
                  then
                     ARM_Contents.Add
                       (Title, ARM_Contents.Plain_Annex,
                        Format_Object.Clause_Number);
                  elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                    Labeled_Normative_Annex
                  then
                     ARM_Contents.Add
                       (Title, ARM_Contents.Normative_Annex,
                        Format_Object.Clause_Number);
                  else
                     ARM_Contents.Add
                       (Title, ARM_Contents.Informative_Annex,
                        Format_Object.Clause_Number);
                  end if;
            end;
...
--Ada.Text_IO.Put_Line (" &Unstack (Header)");
         ...
         when Unnumbered_Section =>
            ...
            Format_Object.Clause_Number :=
              (Section   => 0, Clause => Format_Object.Unnumbered_Section,
               Subclause => 0, Subsubclause => 0);
            ...
            exception
               when ARM_Contents.Not_Found_Error =>
                  ...
                  -- Load the title into the contents package:
                  ARM_Contents.Add
                    (Title, ARM_Contents.Unnumbered_Section,
                     Format_Object.Clause_Number);
            end;
...
--Ada.Text_IO.Put_Line (" &Unstack (Header)");
         ...
         when Labeled_Revised_Annex | Labeled_Revised_Informative_Annex |
           Labeled_Revised_Normative_Annex | Labeled_Revised_Section |
           ...
           Labeled_Revised_Subsubclause =>
            ...
            begin
               Get_Change_Version (Is_First => True, Version => Version);
                  ...
                  loop
                     ARM_Input.Check_One_of_Parameter_Names
                       (Input_Object,
                        ...
                        Is_First => False, Param_Found => Which_Param,
                     ...
                     if Which_Param = 1 and then Close_Ch /= ' ' then
                        ...
                        if Ch /= Close_Ch then
                           Ada.Text_IO.Put_Line
                             ("  ** Bad close for InitialVersion parameter on line " &
                              ARM_File.Line_String (Input_Object));
                           ARM_File.Replace_Char (Input_Object);
                        end if;
               ...
               if Close_Ch /= ' ' then
                  ...
                  ARM_Input.Copy_to_String_until_Close_Char
                    (Input_Object, Close_Ch, Title, Title_Length);
                  Title (Title_Length + 1 .. Title'Last) := (others => ' ');
                  ARM_Input.Check_Parameter_Name
                    (Input_Object,
                     Param_Name =>
                       "Old" & (4 .. ARM_Input.Command_Name_Type'Last => ' '),
                     Is_First => False, Param_Close_Bracket => Close_Ch);
                  if Close_Ch /= ' ' then
                     -- There is a parameter:
                     -- Load the new title into the Title string:
                     ARM_Input.Copy_to_String_until_Close_Char
                       (Input_Object, Close_Ch, Old_Title, Old_Title_Length);
               ...
               if Close_Ch /= Nesting_Stack (Nesting_Stack_Ptr).Close_Char then
                  Ada.Text_IO.Put_Line
                    ("  ** Bad close for Labeled_Revised_(SubClause|Annex) on line " &
                     ARM_File.Line_String (Input_Object));
                  ARM_File.Replace_Char (Input_Object);
               end if;
               ...
               exception
                  when ARM_Contents.Not_Found_Error =>
                     ...
                     then
                        ARM_Contents.Add
                          (Title, ARM_Contents.Subclause,
                           Format_Object.Clause_Number, Version => Version);
                        ARM_Contents.Add_Old
                          (Old_Title, ARM_Contents.Subclause,
                           Format_Object.Clause_Number,
                     ...
                     then
                        ARM_Contents.Add
                          (Title, ARM_Contents.Subsubclause,
                           Format_Object.Clause_Number, Version => Version);
                        ARM_Contents.Add_Old
                          (Old_Title, ARM_Contents.Subsubclause,
                           Format_Object.Clause_Number,
                           Version => Initial_Version);
                     elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                       Labeled_Revised_Clause
                     ...
                     then
                        ARM_Contents.Add
                          (Title, ARM_Contents.Section,
                           Format_Object.Clause_Number, Version => Version);
                        ARM_Contents.Add_Old
                          (Old_Title, ARM_Contents.Section,
                           Format_Object.Clause_Number,
                           Version => Initial_Version);
                     elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                       Labeled_Revised_Annex
                     then
                        ARM_Contents.Add
                          (Title, ARM_Contents.Plain_Annex,
                           Format_Object.Clause_Number, Version => Version);
                        ARM_Contents.Add_Old
                          (Old_Title, ARM_Contents.Plain_Annex,
                           Format_Object.Clause_Number,
                           Version => Initial_Version);
                     elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                     ...
                     then
                        ARM_Contents.Add
                          (Title, ARM_Contents.Normative_Annex,
                           Format_Object.Clause_Number, Version => Version);
                        ARM_Contents.Add_Old
                          (Old_Title, ARM_Contents.Normative_Annex,
                           Format_Object.Clause_Number,
                           Version => Initial_Version);
                        ...
                        ARM_Contents.Add
                          (Title, ARM_Contents.Informative_Annex,
                           Format_Object.Clause_Number, Version => Version);
                        ARM_Contents.Add_Old
                          (Old_Title, ARM_Contents.Informative_Annex,
                           Format_Object.Clause_Number,
                           Version => Initial_Version);
                     end if;
               end;
...
--Ada.Text_IO.Put_Line (" &Unstack (Header)");
         ...
         when Labeled_Added_Annex | Labeled_Added_Informative_Annex |
           Labeled_Added_Normative_Annex | Labeled_Added_Section |
            ...
            begin
               ...
               ARM_Input.Check_Parameter_Name
                 (Input_Object,
                  ...
                  Is_First => False, Param_Close_Bracket => Ch);
               if Ch /= ' ' then
                  -- There is a parameter:
                  -- Load the new title into the Title string:
                  ARM_Input.Copy_to_String_until_Close_Char
                    (Input_Object, Ch, Title, Title_Length);
                  Title (Title_Length + 1 .. Title'Last) := (others => ' ');
               end if;
               ARM_File.Get_Char (Input_Object, Ch);
               if Ch /= Nesting_Stack (Nesting_Stack_Ptr).Close_Char then
                  Ada.Text_IO.Put_Line
                    ("  ** Bad close for Labeled_Added_(Sub)Clause on line " &
                     ARM_File.Line_String (Input_Object));
                  ARM_File.Replace_Char (Input_Object);
               end if;
               ...
               Calc_Change_Disposition
                 (Format_Object, Version => Version,
                  Operation => ARM_Output.Insertion, Text_Kind => How);
                        ...
                        then
                           ...
                           ARM_Contents.Add
                             (Title, ARM_Contents.Subsubclause,
                              Format_Object.Clause_Number, Version => Version);
                           ARM_Contents.Add_Old
                             ((others => ' '), ARM_Contents.Subsubclause,
                              Format_Object.Clause_Number);
                        elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                        ...
                        then
                           ...
                           ARM_Contents.Add
                             (Title, ARM_Contents.Plain_Annex,
                              Format_Object.Clause_Number, Version => Version);
                           ARM_Contents.Add_Old
                             ((others => ' '), ARM_Contents.Plain_Annex,
                              Format_Object.Clause_Number);
                        elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                        ...
                        then
                           ...
                           ARM_Contents.Add
                             (Title, ARM_Contents.Normative_Annex,
                              Format_Object.Clause_Number, Version => Version);
                           ARM_Contents.Add_Old
                             ((others => ' '), ARM_Contents.Normative_Annex,
                              Format_Object.Clause_Number);
                           ...
                           ARM_Contents.Add
                             (Title, ARM_Contents.Informative_Annex,
                              Format_Object.Clause_Number, Version => Version);
                           ARM_Contents.Add_Old
                             ((others => ' '), ARM_Contents.Informative_Annex,
                              Format_Object.Clause_Number);
                        end if;
                  end;
               end if;
...
--Ada.Text_IO.Put_Line (" &Unstack (Header)");
         ...
         when Labeled_Deleted_Clause | Labeled_Deleted_Subclause |
           Labeled_Deleted_Subsubclause =>
            ...
            begin
               ...
               if Ch /= Nesting_Stack (Nesting_Stack_Ptr).Close_Char then
                  Ada.Text_IO.Put_Line
                    ("  ** Bad close for Labeled_Deleted_(Sub)Clause on line " &
                     ARM_File.Line_String (Input_Object));
                  ARM_File.Replace_Char (Input_Object);
               end if;
               ...
               Calc_Change_Disposition
                 (Format_Object, Version => Version,
                  Operation => ARM_Output.Deletion, Text_Kind => How);
...
--Ada.Text_IO.Put_Line ("Labeled_Deleted disp: " & ARM_Output.Change_Type'Image(How));
               if How = ARM_Output.None then
                  ...
                  exception
                     when ARM_Contents.Not_Found_Error =>
                        ...
                        then
                           ...
                           ARM_Contents.Add
                             (Title, ARM_Contents.Subclause,
                              Format_Object.Clause_Number,
                              Version =>
                                '0'); -- Version here is an insertion version, and this was available from the beginning.
                           ARM_Contents.Add_Old
                             ((others => ' '), ARM_Contents.Subclause,
                              Format_Object.Clause_Number);
                        elsif Nesting_Stack (Nesting_Stack_Ptr).Command =
                        ...
                        then
                           ...
                           ARM_Contents.Add
                             (Title, ARM_Contents.Subsubclause,
                              Format_Object.Clause_Number, Version => '0');
                           ARM_Contents.Add_Old
                             ((others => ' '), ARM_Contents.Subsubclause,
                              Format_Object.Clause_Number);
                  ...
                  -- Load the title into the contents package as a dead clause:
                        ARM_Contents.Add
                          (Title, ARM_Contents.Dead_Clause,
                           (Section      => 0, Clause => 1, Subclause => 0,
                            Subsubclause => 0),
                           Version => '0');
                        ARM_Contents.Add_Old
                          ((others => ' '), ARM_Contents.Dead_Clause,
                           (Section      => 0, Clause => 1, Subclause => 0,
                            Subsubclause => 0));
                  end;
         ...
         when Syntax_Rule | Added_Syntax_Rule | Deleted_Syntax_Rule =>
            ...
            begin
               if Nesting_Stack (Nesting_Stack_Ptr).Command /= Syntax_Rule then
                  ...
                  Get_Change_Version (Is_First => True, Version => Ch);
               end if;
               ...
               if Ch = 'T' or else Ch = 't' then
                  ARM_Input.Check_Parameter_Name
                    (Input_Object,
                     Param_Name =>
                       "Tabs" & (5 .. ARM_Input.Command_Name_Type'Last => ' '),
               ...
               ARM_Input.Check_Parameter_Name
                 (Input_Object,
                  Param_Name =>
                    "LHS" & (4 .. ARM_Input.Command_Name_Type'Last => ' '),
                  Is_First            => (not Seen_First_Param),
                  Param_Close_Bracket => Close_Ch);
               if Close_Ch /= ' ' then
                  -- Copy over the non-terminal:
                  ARM_Input.Copy_to_String_until_Close_Char
                    (Input_Object, Close_Ch, Non_Terminal, NT_Len);
                  -- else no parameter. Weird.
               end if;
               ...
               ARM_Input.Check_Parameter_Name
                 (Input_Object,
                  Param_Name =>
                    "RHS" & (4 .. ARM_Input.Command_Name_Type'Last => ' '),
                  Is_First => False, Param_Close_Bracket => Close_Ch);
               Seen_First_Param := True;
               if Close_Ch /= ' ' then
                  -- Grab the tab string:
               ...
               begin
                  if Ada.Strings.Fixed.Index (The_Non_Terminal, "@") /= 0 then
                     ...
                     Ada.Text_IO.Put_Line
                       ("** Saw Non-Terminal with embedded commands: " &
...
--   " on line " & ARM_File.Line_String (Input_Object));
            ...
            ARM_File.Replace_Char
              (Input_Object); -- Put the close character back.
   ...
   begin
      ...
      if Ch = '\' then
         ...
         if ARM_Input.Is_Open_Char (Ch) then -- Start parameter:
            Set_Nesting_for_Command (Command => Unknown, Param_Ch => Ch);
         else -- No parameter. Weird.
            ARM_File.Replace_Char (Input_Object);
         end if;
         return;
...
--Ada.Text_IO.Put_Line("Command=" & Command_Name & " Nesting=" & Natural'Image(Nesting_Stack_Ptr));
      ...
      if ARM_Input.Is_Open_Char (Ch) then -- Start parameter:
         Set_Nesting_for_Command
           (Command =>
              Command (Ada.Characters.Handling.To_Lower (Command_Name)),
            Param_Ch => Ch);
...
--Ada.Text_IO.Put_Line("Char=" & Char & " Nesting=" & Natural'Image(Nesting_Stack_Ptr));
         case Char is
            ...
            when ASCII.SUB =>
               exit; -- End of file.
            when others =>
               if Nesting_Stack_Ptr /= 0
                 and then Nesting_Stack (Nesting_Stack_Ptr).Close_Char /= ' '
                 and then Nesting_Stack (Nesting_Stack_Ptr).Close_Char = Char
               then
   ...
   Ada.Text_IO.Put_Line
     ("  Lines scanned: " & ARM_File.Line_String (Input_Object));
   ...
   if Nesting_Stack_Ptr /= 0 then
      ...
      for I in reverse 1 .. Nesting_Stack_Ptr loop
         Ada.Text_IO.Put_Line
           ("      Open command=" & Nesting_Stack (I).Command'Image &
            "; Close_Char=" & Nesting_Stack (I).Close_Char & "; Line=" &
