with ARM_Output, ARM_Contents, Ada.Text_IO, Ada.Exceptions, Ada.Strings.Maps
  .Constants, Ada.Strings.Fixed, Ada.Unchecked_Deallocation;
...
--  9/10/04 - RLB - Added "Both" to possible changes to handle
   ...
   SMALL_SWISS_FONT_CODE : constant String :=
     "<font face=""Arial, Helvetica"" size=-1>";

   TINY_SWISS_FONT_CODE : constant String :=
   ...
   function Make_Clause_Anchor_Name
     (Output_Object : in HTML_Output_Type; Clause_Number : in String)
      return String
   is
      -- Internal routine.
   ...
   begin
      ...
      if Output_Object.DOS_Filenames and (not Output_Object.Big_Files) then
         ...
         if Clause_Name_Len = 1 then
            ...
            if Clause_Name_Len < 4 or else Clause_Name (3) /= '-'
              or else Clause_Name (4) not in '0' .. '9'
              or else
              (Clause_Name_Len >= 5 and then Clause_Name (5) /= '-'
               and then Clause_Name (5) not in '0' .. '9')
              or else
              (Clause_Name_Len >= 6 and then Clause_Name (5) /= '-'
               and then Clause_Name (6) /= '-')
            then
               ...
               Clause_Name (3) :=
                 Character'Val
                   (Character'Pos (Clause_Name (5)) - Character'Pos ('0') +
                    Character'Pos ('A'));
            ...
            elsif Clause_Name_Len < 5 or else Clause_Name (4) /= '-'
              or else Clause_Name (5) not in '0' .. '9'
              or else
              (Clause_Name_Len >= 6 and then Clause_Name (6) /= '-'
               and then Clause_Name (6) not in '0' .. '9')
              or else
              (Clause_Name_Len >= 7 and then Clause_Name (6) /= '-'
               and then Clause_Name (7) /= '-')
            then
               ...
               Clause_Name (4) :=
                 Character'Val
                   (Character'Pos (Clause_Name (6)) - Character'Pos ('0') +
                    Character'Pos ('A'));
            ...
            elsif Clause_Name_Len < 6 or else Clause_Name (5) /= '-'
              or else Clause_Name (6) not in '0' .. '9'
              or else
              (Clause_Name_Len >= 7 and then Clause_Name (7) not in '0' .. '9')
              or else (Clause_Name_Len >= 8)
            then
               ...
               Clause_Name (5) :=
                 Character'Val
                   (Character'Pos (Clause_Name (7)) - Character'Pos ('0') +
                    Character'Pos ('A'));
               Clause_Name_Len := 5;
            elsif Clause_Name (5) = '2' then
   ...
   function Make_Clause_Link_Name
     (Output_Object : in HTML_Output_Type; Clause_Number : in String)
      return String
   is
   -- Internal routine.
   ...
   begin
      if Output_Object.Big_Files then -- One big file.
         ...
         if Output_Object.DOS_Filenames then
            return
              Make_Clause_File_Name (Output_Object, Clause_Number) & ".HTM";
         else
            return
   ...
   procedure Put_EMs (Fyle : in Ada.Text_IO.File_Type; Value : in Natural) is
   ...
   begin
      if Value <= 9 then
         ...
         Ada.Text_IO.Put
           (Fyle, Character'Val (Character'Pos ('0') + (Value / 10)));
      else
         Ada.Text_IO.Put (Fyle, Natural'Image (Value / 10));
      end if;
      Ada.Text_IO.Put (Fyle, '.');
      Ada.Text_IO.Put
        (Fyle, Character'Val (Character'Pos ('0') + (Value mod 10)));
      Ada.Text_IO.Put (Fyle, "em");
   end Put_EMs;

   procedure Make_Navigation_Bar
     (Output_Object : in out HTML_Output_Type; Is_Top : in Boolean)
   is
      -- Internal routine.
      ...
      if Output_Object.Use_Buttons then
         if Is_Top and then Output_Object.HTML_Kind > HTML_3 then
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "<div style=""margin-top: 0.6em; margin-bottom: 0.0em"">");
         elsif (not Is_Top) and then Output_Object.HTML_Kind > HTML_3 then
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "<div style=""margin-top: 0.0em; margin-bottom: 0.6em"">");
         else
         ...
         if Output_Object.DOS_Filenames then
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               """><IMG SRC=""CONT.GIF"" ALT=""Contents"" BORDER=0></A>&nbsp;");
            ...
            begin
               ...
               if Output_Object.DOS_Filenames then
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
                     "&nbsp;<A HREF=""" &
                     Make_Clause_Link_Name
                       (Output_Object,
                        ARM_Contents.Lookup_Clause_Number
                          ("Index" &
                           (6 .. ARM_Contents.Title_Type'Last => ' '))) &
                     """><IMG SRC=""INDEX.GIF"" ALT=""Index"" BORDER=0></A>&nbsp;");
               else
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
            ...
            begin
               ...
               Ada.Text_IO.Put
                 (Output_Object.Output_File,
                  "&nbsp;<A HREF=""" &
                  Make_Clause_Link_Name
                    (Output_Object, ARM_Contents.Previous_Clause (Clause)));
               if Output_Object.DOS_Filenames then
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
            ...
            begin
               ...
               Ada.Text_IO.Put
                 (Output_Object.Output_File,
                  "&nbsp;<A HREF=""" &
                  Make_Clause_Link_Name
                    (Output_Object, ARM_Contents.Next_Clause (Clause)));
               if Output_Object.DOS_Filenames then
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
         ...
         if Ada.Strings.Unbounded.Length (Output_Object.Ref_URL) /= 0 then
            ...
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               Ada.Strings.Unbounded.To_String (Output_Object.Ref_URL));
            Ada.Text_IO.Put
              (Output_Object.Output_File, """>Reference Documents</A>");
         else -- Link to the section named "References".
            begin
               -- Note: We do the following in one big glup so that if
               -- Not_Found_Error is raised, nothing is output.
   ...
   type Special_Style_Kinds is
     (None, Hanging_Term, Hanging_Body, Bulleted_Item, Bulleted_No_Prefix);

   procedure Make_Style
     (Output_Object         : in out HTML_Output_Type; Name : in String;
      Style                 : in     ARM_Output.Paragraph_Style_Type;
      Indent                : in     ARM_Output.Paragraph_Indent_Type;
      Kind                  :        Special_Style_Kinds := None;
      Enumerated_Adjustment : in     Boolean             := False)
   is
      -- Internal routine.
         ...
         if Output_Object.HTML_Kind = HTML_4_Only then
            case Paragraph_Info (Style, Indent).Font is
               when ARM_Output.Default =>
                  Normal :=
                    ARM_Output."=" (Output_Object.Body_Font, ARM_Output.Roman);
               when ARM_Output.Roman =>
                  Normal := True;
               when ARM_Output.Fixed |
                 ARM_Output.Swiss => -- Start at 90% (otherwise they are huge!)
                  Normal := False;
            end case;
            if Normal then
               case Paragraph_Info (Style, Indent).Size is
                  when 0 =>
                     return Value * 20;
                  when 1 =>
                     return Value * 16; -- 20/1.25.
                  when 2 =>
                     return Value * 13; -- 20/1.56.
                  when 3 =>
                     return Value * 10; -- 20/1.93.
                  when -1 =>
                     return Value * 25; -- 20/0.80.
                  when -2 =>
                     return Value * 31; -- 20/0.64.
                  when -3 =>
                     return Value * 40; -- 20/0.50.
                  when others =>
               ...
               case Paragraph_Info (Style, Indent).Size is
                  when 0 =>
                     return Value * 22; -- 20/0.90
                  when 1 =>
                     return Value * 18; -- 20/1.13.
                  when 2 =>
                     return Value * 14; -- 20/1.40.
                  when 3 =>
                     return Value * 11; -- 20/1.75.
                  when -1 =>
                     return Value * 28; -- 20/0.72.
                  when -2 =>
                     return Value * 34; -- 20/0.58.
                  when -3 =>
                     return Value * 44; -- 20/0.45.
                  when others =>
                     return Value; -- Out of range.
               end case;
            end if;
         elsif ARM_Output."="
             (Paragraph_Info (Style, Indent).Font, ARM_Output.Fixed)
         then
   ...
   begin
      ...
      then
         ...
         if OPTIMIZE_FOR_FIREFOX then
            ...
            Ada.Text_IO.Put
              (Output_Object.Output_File,
               "position: absolute; top: auto; left: 0.6em; ");
            -- This does not work on Firefox: the text is too high by
            -- about half a line and thus doesn't line up properly.
         end if;
      elsif Kind = Hanging_Body and then Output_Object.HTML_Kind = HTML_4_Only
      then
         Ada.Text_IO.Put (Output_Object.Output_File, "    DIV.");
         Ada.Text_IO.Put (Output_Object.Output_File, Name & "-Body {");
      elsif (Kind = Bulleted_Item or else Kind = Bulleted_No_Prefix)
        and then Output_Object.HTML_Kind = HTML_4_Only
      then
         Ada.Text_IO.Put (Output_Object.Output_File, "    DIV.");
         ...
         begin
            ...
            if Normal then
               case Paragraph_Info (Style, Indent).Size is
                  ...
                  when 1 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 125%");
                  when 2 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 156%");
                  when 3 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 195%");
                  when -1 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 80%");
                  when -2 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 64%");
                  when -3 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 50%");
                  when others =>
                     null; -- Out of range.
               end case;
            else -- Start at 90% (otherwise they are huge!)
   -- Note: This size adjustment is for sections of text, not for in-line text.
               case Paragraph_Info (Style, Indent).Size is
                  when 0 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 90%");
                  when 1 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 113%");
                  when 2 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 140%");
                  when 3 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 175%");
                  when -1 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 72%");
                  when -2 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 58%");
                  when -3 =>
                     Ada.Text_IO.Put
                       (Output_Object.Output_File, "; font-size: 45%");
                  when others =>
                     null; -- Out of range.
               end case;
            end if;
         ...
         case Paragraph_Info (Style, Indent).Size is
            ...
            when 3 =>
               Ada.Text_IO.Put
                 (Output_Object.Output_File, "; font-size: 195%");
            when -1 =>
               Ada.Text_IO.Put (Output_Object.Output_File, "; font-size: 80%");
            when -2 =>
               Ada.Text_IO.Put (Output_Object.Output_File, "; font-size: 64%");
            when -3 =>
               Ada.Text_IO.Put (Output_Object.Output_File, "; font-size: 50%");
            when others =>
               null; -- Out of range.
         end case;
      ...
      -- Adjust the left margin to indent the prefix slightly (1/4 of a unit):
         declare
            ...
            Prefix_Indent : Natural := Units_to_EMs (1) / 4;
         begin
            Ada.Text_IO.Put (Output_Object.Output_File, "; margin-left: ");
            if Output_Object.Number_Paragraphs then
               Put_EMs
                 (Output_Object.Output_File,
                  Org_Margin + Prefix_Indent + INDENT_EMS_FOR_PARANUMS);
            else
      ...
      if Paragraph_Info (Style, Indent).Before /= 0 then
         ...
         Put_EMs
           (Output_Object.Output_File, Paragraph_Info (Style, Indent).Before);
      ...
      then
         ...
         Put_EMs
           (Output_Object.Output_File, Paragraph_Info (Style, Indent).After);
      end if;
   ...
   begin
      if Output_Object.HTML_Kind = HTML_4_Only then
         if ARM_Output."=" (Style, ARM_Output.Enumerated)
           or else ARM_Output."=" (Style, ARM_Output.Small_Enumerated)
         then
            Make_Style
              (Output_Object, Name, Style, Indent, Kind => Hanging_Term,
   ...
   function Paragraph_Name
     (Style  : in ARM_Output.Paragraph_Style_Type;
      Indent : in ARM_Output.Paragraph_Indent_Type) return String
   is
      -- Internal routine.
      ...
      case Style is
         when ARM_Output.Normal =>
            ...
            -- "CodeIndented"; Indent 3: "Indented"; Indent 4: "InnerIndented".
               return
                 "Indented" & Character'Val (Character'Pos ('0') + Indent);
            end if;
         ...
         when ARM_Output.Small =>
            if Indent = 0 then
               ...
               return
                 "SmallIndented" &
                 Character'Val (Character'Pos ('0') + Indent);
            end if;
         when ARM_Output.Small_Wide_Above =>
            if Indent = 0 then
               ...
               return
                 "SmallIndented" &
                 Character'Val (Character'Pos ('0') + Indent) & "WideAbove";
            end if;
         ...
         when ARM_Output.Small_Header =>
            if Indent = 0 then
               ...
               return
                 "SmallIndented" &
                 Character'Val (Character'Pos ('0') + Indent) & "Header";
            end if;
         ...
         when ARM_Output.Syntax_Summary =>
            if Indent = 1 then
               ...
               return
                 "SynSumInd" & Character'Val (Character'Pos ('0') + Indent);
            end if;
         ...
         when ARM_Output.Small_Examples =>
            if Indent = 3 then
               ...
               return
                 "SmallIndented" &
                 Character'Val (Character'Pos ('0') + Indent) & "Examples";
            end if;
         ...
         when ARM_Output.Small_Hanging_in_Bulleted =>
            if Indent = 5 then
               ...
               return
                 "Indented" & Character'Val (Character'Pos ('0') + Indent) &
                 "SmallHanging_in_Bulleted";
            end if;
      end case;
   end Paragraph_Name;

   procedure Make_Paragraph_Styles (Output_Object : in out HTML_Output_Type) is
   -- Internal routine.
   ...
   begin
      ...
      if Paranum_Used then
         if Output_Object.HTML_Kind = HTML_4_Compatible then
            if OPTIMIZE_FOR_FIREFOX then
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File,
                  "    DIV.paranum {float: left; font-family: Arial, Helvetica, sans-serif; width: 2.8em; " &
               ...
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File,
                  "    DIV.paranum {position: absolute; font-family: Arial, Helvetica, sans-serif; left: 0.5em; top: auto; margin-top: 0.2em}");
            end if;
            ...
            if OPTIMIZE_FOR_FIREFOX then
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File,
                  "    DIV.paranum {float: left; font-family: Arial, Helvetica, sans-serif; font-size: 64%; width: 2.8em; " &
                  "margin-left: -0.4em; margin-right: -3.0em; margin-top: 0.2em}");
               -- Uses floating elements; see above.
            else
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File,
                  "    DIV.paranum {position: absolute; font-family: Arial, Helvetica, sans-serif; font-size: 64%; " &
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "    DT {display: compact}"); -- CSS2. This doesn't seem to work on IE 4.01, but it is harmless.

      -- Revision styles:
      if Revision_Used ('0') then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.insert0 {text-decoration: underline; color: black}");
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.delete0 {text-decoration: line-through; color: black }");
      ...
      if Revision_Used ('1') then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.insert1 {text-decoration: underline; color: rgb(0,51,51) }"); -- (Very) dark turquoise.
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
      ...
      if Revision_Used ('2') then
         if Revision_Used ('5') or else Output_Object.Force_New_Revision_Colors
         then
            ...
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "    SPAN.insert2 {text-decoration: underline; color: rgb(0,102,0) }"); -- Dark green.
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
      ...
      if Revision_Used ('3') then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.insert3 {text-decoration: underline; color: rgb(102,51,0) }"); -- Dark brown.
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.delete3 {text-decoration: line-through; color: rgb(102,51,0) }");
      ...
      if Revision_Used ('4') then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.insert4 {text-decoration: underline; color: rgb(153,0,0) }"); -- Dark red.
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
      ...
      if Revision_Used ('5') then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.insert5 {text-decoration: underline; color: rgb(0,102,0) }"); -- Dark green.
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
      ...
      if Revision_Used ('6') then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.insert6 {text-decoration: underline; color: rgb(0,102,153) }"); -- Turquiose.
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "    SPAN.delete6 {text-decoration: line-through; color: rgb(0,102,153) }");
      ...
      for S in ARM_Output.Bullet_Prefixed_Style_Subtype loop
         ...
         for I in 1 .. ARM_Output.Paragraph_Indent_Type'Last loop
            if Output_Object.HTML_Kind = HTML_4_Only then
               Make_Style
                 (Output_Object, Paragraph_Name (S, I) & "-NoPrefix", S, I,
                  Kind => Bulleted_No_Prefix);
               Make_Style
                 (Output_Object, Paragraph_Name (S, I), S, I,
                  Kind => Bulleted_Item);
            else
               Make_Style (Output_Object, Paragraph_Name (S, I), S, I);
            end if;
   ...
   procedure Start_HTML_File
     (Output_Object : in out HTML_Output_Type; File_Name : in String;
      Title         : in     String; Clause : in String)
   is
   -- Internal routine.
...
--Ada.Text_IO.Put_Line ("--Creating " & File_Name & ".html");
      ...
      if Output_Object.HTML_Kind > HTML_3 then
         ...
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            """http://www.w3.org/TR/html4/loose.dtd"">"); -- HTML 4.01 (with depreciated features)
      else
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File, "    <TITLE>" & Title & "</TITLE>");
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
      ...
      if Output_Object.HTML_Kind = HTML_4_Only then
         -- The style sheet.
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "    <STYLE type=""text/css"">");
         -- Element styles:
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "    H4.centered {text-align: center}");
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
      ...
      Ada.Text_IO.Put_Line
        (Output_Object.Output_File,
         "<BODY TEXT=""" & Output_Object.Text_Color & """ BGCOLOR=""" &
         Output_Object.Background_Color & """ LINK=""" &
      ...
      if Output_Object.Nav_On_Top
        or else Ada.Strings.Unbounded.Length (Output_Object.Header_HTML) /= 0
      then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "<HR>"); -- Horizontal line (rule).
   ...
   begin
      Ada.Text_IO.New_Line
        (Output_Object.Output_File); -- Blank line to set off paragraphs.

      if Output_Object.Nav_On_Bottom
        or else Ada.Strings.Unbounded.Length (Output_Object.Footer_HTML) /= 0
      then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "<HR>"); -- Horizontal line (rule).
         -- else nothing on top at all.
      ...
      -- any ">" is the end of a start tag unless there is a "</" preceding it.
         ...
         begin
            Ada.Text_IO.Close (Output_Object.Output_File);
            Ada.Text_IO.Open
              (Reading_File, Ada.Text_IO.In_File, Original_Name);
            if Output_Object.DOS_Filenames then
               Ada.Text_IO.Create
                 (Output_Object.Output_File, Ada.Text_IO.Out_File,
                  Real_Name & "HTM");
            ...
            begin
               loop
                  ...
                  if Buffer (1 .. Len) = MAGIC_STYLE_MARKER then
                     ...
                     if Ada.Strings.Fixed.Index (Buffer (1 .. Len), "<BODY") /=
                       0
                     then
                        Body_Seen := True;
                     end if;
                     ...
                     loop
                        Loc :=
                          Ada.Strings.Fixed.Index (Buffer (1 .. Len), " </");
                        exit when Loc = 0;
   ...
   procedure Create
     (Output_Object : in out HTML_Output_Type; Big_Files : in Boolean;
      File_Prefix               : in     String; Output_Path : in String;
      DOS_Filenames             : in     Boolean; HTML_Kind : in HTML_Type;
      Use_Unicode               : in Boolean; Number_Paragraphs : in Boolean;
      Ref_URL : in     String; Srch_URL : in String; Index_URL : in String;
      Use_Buttons :    Boolean; Nav_On_Top : Boolean; Nav_On_Bottom : Boolean;
      Tab_Emulation             :    Tab_Emulation_Type; Script_HTML : String;
      Header_HTML :    String; Footer_HTML : String; Title : in String := "";
      Body_Font                 :        ARM_Output.Font_Family_Type;
      Force_New_Revision_Colors :        Boolean; Text_Color : Color_String;
      Background_Color          :    Color_String; Link_Color : Color_String;
      VLink_Color               :    Color_String; ALink_Color : Color_String)
   is
   ...
   begin
      ...
      if Output_Object.Big_Files then
         ...
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "<A NAME=""TTL""></A>");
      end if;
   end Create;
   ...
   procedure Section
     (Output_Object : in out HTML_Output_Type; Section_Title : in String;
   ...
   begin
      ...
      if Section_Name'Length > 3 then
         Output_Object.Section_Name :=
           Section_Name (Section_Name'First .. Section_Name'First + 2);
      else
   ...
   begin
      ...
      if Number_of_Columns >= 4 then
         ...
         if Output_Object.Current_Column = 1 then
            ...
            begin
               Col_Count :=
                 (Col_Count + Output_Object.Column_Count - 1) /
                 Output_Object.Column_Count;
               Ada.Text_IO.Put_Line
                 ("  @@ Calc columns for" &
                  Natural'Image (Output_Object.Column_Count) & " columns;" &
                  Natural'Image (Output_Object.Current_Item - 1) &
                  " total items;" & Natural'Image (Col_Count) &
                  " per column.");
...
--Natural'Image(Col_Count*(I-1)));
                  ...
                  if Where.Item <= Col_Count * (I - 1) then
                     ...
                     while Where.Next /= null
                       and then Where.Next.Item > Col_Count * (I - 1)
                     loop
...
--Ada.Text_IO.Put_Line("    split item=" & Natural'Image(Where.Item));
         ...
         if Output_Object.HTML_Kind = HTML_3 then
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "<UL><UL><TABLE Width=""70%"">"); -- Table with no border or caption, takes up 70% of the screen.
         else
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "<div class=""" & Paragraph_Name (ARM_Output.Normal, 2) &
               """><table width=""70%"">"); -- Table with no border or caption, takes up 70% of the screen.
            Paragraph_Used (ARM_Output.Normal, 2) := True;
         end if;
         -- And start the first row:
         Ada.Text_IO.Put
           (Output_Object.Output_File, "<TR><TD align=""left"">");
         for Item in 1 .. 5_000 loop
            for Col in 1 .. Output_Object.Column_Count loop
               ...
               begin
                  ...
                  if Find = null then
                     ...
                     while Find.Next /= null and then Find.Next.Item /= Item
                     loop
                        Find := Find.Next;
                     end loop;
                     Temp      := Find;
               ...
               Ada.Text_IO.Put
                 (Output_Object.Output_File, "<TD align=""left"">");
            end loop;
            if Output_Object.Column_Text =
              Column_Text_Ptrs_Type'(others => null)
            then
               -- We've output everything.
               Ada.Text_IO.New_Line (Output_Object.Output_File);
               if Output_Object.HTML_Kind = HTML_3 then
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File, "</TABLE></UL></UL>");
               else
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File, "</table></div>");
               end if;
               exit;
            end if;
            -- End the row:
   ...
   begin
      if not Ada.Text_IO.Is_Open (Output_Object.Output_File) then
         if (not Output_Object.Big_Files) and then Output_Object.DOS_Filenames
           and then Output_Object.Section_Name (1) = '-'
         then
            Start_HTML_File
              (Output_Object,
               Ada.Strings.Fixed.Trim
               ...
               Make_Clause_Anchor_Name
                 (Output_Object, Output_Object.Section_Name (2 .. 3)),
               "", Clause => "");
         else
            Start_HTML_File
              (Output_Object,
   ...
   procedure Put_Compatibility_Font_Info
     (Output_Object : in out HTML_Output_Type;
      Style         : in     ARM_Output.Paragraph_Style_Type;
      Indent        : in     ARM_Output.Paragraph_Indent_Type)
   is
      ...
      if Output_Object.HTML_Kind = HTML_4_Compatible then
            ...
            when ARM_Output.Default =>
               ...
               then
                  ...
                  Output_Object.Char_Count :=
                    Output_Object.Char_Count + SWISS_FONT_CODE'Length;
         ...
         then
            ...
            case Paragraph_Info (Style, Indent).Size is
               ...
               when 1 =>
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<font size=""+1"">");
                  Output_Object.Char_Count := Output_Object.Char_Count + 16;
               when 2 =>
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<font size=""+2"">");
                  Output_Object.Char_Count := Output_Object.Char_Count + 16;
               when 3 =>
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<font size=""+3"">");
                  Output_Object.Char_Count := Output_Object.Char_Count + 16;
               when -1 =>
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<font size=""-1"">");
   ...
   procedure Put_End_Compatibility_Font_Info
     (Output_Object : in out HTML_Output_Type;
      Style         : in     ARM_Output.Paragraph_Style_Type;
      Indent        : in     ARM_Output.Paragraph_Indent_Type)
   is
   ...
   is
      ...
      procedure Put_Style
        (Name    : in String; Include_Compatibility : in Boolean := True;
         Use_DIV : in Boolean := False)
      is
      ...
      begin
         ...
         case Space_After is
            ...
            when ARM_Output.Narrow =>
               Ada.Text_IO.Put
                 (Output_Object.Output_File, " style=""margin-bottom: ");
               Output_Object.Char_Count := Output_Object.Char_Count + 24;
               Put_EMs
                 (Output_Object.Output_File,
                  (Paragraph_Info (Style, Indent).After * LEADING_PERCENT) /
                  100);
               Ada.Text_IO.Put (Output_Object.Output_File, """");
            ...
            when ARM_Output.Wide =>
               ...
               Put_EMs
                 (Output_Object.Output_File,
                  (Paragraph_Info (Style, Indent).After * TRAILING_PERCENT) /
                  100);
               Ada.Text_IO.Put (Output_Object.Output_File, """");
               Output_Object.Char_Count := Output_Object.Char_Count + 6;
         end case;
         ...
         if Output_Object.HTML_Kind = HTML_4_Compatible
           and then Include_Compatibility
         then
   ...
   begin
      ...
      if not Output_Object.Number_Paragraphs and then Number /= "" then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity,
      ...
      if Output_Object.Column_Count >= 4 then
         ...
         if Tab_Stops.Number /= 0 then
            Ada.Exceptions.Raise_Exception
              (ARM_Output.Not_Valid_Error'Identity, "Tabs in 4+ column text");
         end if;
         case Style is
            when ARM_Output.Normal =>
               null;
         ...
         when ARM_Output.Normal | ARM_Output.Wide_Above | ARM_Output.Small |
           ...
           ARM_Output.Small_Swiss_Examples =>
            ...
            for I in 1 .. Tab_Stops.Number loop
               if ARM_Output."="
                   (Tab_Stops.Stops (I).Kind, ARM_Output.Left_Proportional)
               then
                  if ARM_Output."="
                      (Paragraph_Info (Style, Indent).Font, ARM_Output.Fixed)
                  then
      ...
      if Output_Object.HTML_Kind = HTML_3 then
         ...
         case Style is
              ...
              ARM_Output.Header =>
               if ARM_Output."=" (Indent, 0) then
                  case Justification is
                     ...
                     when ARM_Output.Center =>
                        Ada.Text_IO.Put
                          (Output_Object.Output_File, "<P ALIGN=CENTER>");
                        Output_Object.Char_Count := 16;
                     when ARM_Output.Right =>
                        Ada.Text_IO.Put
                          (Output_Object.Output_File, "<P ALIGN=RIGHT>");
            ...
            when ARM_Output.Small_Examples =>
               Ada.Text_IO.Put
                 (Output_Object.Output_File, "<TT><FONT SIZE=-1>");
            ...
            when ARM_Output.Small_Swiss_Examples =>
               Ada.Text_IO.Put
                 (Output_Object.Output_File, SMALL_SWISS_FONT_CODE);
               Output_Object.Char_Count :=
                 Output_Object.Char_Count + SMALL_SWISS_FONT_CODE'Length;
            ...
            when ARM_Output.Bulleted | ARM_Output.Nested_Bulleted =>
               if No_Prefix then
                  ...
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<LI TYPE=DISC>");
                  Output_Object.Char_Count := Output_Object.Char_Count + 14;
               end if;
            ...
            when ARM_Output.Small_Giant_Hanging |
              ...
              ARM_Output.Small_Enumerated          =>
               if No_Prefix then
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<DL><DD><FONT SIZE=-1>");
                  Output_Object.Char_Count   := Output_Object.Char_Count + 22;
                  Output_Object.Saw_Hang_End := True;
               else -- Has prefix.
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<DL><DT><FONT SIZE=-1>");
                  Output_Object.Char_Count   := Output_Object.Char_Count + 22;
                  Output_Object.Saw_Hang_End := False;
               end if;
         end case;
         ...
         if Number /= "" then -- Has paragraph number.
            ...
            Ada.Text_IO.Put
              (Output_Object.Output_File, Paranum_Anchor (Number));
            Ada.Text_IO.Put (Output_Object.Output_File, """>");
            ...
            Output_Object.Char_Count :=
              Output_Object.Char_Count + TINY_SWISS_FONT_CODE'Length +
              Number'Length + 8;
            Output_Object.Disp_Char_Count :=
              Output_Object.Disp_Char_Count + ((Number'Length + 1) / 2) + 1;
         ...
         if Number /= "" then -- Has paragraph number.
            Paranum_Used := True;
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<div class=""paranum"">");
            Ada.Text_IO.Put (Output_Object.Output_File, "<a name=""p");
            Ada.Text_IO.Put
              (Output_Object.Output_File, Paranum_Anchor (Number));
         ...
         case Style is
            when ARM_Output.Normal | ARM_Output.Wide_Above |
              ARM_Output.Header | ARM_Output.Small |
              ARM_Output.Small_Wide_Above | ARM_Output.Small_Header |
              ARM_Output.Index | ARM_Output.Syntax_Summary | ARM_Output.Title |
            ...
            when ARM_Output.Giant_Hanging | ARM_Output.Wide_Hanging |
              ...
              ARM_Output.Small_Enumerated          =>
               declare
                  PName : constant String := Paragraph_Name (Style, Indent);
               begin
...
--Ada.Text_IO.Put_Line("HTML: 4Comp, Hang Start");
         ...
         Output_Object.Added_Version    := '0';
         if Number /= "" then -- Has paragraph number.
            if Paragraph_Info (Style, Indent).Indent = 0
              and then
              ((not No_Prefix)
               or else Style in ARM_Output.Unprefixed_Style_Subtype)
            then
               ...
               -- others.
               for I in
                 1 .. (Number'Length + 2) - (INDENT_EMS_FOR_PARANUMS / 5)
               loop
         ...
         case Style is
            ...
            when ARM_Output.Bulleted | ARM_Output.Nested_Bulleted |
              ARM_Output.Small_Bulleted | ARM_Output.Small_Nested_Bulleted =>
               ...
               if No_Prefix then
                  Put_Style
                    (Paragraph_Name (Style, Indent) & "-NoPrefix",
                     Use_DIV => True);
               else
            ...
            when ARM_Output.Giant_Hanging | ARM_Output.Wide_Hanging |
              ...
              ARM_Output.Small_Enumerated          =>
               if No_Prefix then
                  Put_Style
                    (Paragraph_Name (Style, Indent) & "-Body",
                     Use_DIV => True);
                  Output_Object.Saw_Hang_End := True;
--Ada.Text_IO.Put_Line("HTML: 4, Hang Start, Noprefix");
               else -- Has prefix.
                  Put_Style
                    (Paragraph_Name (Style, Indent) & "-Term",
                     Use_DIV => True);
                  Output_Object.Saw_Hang_End := False;
--Ada.Text_IO.Put_Line("HTML: 4, Hang Start, Normal");
         ...
         if Number /= "" then -- Has paragraph number.
            ...
            then
               ...
               for I in
                 1 ..
                   (Number'Length + 2) -
                   ((INDENT_EMS_FOR_PARANUMS + 5) * 3 / 10)
               loop
                  -- We assume that each space is roughly equal to
   ...
   begin
      ...
      if Output_Object.In_Local_Link then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Unclosed Local_Link");
         Output_Object.In_Local_Link := False;
      end if;
      if Output_Object.Column_Count >= 4 then
         -- Formatting is deferred; only a few formats are supported.
         ...
         Output_Object.Current_Item :=
           Output_Object.Current_Item + 2; -- Skip an item.
         Output_Object.Char_Count            := 0;
         Output_Object.Disp_Char_Count       := 0;
         Output_Object.Disp_Large_Char_Count := 0;
         Output_Object.Any_Nonspace          := False;
      ...
      if Output_Object.HTML_Kind = HTML_3 then
         case Output_Object.Paragraph_Style is
            when ARM_Output.Normal | ARM_Output.Wide_Above |
              ARM_Output.Header | ARM_Output.Index =>
         ...
         case Output_Object.Paragraph_Style is
            ...
            when ARM_Output.Bulleted | ARM_Output.Nested_Bulleted |
              ARM_Output.Small_Bulleted | ARM_Output.Small_Nested_Bulleted =>
               Put_End_Compatibility_Font_Info
   ...
   procedure Category_Header
     (Output_Object : in out HTML_Output_Type; Header_Text : String)
   is
   ...
   begin
      ...
      if Output_Object.HTML_Kind = HTML_4_Only then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "<H4 Class=""centered"">" & Header_Text & "</H4>");
      else
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File,
            "<H4 ALIGN=CENTER>" & Header_Text & "</H4>");
      end if;
      Output_Object.Char_Count            := 0;
      Output_Object.Disp_Char_Count       := 0;
      Output_Object.Disp_Large_Char_Count := 0;
   ...
   procedure Clause_Header
     (Output_Object : in out HTML_Output_Type; Header_Text : in String;
   ...
   begin
      ...
      if not Output_Object.Big_Files then
         ...
         then -- ISO 2004 format.
            ...
            if Header_Text = "Table of Contents" then -- Ada 95 format
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "<H1>Table of Contents</H1>");
            else
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "<H1>Contents</H1>");
            end if;
            Output_Object.Char_Count            := 0;
            Output_Object.Disp_Char_Count       := 0;
            Output_Object.Disp_Large_Char_Count := 0;
         ...
         Start_HTML_File
           (Output_Object,
            Make_Clause_File_Name (Output_Object, Clause_Number), Header_Text,
            Clause_Number);
      else -- Big Files:
         if Clause_Number = ""
           and then
         ...
         then -- ISO 2004 format.
            -- Insert an anchor:
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<A NAME=""TOC""></A>");
            if Header_Text = "Table of Contents" then -- Ada 95 format
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "<H1>Table of Contents</H1>");
            else
      ...
      case Level is
         when ARM_Contents.Plain_Annex =>
            ...
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, Header_Text & "</H1>");
         when ARM_Contents.Normative_Annex =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<H1>" & Clause_Number & "</H1>");
            -- Note: Clause_Number includes "Annex"
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<H2>(normative)</H2>");
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<H1>" & Header_Text & "</H1>");
         when ARM_Contents.Informative_Annex =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<H1>" & Clause_Number & "</H1>");
            -- Note: Clause_Number includes "Annex"
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<H2>(informative)</H2>");
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, "<H1>" & Header_Text & "</H1>");
         ...
         when ARM_Contents.Section =>
            case Top_Level_Subdivision_Name is
               when ARM_Output.Chapter =>
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
                     "<H1>Chapter " & Clause_Number & ": " & Header_Text &
                     "</H1>");
               when ARM_Output.Section =>
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
                     "<H1>Section " & Clause_Number & ": " & Header_Text &
                     "</H1>");
               when ARM_Output.Clause =>
                  Ada.Text_IO.Put_Line
                    (Output_Object.Output_File,
                     "<H1>" & Clause_Number & " &nbsp; " & Header_Text &
                     "</H1>");
            end case;
         when ARM_Contents.Clause | ARM_Contents.Subclause |
           ARM_Contents.Subsubclause =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File,
               "<H1>" & Clause_Number & ' ' & Header_Text & "</H1>");
   ...
   procedure Revised_Clause_Header
     (Output_Object : in out HTML_Output_Type; New_Header_Text : in String;
      ...
      begin
         if Output_Object.HTML_Kind = HTML_3 then
            if Old_Version = '0' then -- Old text is original text
               return
                 "<U>" & New_Header_Text & "</U><S>" & Old_Header_Text &
                 "</S>";
            else
               return
                 "<U>" & New_Header_Text & "</U><S><U>" & Old_Header_Text &
                 "</U></S>";
            end if;
         elsif Old_Version = '0' then -- Old text is original text
            Revision_Used (Version) := True;
            return
              "<span class=""insert" & Version & """>" & New_Header_Text &
   ...
   procedure TOC_Marker
     (Output_Object : in out HTML_Output_Type; For_Start : in Boolean)
   is
...
--Ada.Text_IO.Put_Line (Output_Object.Output_File, "<HR>"); -- Horizontal line.
   ...
   procedure Separator_Line
     (Output_Object : in out HTML_Output_Type; Is_Thin : Boolean := True)
   is
   ...
   begin
      ...
      if Is_Thin then
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "<HR SIZE=1>"); -- Horizontal line.
      else
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "<HR SIZE=2>"); -- Horizontal line.
      end if;
      Ada.Text_IO.New_Line (Output_Object.Output_File);
   end Separator_Line;
   ...
   begin
      ...
      if Output_Object.HTML_Kind /= HTML_3 then
         Ada.Text_IO.Put
           (Output_Object.Output_File,
            "<div class=""" & Paragraph_Name (ARM_Output.Normal, 1) & """>");
         Paragraph_Used (ARM_Output.Normal, 1) := True;
      end if;
      if Has_Border then
         Ada.Text_IO.Put
           (Output_Object.Output_File,
            "<TABLE frame=""border"" rules=""all"" border=""2"" cellpadding=""4"">");
      else
         Ada.Text_IO.Put
           (Output_Object.Output_File,
            "<TABLE frame=""void"" rules=""none"" border=""0"" cellpadding=""2"">");
      end if;
      if Header_Kind = ARM_Output.Both_Caption_and_Header then
         ...
         if Alignment = ARM_Output.Center_All then
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<TR><TH align=""center"">");
            Output_Object.Char_Count := 24;
         else
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<TR><TH align=""left"">");
         ...
         if Alignment = ARM_Output.Center_All then
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<TR><TD align=""center"">");
            Output_Object.Char_Count := 24;
         else
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<TR><TD align=""left"">");
      ...
      if Output_Object.Table_Has_Small_Text then
         if Output_Object.HTML_Kind = HTML_4_Only then
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<SPAN STYLE=""font-size: 80%"">");
   ...
   begin
      ...
      case Marker is
         when ARM_Output.End_Item =>
            ...
            if Output_Object.In_Header then
               if Output_Object.Table_Column_Alignment = ARM_Output.Left_All
               then
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<TH align=""left"">");
                  Output_Object.Char_Count := Output_Object.Char_Count + 18;
               else
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<TH align=""center"">");
               ...
               then
                  ...
                  Ada.Text_IO.Put
                    (Output_Object.Output_File, "<TD align=""center"">");
                  Output_Object.Char_Count := Output_Object.Char_Count + 20;
               end if;
            end if;
      ...
      if Output_Object.Table_Has_Small_Text
        and then ARM_Output."/=" (Marker, ARM_Output.End_Table)
      then
         if Output_Object.HTML_Kind = HTML_4_Only then
            Ada.Text_IO.Put
              (Output_Object.Output_File, "<SPAN STYLE=""font-size: 80%"">");
     ...
     Ada.Strings.Maps."or"
       (Ada.Strings.Maps.To_Set
          (Ada.Strings.Maps.Character_Range'
             (Low => Character'Val (127), High => Character'Val (255))),
        Ada.Strings.Maps."or"
          (Ada.Strings.Maps.To_Set ('<'),
           Ada.Strings.Maps."or"
             (Ada.Strings.Maps.To_Set ('>'),
       ...
       (Ada.Strings.Maps.Constants.Upper_Set,
        Ada.Strings.Maps."or"
          (Ada.Strings.Maps.To_Set
           ...
           Ada.Strings.Maps."or"
             (Ada.Strings.Maps.To_Set ('m'),
              Ada.Strings.Maps."or"
                (Ada.Strings.Maps.To_Set ('w'),
                 Ada.Strings.Maps."or"
                   (Ada.Strings.Maps.To_Set ('<'),
                    Ada.Strings.Maps."or"
                      (Ada.Strings.Maps.To_Set ('>'),
   ...
   procedure Output_Text
     (Output_Object : in out HTML_Output_Type; Text : in String)
   is
            ...
            Output_Object.Column_Text (Output_Object.Current_Column) :=
              new Column_Text_Item_Type'
                (Text => (others => ' '), Length => 0,
                 Item => Output_Object.Current_Item, End_Para => False,
                 Next =>
                   Output_Object.Column_Text (Output_Object.Current_Column));
         end if;
         ...
         if Output_Object.Char_Count + Text'Length >= LINE_LENGTH - 10 then
            ...
            if Output_Object.Conditional_Space then
               ...
               if Ada.Strings.Maps.Is_In
                   (Text (Text'First), No_Conditional_Set)
               then
            ...
            Output_Object.Disp_Char_Count :=
              Output_Object.Disp_Char_Count + Text'Length;
   ...
   procedure Ordinary_Character
     (Output_Object : in out HTML_Output_Type; Char : in Character)
   is
   ...
   begin
      ...
      if Char = ' ' then
         if Output_Object.Char_Count >= LINE_LENGTH - 10
           and then Output_Object.Column_Count < 4
         then
            if Output_Object.HTML_Kind > HTML_3 then
   ...
   begin
      ...
      -- Output_Object.Any_Nonspace := <unchanged>;
      Output_Object.Last_was_Space    := True;
      Output_Object.Conditional_Space :=
        False; -- Never need a conditional space here.
   end Hard_Space;
   ...
   begin
      if ARM_Output."/=" (Output_Object.Paragraph_Style, ARM_Output.Index) then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Not index format");
      end if;
...
-- a hyphen will be added to the text.
   ...
   begin
      ...
      if Output_Object.Tab_Emulation = Single_Space then
         ...
         -- Output_Object.Disp_Large_Char_Count := <unchanged>;
      elsif Output_Object.Tab_Emulation = Emulate_Fixed_Only
        or else Output_Object.Tab_Emulation = Emulate_Fixed_Only_Quad
      then
         if Output_Object.Can_Emulate_Tabs
           or else (not Output_Object.Any_Nonspace)
         then -- Always can emulate if they're first on a line.
            Output_Text (Output_Object, "&nbsp;");
            Output_Object.Disp_Char_Count := Output_Object.Disp_Char_Count + 1;
            -- Output_Object.Disp_Large_Char_Count := <unchanged>;
            ...
            Output_Object.Disp_Char_Count := Output_Object.Disp_Char_Count + 1;
            if ARM_Output."="
                (Output_Object.Paragraph_Style, ARM_Output.Syntax_Summary)
              and then Output_Object.Column_Count > 1
            then
   ...
   begin
      ...
      case Char is
         when ARM_Output.EM_Dash =>
            if Output_Object.HTML_Kind > HTML_3 and Output_Object.Use_Unicode
            then
         ...
         when ARM_Output.Left_Ceiling =>
            if False and
              (Output_Object.HTML_Kind > HTML_3 and Output_Object.Use_Unicode)
            then
         ...
         when ARM_Output.Small_Dotless_I =>
            if Output_Object.HTML_Kind > HTML_3
            then --and Output_Object.Use_Unicode then -- We'll use it if it might be supported.
   ...
   begin
      ...
      Output_Text
        (Output_Object, "&#" & Char_Code (2 .. Char_Code'Length) & ';');
      Output_Object.Disp_Char_Count       := Output_Object.Disp_Char_Count + 1;
      Output_Object.Disp_Large_Char_Count :=
      ...
      then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity,
            ...
            ARM_Output.Paragraph_Style_Type'Image
              (Output_Object.Paragraph_Style));
      end if;
      ...
      if Output_Object.HTML_Kind = HTML_3 then
         case Output_Object.Paragraph_Style is
            ...
            when ARM_Output.Small_Giant_Hanging |
              ...
              ARM_Output.Small_Enumerated          =>
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "</FONT><DD><FONT SIZE=-1>");
            when others =>
         ...
         begin
            ...
            Saved_Format :=
              (Bold          => Output_Object.Is_Bold,
               Italic => Output_Object.Is_Italic, Font => Output_Object.Font,
               Size => Output_Object.Size, Color => Output_Object.Color,
              ...
              (Output_Object,
               Format =>
                 (Bold => False, Italic => False, Font => ARM_Output.Default,
                  ...
                  Change        => ARM_Output.None, Version => '0',
                  Added_Version => '0', Location => ARM_Output.Normal));
...
--" Hang_Outdent=" & Natural'Image(Paragraph_Info(Output_Object.Paragraph_Format).Hang_Outdent));
            ...
            then
               Ada.Text_IO.Put_Line
                 (Output_Object.Output_File, "<br clear=""left"">");
   ...
   begin
      ...
      if Output_Object.Column_Count <= 1 then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Not in a multi-column area");
      end if;
      if Output_Object.Column_Count >= 4 then
   ...
   is
      ...
      begin
         return
           (Format.Change /= Output_Object.Change
      ...
      begin
         return
           (ARM_Output."/=" (Format.Font, Output_Object.Font)
            or else Change_Needs_Close_or_Open);
      ...
      begin
         return
           Format.Location /= Output_Object.Location
           or else Change_Needs_Close_or_Open or else Font_Needs_Close_or_Open;
      ...
      begin
         return
           (Format.Color /= Output_Object.Color
            or else Change_Needs_Close_or_Open or else Font_Needs_Close_or_Open
            or else Location_Needs_Close_or_Open);
      ...
      begin
         return
           (Format.Size /= Output_Object.Size
            or else Change_Needs_Close_or_Open or else Font_Needs_Close_or_Open
            or else Location_Needs_Close_or_Open
            or else Color_Needs_Close_or_Open);
      ...
      begin
         return
           (Format.Italic /= Output_Object.Is_Italic
            or else Change_Needs_Close_or_Open or else Font_Needs_Close_or_Open
            or else Location_Needs_Close_or_Open
            or else Size_Needs_Close_or_Open
            or else Color_Needs_Close_or_Open);
   ...
   begin
      ...
      if Output_Object.Is_Bold
        and then ((not Format.Bold) or else Italic_Needs_Close_or_Open)
      then
      ...
      if Output_Object.Is_Italic
        and then ((not Format.Italic) or else Size_Needs_Close_or_Open)
      then
         -- The latter so that nesting is preserved; we'll reopen
      ...
      if Format.Size /= Output_Object.Size or else Color_Needs_Close_or_Open
      then
         -- The latter so that nesting is preserved; we'll reopen
      ...
      if Format.Color /= Output_Object.Color
        or else Location_Needs_Close_or_Open
      then
         -- The latter so that nesting is preserved; we'll reopen
         -- the size on the other side in that case.
      ...
      if Format.Location /= Output_Object.Location
        or else Change_Needs_Close_or_Open
      then
         -- The latter so that nesting is preserved; we'll reopen
         ...
         Output_Object.Location :=
           ARM_Output.Normal; -- That's the location now.
      end if;

      if ARM_Output."/=" (Format.Font, Output_Object.Font)
        or else Change_Needs_Close_or_Open
      then
         -- The latter so that nesting is preserved; we'll reopen
         ...
         Output_Object.Font :=
           ARM_Output.Default; -- We're now in the default state.
      end if;
...
-- to nest them. But that might not be right, either (it works on IE).
               ...
               if Output_Object.Last_was_Space
               then -- See above for reasons for this.
                  null;
               else
                  Output_Object.Conditional_Space := True;
               end if;
         ...
         case Format.Change is
            when ARM_Output.Insertion =>
               if Output_Object.HTML_Kind = HTML_3 then
                  ...
                  Output_Text
                    (Output_Object,
                     "<span class=""insert" & Format.Version & """>");
            ...
            when ARM_Output.Deletion =>
               if Output_Object.HTML_Kind = HTML_3 then
                  ...
                  Output_Text
                    (Output_Object,
                     "<span class=""delete" & Format.Version & """>");
                  Revision_Used (Format.Version) := True;
               end if;
            when ARM_Output.Both =>
               if Output_Object.HTML_Kind = HTML_3 then
...
-- to nest them. But that might not be right, either (it works on IE).
                  Output_Text
                    (Output_Object,
                     "<span class=""insert" & Format.Added_Version & """>");
                  Output_Text
                    (Output_Object,
                     "<span class=""delete" & Format.Version & """>");
      ...
      if Format.Location /= Output_Object.Location then
         ...
         case Format.Location is
            when ARM_Output.Superscript =>
               if Output_Object.HTML_Kind = HTML_4_Only then
                  Output_Text
                    (Output_Object, "<SUP><SPAN STYLE=""font-size: 140%"">");
            ...
            when ARM_Output.Subscript =>
               if Output_Object.HTML_Kind = HTML_4_Only then
                  Output_Text
                    (Output_Object, "<SUB><SPAN STYLE=""font-size: 140%"">");
               -- This is a bit larger than +1; the text is usually too small.
               else
         ...
         if Output_Object.HTML_Kind = HTML_4_Only then
            case Format.Color is
               ...
               when ARM_Output.Black =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""color: rgb(0,0,0)"">");
               when ARM_Output.Red =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""color: rgb(153,0,0)"">");
               when ARM_Output.Green =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""color: rgb(0,153,0)"">");
               when ARM_Output.Blue =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""color: rgb(0,0,153)"">");
            end case;
         else
            case Format.Color is
               when ARM_Output.Default =>
      ...
      if Format.Size /= Output_Object.Size then
         if Output_Object.HTML_Kind = HTML_4_Only then
            case Format.Size is
               ...
               when 1 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 125%"">");
               when 2 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 156%"">");
               when 3 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 194%"">");
               when 4 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 244%"">");
               when 5 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 305%"">");
               when -1 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 80%"">");
               when -2 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 64%"">");
               when -3 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 51%"">");
               when -4 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 41%"">");
               when -5 =>
                  Output_Text
                    (Output_Object, "<SPAN STYLE=""font-size: 33%"">");
               when others =>
                  -- Too much change:
   ...
   procedure Index_Target
     (Output_Object : in out HTML_Output_Type; Index_Key : in Natural)
   is
   ...
   begin
      ...
      declare
         Num : Integer :=
           Integer'Value (DR_Number (DR_Number'Last - 3 .. DR_Number'Last));
      begin
         Output_Text (Output_Object, "<A HREF=""");
   ...
   begin
      ...
      elsif Hyphen_1 = AI_String'First + 4
        and then AI_String (AI_String'First .. Hyphen_1 - 1) /= "AI95"
      then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity,
            "Unknown AI reference " & AI_String);
         Result := "00001";
      elsif Hyphen_1 = AI_String'First + 2
        and then AI_String (AI_String'First .. Hyphen_1 - 1) /= "AI"
      then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity,
         ...
         if Hyphen_2 = 0 then
            ...
            if AI_String'Last < Hyphen_2 + 1
              or else AI_String'Last > Hyphen_2 + 2
            then
               Ada.Exceptions.Raise_Exception
                 (ARM_Output.Not_Valid_Error'Identity,
            ...
            elsif AI_String'Last = Hyphen_2 + 2
              and then AI_String (Hyphen_2 + 1) = '0'
            then
               Result (1) := Character'Pred (AI_String (Hyphen_2 + 2));
            elsif AI_String'Last = Hyphen_2 + 2
              and then AI_String (Hyphen_2 + 1) = '1'
            then
               if AI_String (Hyphen_2 + 2) = '0' then
                  ...
                  Result (1) :=
                    Character'Val
                      (Character'Pos (AI_String (Hyphen_2 + 2)) -
                       Character'Pos ('1') + Character'Pos ('A'));
               end if;
            elsif AI_String'Last = Hyphen_2 + 2
              and then AI_String (Hyphen_2 + 1) = '2'
            then
               Result (1) :=
                 Character'Val
                   (Character'Pos (AI_String (Hyphen_2 + 2)) -
                    Character'Pos ('1') + Character'Pos ('A') + 10);
            else
               Ada.Exceptions.Raise_Exception
                 (ARM_Output.Not_Valid_Error'Identity,
                  "Bad AI alternative reference " & AI_String);
   ...
   begin
      ...
      if AI_Number'Length > 5
        and then AI_Number (AI_Number'First .. AI_Number'First + 4) = "AI12-"
      then
         -- AI12:
         if AI_Number'Length >= 9 then
            if AI_Number (AI_Number'First + 5) not in '0' .. '9'
              or else AI_Number (AI_Number'First + 6) not in '0' .. '9'
              or else AI_Number (AI_Number'First + 7) not in '0' .. '9'
              or else AI_Number (AI_Number'First + 8) not in '0' .. '9'
            then
               Ada.Exceptions.Raise_Exception
                 (ARM_Output.Not_Valid_Error'Identity,
         ...
         if AI_Number'Length = 9 then
            Output_Text
              (Output_Object,
               "<A HREF=""http://www.ada-auth.org/cgi-bin/cvsweb.cgi/AI12s/");
            Output_Text (Output_Object, AI_Number & "-1");
         elsif AI_Number'Length = 11 then
            if AI_Number (AI_Number'Last - 1) /= '-'
              or else AI_Number (AI_Number'Last) not in '0' .. '9'
            then
               Ada.Exceptions.Raise_Exception
                 (ARM_Output.Not_Valid_Error'Identity,
      ...
      elsif AI_Number'Length > 5
        and then AI_Number (AI_Number'First .. AI_Number'First + 4) = "AI05-"
      then
         -- AI05:
         if AI_Number'Length >= 9 then
            if AI_Number (AI_Number'First + 5) not in '0' .. '9'
         ...
         if AI_Number'Length = 9 then
            Output_Text
              (Output_Object,
               "<A HREF=""http://www.ada-auth.org/cgi-bin/cvsweb.cgi/AI05s/");
            Output_Text (Output_Object, AI_Number & "-1");
         elsif AI_Number'Length = 11 then
            if AI_Number (AI_Number'Last - 1) /= '-'
              or else AI_Number (AI_Number'Last) not in '0' .. '9'
        ...
        and then AI_Number (AI_Number'First .. AI_Number'First + 4) = "SI99-"
      then
         if AI_Number'Length >= 9 then
            if AI_Number (AI_Number'First + 5) not in '0' .. '9'
              or else AI_Number (AI_Number'First + 6) not in '0' .. '9'
         ...
         if AI_Number'Length = 9 then
            Output_Text
              (Output_Object,
               "<A HREF=""http://www.ada-auth.org/cgi-bin/cvsweb.cgi/SI99s/");
            Output_Text (Output_Object, AI_Number & "-1");
         elsif AI_Number'Length = 11 then
            if AI_Number (AI_Number'Last - 1) /= '-'
              or else AI_Number (AI_Number'Last) not in '0' .. '9'
         ...
         declare
            Folded : constant String :=
              Folded_AI95_Number
                (AI_Number); -- We don't want to have written anything if we raise an exception.
         begin
            Output_Text
              (Output_Object,
               "<A HREF=""http://www.ada-auth.org/cgi-bin/cvsweb.cgi/AIs/AI-");
   ...
   procedure Local_Link_Start
     (Output_Object : in out HTML_Output_Type; Target : in String;
      Clause_Number : in     String)
   is
   -- Generate a local link to the target and clause given.
   ...
   begin
      ...
      if Output_Object.In_Local_Link then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Already in a local link");
      end if;
   ...
   begin
      ...
      if not Output_Object.In_Local_Link then
         Ada.Exceptions.Raise_Exception
           (ARM_Output.Not_Valid_Error'Identity, "Not in a local link");
      end if;
      Output_Text (Output_Object, "</A>");
      Output_Object.In_Local_Link := False;
   end Local_Link_End;
   ...
   is
      ...
      begin
         ...
         Output_Text
           (Output_Object,
            " height=""" & H (2 .. H'Last) & """ width=""" & W (2 .. W'Last) &
            """");
      ...
      case Alignment is
         ...
         when ARM_Output.Alone_Left =>
            ...
            if Output_Object.HTML_Kind = HTML_4_Only then
               ...
               Put_EMs
                 (Output_Object.Output_File,
                  (Paragraph_Info (ARM_Output.Normal, 0).After *
                   LEADING_PERCENT) /
                  100);
               Output_Text (Output_Object, """>");
            else
         ...
         when ARM_Output.Alone_Right =>
            ...
            if Output_Object.HTML_Kind = HTML_4_Only then
               Output_Text
                 (Output_Object,
                  "<DIV Style=""text-align: right; margin-bottom: ");
               Put_EMs
                 (Output_Object.Output_File,
                  (Paragraph_Info (ARM_Output.Normal, 0).After *
                   LEADING_PERCENT) /
...
begin
   ...
   for I in ARM_Output.Paragraph_Indent_Type loop
      Paragraph_Info (ARM_Output.Normal, I) :=
        (Defined => True,
         ...
         Indent  => Natural (I), Right_Indent => 0, Hang_Outdent => 0,
         Before  => 0, After => 6); -- 120
   end loop;
   -- Wide_Above:
   for I in ARM_Output.Paragraph_Indent_Type loop
   ...
   for I in 1 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Bulleted, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I), Right_Indent => 1, Hang_Outdent => 0,
         Before  => 0, After => 5);
   end loop;
   -- Nested Bulleted:
   -- Note: Indent = 0 is not allowed.
   ...
   for I in 1 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Enumerated, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I) - 1, Right_Indent => 1, Hang_Outdent => 1,
         Before  => 0, After => 5);
   end loop;
   -- Small Enumerated:
   -- Note: Indent = 0 is not allowed.
   ...
   for I in 4 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Giant_Hanging, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I) - 4, Right_Indent => 0, Hang_Outdent => 4,
         Before  => 0, After => 6);
   end loop;
   -- Small Giant Hanging:
   -- Note: Indent < 4 is not allowed.
   ...
   for I in 3 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Wide_Hanging, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I) - 3, Right_Indent => 0, Hang_Outdent => 3,
         Before  => 0, After => 6);
   end loop;
   -- Small Wide Hanging:
   -- Note: Indent < 3 is not allowed.
   ...
   for I in 2 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Medium_Hanging, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I) - 2, Right_Indent => 0, Hang_Outdent => 2,
         Before  => 0, After => 6);
   end loop;
   ...
   for I in 1 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Narrow_Hanging, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I) - 1, Right_Indent => 0, Hang_Outdent => 1,
         Before  => 0, After => 6);
   end loop;
   ...
   for I in 2 .. ARM_Output.Paragraph_Indent_Type'Last loop
      Paragraph_Info (ARM_Output.Hanging_in_Bulleted, I) :=
        (Defined => True,
         ...
         Size    => 0, -- 18
         Font    => ARM_Output.Default,
         Indent  => Natural (I) - 2, Right_Indent => 1, Hang_Outdent => 2,
         Before  => 0, After => 5);
   end loop;
   ...
   Paragraph_Info (ARM_Output.Index, 0) :=
     (Defined => True, Tag => DIV, Size => 0, Font => ARM_Output.Default,
      Indent  => 0, Right_Indent => 0, Hang_Outdent => 0, Before => 0,
      After   => 0);

   -- Syntax Summary. Only define the form that we'll use.
   Paragraph_Info (ARM_Output.Syntax_Summary, 1) :=
     (Defined => True, Tag => DIV, Size => -1, Font => ARM_Output.Default,
      Indent  => 1, Right_Indent => 0, Hang_Outdent => 0, Before => 0,
      After   => 4);
   ...
   Paragraph_Info (ARM_Output.Title, 0) :=
     (Defined => True,
      ...
      Size    => 3, -- 36
      Font    => ARM_Output.Default,
      Indent  => 0, Right_Indent => 0, Hang_Outdent => 0, Before => 6,
      After   => 6);
