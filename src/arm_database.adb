...
with Ada.Unchecked_Deallocation, Ada.Strings.Fixed, Ada.Characters.Handling;
...
--  1/27/19 - RLB - Lengthened components to allow Reduce attributes.
   ...
   procedure Insert
     (Database_Object : in out Database_Type; Sort_Key : in String;
      Hang_Item       : in     String; Text : in String;
      Change_Kind     : in     Paragraph_Change_Kind_Type := ARM_Database.None;
      Version : in     Character := '0'; Initial_Version : in Character := '0')
   is
...
--end if;
      ...
      Ada.Strings.Fixed.Move
        (Target => Temp_Item.Sort_Key,
         Source => Ada.Characters.Handling.To_Lower (Sort_Key),
         Drop   => Ada.Strings.Right, Pad => ' ');
      Ada.Strings.Fixed.Move
        (Target => Temp_Item.Hang, Source => Hang_Item,
   ...
   --                              Text_Name : in String);
   procedure Report
     (Database_Object : in out Database_Type; In_Format : in Format_Type;
      Sorted : in     Boolean; Added_Version : Character := '0';
         ...
         case Item.Change_Kind is
            ...
            when Inserted | Inserted_Normal_Number =>
               if Item.Initial_Version <= Added_Version then
                  return
                    "@ChgRef{Version=[" & Item.Version &
                    "],Kind=[AddedNormal]}";
               else
                  return
                    "@ChgRef{Version=[" & Item.Version & "],Kind=[Added]}";
               end if;
            when Revised | Revised_Inserted_Number =>
               if Item.Initial_Version <= Added_Version then
                  return
                    "@ChgRef{Version=[" & Item.Version & "],Kind=[Revised]}";
               else
                  return
                    "@ChgRef{Version=[" & Item.Version &
                    "],Kind=[RevisedAdded]}";
   ...
   begin
      ...
      if Sorted then
         ...
         begin
            ...
            begin
               for Right in Items'First + 1 .. Items'Last
               loop -- Right sorting stop
                  Temp := Items (Right);
                  Left := Right - 1;
                  while Temp.Sort_Key <= Items (Left).Sort_Key
                  loop -- Switch items
...
--Temp.Text.all & Ascii.LF & Ascii.LF);
               Format_Text
                 (Change_if_Needed (Temp) & Temp.Hang (1 .. Temp.Hang_Len) &
                  "@\" & Temp.Text.all & ASCII.LF & ASCII.LF,
                  Temp.Sort_Key);
               Temp := Temp.Next;
            end loop;
         ...
         when Bullet_List =>
            ...
            while Temp /= null loop
               Format_Text
                 (Change_if_Needed (Temp) & Temp.Text.all & ASCII.LF &
                  ASCII.LF,
                  Temp.Sort_Key);
               Temp := Temp.Next;
            end loop;
         ...
         when Normal_Indexed_List =>
            ...
            while Temp /= null loop
               case Temp.Change_Kind is
                  when None | Inserted | Inserted_Normal_Number | Revised |
                    Revised_Inserted_Number =>
--** Debug:
...
--                      Temp.Text.all);
                     -- Index this item.
                     Format_Text
                       (Change_if_Needed (Temp) & "@defn{" &
                        Ada.Strings.Fixed.Trim
                          (Temp.Sort_Key, Ada.Strings.Right) &
                        "}" & ASCII.LF & Temp.Text.all & ASCII.LF & ASCII.LF,
                        Temp.Sort_Key);
                  when Deleted | Deleted_Inserted_Number |
...
--                      Temp.Text.all);
                     ...
                     Format_Text
                       (Change_if_Needed (Temp) & ASCII.LF & Temp.Text.all &
                        ASCII.LF & ASCII.LF,
                        Temp.Sort_Key);
               end case;
               Temp := Temp.Next;
