with ARM_Output, ARM_Contents;
--private -- Ada 2005
with ARM_Input, ARM_Database, ARM_Subindex;
...
--          - RLB - Added Changes_Only and versioning for individual changes.
   ...
   procedure Create
     (Format_Object : in out Format_Type; Changes : in ARM_Format.Change_Kind;
      Change_Version               : in     ARM_Contents.Change_Version_Type;
      Base_Change_Version          : in     ARM_Contents.Change_Version_Type;
      Display_Index_Entries : in     Boolean; Include_Annotations : in Boolean;
      Include_ISO : in     Boolean; Link_Non_Terminals : in Boolean;
   ...
   procedure Format
     (Format_Object : in out Format_Type; Text : in String;
      Output_Object : in out ARM_Output.Output_Type'Class;
      Text_Name     : in     String; No_Annotations : in Boolean);
...
private
   type Paragraph_Type is
     (Plain,
      ...
      Static_Semantics, Link_Time, Run_Time, Bounded_Errors, Erroneous,
      Requirements, Documentation, Metrics, Permissions, Advice, Notes,
      Single_Note, Examples, Ada83_Inconsistencies,
      Ada83_Incompatibilities, -- AARM-only.
      Ada83_Extensions,
      Ada83_Wording, -- AARM-only.
      Ada95_Inconsistencies,
      Ada95_Incompatibilities, -- AARM-only.
      Ada95_Extensions, Ada95_Wording, -- AARM-only.
      Ada2005_Inconsistencies,
      Ada2005_Incompatibilities, -- AARM-only.
      Ada2005_Extensions,
      Ada2005_Wording, -- AARM-only.
      Ada2012_Inconsistencies, Ada2012_Incompatibilities, -- AARM-only.
      Ada2012_Extensions,
      Ada2012_Wording, -- AARM-only.
      Element_Ref, Child_Ref, Usage_Note, -- For ASIS (AASIS-only).
   -- AARM annotations (no headers)

      Reason, Ramification, Proof, Imp_Note, Corr_Change, Discussion, Honest,
      Glossary_Marker, Bare_Annotation,
   -- Format only:
   Wide_Above, Example_Text,
      Child_Example_Text, Indented_Example_Text, Code_Indented, Indent,
      Bulleted, Nested_Bulleted, Nested_X2_Bulleted, Display, Syntax_Display,
      Syntax_Indented, Syntax_Production, Enumerated, Nested_Enumerated,
   ...
   type Impdef_Info_Type (Command : Impdef_Command_Type := None) is record
      case Command is
         when None =>
            null; -- No impdef, docreq, impladv, aspectdesc in use.
         when others =>
   ...
   type Format_Type is tagged limited record
      ...
      Style : ARM_Output
        .Paragraph_Style_Type; -- What is the current paragraph style?
      Indent : ARM_Output
        .Paragraph_Indent_Type; -- What is the current paragraph indent?
      ...
      -- Syntax:
      Syntax_NT : String
        (1 .. 80); -- Syntax non-terminal; used only during the
      ...
      -- Attributes:
      Prefix_Text : String (1 .. MAX_PREFIX) :=
        "@b{NONE!}" & (10 .. MAX_PREFIX => ' ');
