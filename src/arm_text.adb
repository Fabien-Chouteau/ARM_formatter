...
package body ARM_Text is
   ...
   procedure Put_Line_Centered
     (File : in out Ada.Text_IO.File_Type; Text : in String)
   is
   ...
   procedure Create
     (Output_Object : in out Text_Output_Type; File_Prefix : in String;
   ...
   begin
      ...
      Ada.Strings.Fixed.Move
        (Target => Output_Object.Output_Path, Source => Output_Path);
   ...
   procedure Section
     (Output_Object : in out Text_Output_Type; Section_Title : in String;
   ...
   begin
      if Output_Object.Output_Buffer_Space_Before then
         ...
         Output_Object.Char_Count :=
           Output_Object.Char_Count + 1; -- Count the space.
         Output_Object.Out_Char_Count :=
           Output_Object.Out_Char_Count + 1; -- Count the space.
      end if;
      if Output_Object.Output_Buffer_Len /= 0 then
         Ada.Text_IO.Put
           (Output_Object.Output_File,
            Output_Object.Output_Buffer
              (1 .. Output_Object.Output_Buffer_Len));
...
--     Output_Object.Output_Buffer (1 .. Output_Object.Output_Buffer_Len));
         ...
         Output_Object.Out_Char_Count    :=
           Output_Object.Out_Char_Count + Output_Object.Output_Buffer_Len;
      end if;
      Output_Object.Output_Buffer_Space_Before := False;
   end Spill;

   procedure Buffer
     (Output_Object : in out Text_Output_Type; Char : in Character)
   is
   -- Internal:
...
--Ada.Text_IO.Put_Line("** Buffer overflow!!");
   ...
   begin
      ...
      Output_Object.Output_Buffer_Space_Before :=
        False; -- Nothing in it or on the line.
      Output_Object.Output_Buffer_Len := 0;
      ...
      if Number /= "" then
         ...
         if Start_Indent > 4
         then -- (Really 8 or more); add the next three characters of the indent, but take into account any paragraph number length.
         ...
         -- into account any use of this space by the paragraph number.
         if Start_Indent > 8
         then -- (Really 12 or more); add the next three characters of the indent, but take into account any paragraph number length.
...
--    Natural'Image(Output_Object.Char_Count));
   ...
   begin
      ...
      Ada.Text_IO.New_Line
        (Output_Object.Output_File, 2); -- Double space paragraphs.
   end End_Paragraph;

   procedure Category_Header
     (Output_Object : in out Text_Output_Type; Header_Text : String)
   is
   ...
   procedure Clause_Header
     (Output_Object : in out Text_Output_Type; Header_Text : in String;
      ...
      case Level is
         ...
         when ARM_Contents.Normative_Annex =>
            Put_Line_Centered
              (Output_Object.Output_File,
               Clause_Number); -- Note: Clause_Number includes "Annex"
            Put_Line_Centered (Output_Object.Output_File, "(normative)");
            Ada.Text_IO.New_Line (Output_Object.Output_File);
            Put_Line_Centered (Output_Object.Output_File, Header_Text);
         when ARM_Contents.Informative_Annex =>
            Put_Line_Centered
              (Output_Object.Output_File,
               Clause_Number); -- Note: Clause_Number includes "Annex"
            Put_Line_Centered (Output_Object.Output_File, "(informative)");
            Ada.Text_IO.New_Line (Output_Object.Output_File);
            Put_Line_Centered (Output_Object.Output_File, Header_Text);
         ...
         when ARM_Contents.Clause | ARM_Contents.Subclause |
           ARM_Contents.Subsubclause =>
            Ada.Text_IO.Put_Line
              (Output_Object.Output_File, Clause_Number & ' ' & Header_Text);
   ...
   procedure Revised_Clause_Header
     (Output_Object : in out Text_Output_Type; New_Header_Text : in String;
   ...
   procedure TOC_Marker
     (Output_Object : in out Text_Output_Type; For_Start : in Boolean)
   is
               ...
               then
                  for J in
                    Output_Object.Char_Count + 1 ..
                      Output_Object.Tab_Stops.Stops (I).Stop - 1
                  loop
                     Ada.Text_IO.Put (Output_Object.Output_File, " ");
                     Output_Object.Char_Count := Output_Object.Char_Count + 1;
                     Output_Object.Out_Char_Count :=
   ...
   procedure Separator_Line
     (Output_Object : in out Text_Output_Type; Is_Thin : Boolean := True)
   is
   ...
   procedure Ordinary_Text
     (Output_Object : in out Text_Output_Type; Text : in String)
   is
...
--" Buffer=" & Natural'Image(Output_Object.Output_Buffer_Len));
      if Output_Object.Char_Count + Text'Length >= LINE_LENGTH - 2
        and then Output_Object.Out_Char_Count > Output_Object.Indent_Amount
      then
-- We want a line break here if the line is too long and something was output:
      ...
      Output_Object.Out_Char_Count :=
        Output_Object.Out_Char_Count + Text'Length;
   ...
   begin
      ...
      if Output_Object.Char_Count >= LINE_LENGTH
        and then Output_Object.Out_Char_Count > Output_Object.Indent_Amount
      then
...
--Ada.Text_IO.Put_Line ("Ordinary_Char [Break, no spill]: Cnt=" & Natural'Image(Output_Object.Char_Count));
      ...
      -- Add the contents of the buffer to the character count for this line:
         Output_Object.Char_Count :=
           Output_Object.Char_Count + Output_Object.Output_Buffer_Len;
         if Char /= ' ' then
...
-- a hyphen will be added to the text.
   begin
      ...
      if Output_Object.Char_Count >= LINE_LENGTH - 8 then
         Spill (Output_Object);
         Ada.Text_IO.Put_Line
           (Output_Object.Output_File, "-"); -- Add the hyphen and break.
         Make_Indent (Output_Object);
         -- else we don't need a line break.
      end if;
...
--Ada.Text_IO.New_Line;
      Output_Object.Output_Buffer_Space_Before :=
        False; -- Spaces needed were output.
   end Tab;
   ...
   begin
      case Char is
         when ARM_Output.EM_Dash =>
            Ordinary_Character
              (Output_Object, '-'); -- Not available in plain text.
         ...
         when ARM_Output.GEQ =>
            Ordinary_Text
              (Output_Object,
               ">="); -- Not available in plain text, use the Ada one.
         when ARM_Output.LEQ =>
            Ordinary_Text
              (Output_Object,
               "<="); -- Not available in plain text, use the Ada one.
         ...
         when ARM_Output.PI =>
            Ordinary_Text
              (Output_Object, "PI"); -- Not available in plain text.
         when ARM_Output.Left_Ceiling =>
            Ordinary_Text
              (Output_Object, "Ceiling("); -- Not available in plain text.
         when ARM_Output.Right_Ceiling =>
            Ordinary_Text (Output_Object, ")"); -- Not available in plain text.
         when ARM_Output.Left_Floor =>
            Ordinary_Text
              (Output_Object, "Floor("); -- Not available in plain text.
         ...
         when ARM_Output.Left_Quote =>
            Ordinary_Text
              (Output_Object,
               "`"); -- Not available in plain text, use back-quote.
         when ARM_Output.Right_Quote =>
            Ordinary_Text
              (Output_Object, "'"); -- Not available in plain text, use quote.
         when ARM_Output.Left_Double_Quote =>
            Ordinary_Text
              (Output_Object,
               """"); -- Not available in plain text, use double quote.
         ...
         when ARM_Output.Small_Dotless_I =>
            Ordinary_Text
              (Output_Object,
               "i"); -- Not available in plain text, use the nearest text.
   ...
   begin
      ...
      Ordinary_Text
        (Output_Object, "<Unicode-" & Char_Code (2 .. Char_Code'Last) & ">");
   ...
   begin
      ...
      if Output_Object.Char_Count >= Output_Object.Indent_Amount then
         ...
         Spill (Output_Object);
         for I in Output_Object.Char_Count + 1 .. Output_Object.Indent_Amount
         loop
            Ada.Text_IO.Put (Output_Object.Output_File, ' ');
         end loop;
         Output_Object.Char_Count := Output_Object.Indent_Amount;
   ...
   procedure Index_Target
     (Output_Object : in out Text_Output_Type; Index_Key : in Natural)
   is
   ...
   procedure Local_Link_Start
     (Output_Object : in out Text_Output_Type; Target : in String;
      Clause_Number : in     String)
   is
   -- Generate a local link to the target and clause given.
   ...
   procedure Picture
     (Output_Object : in out Text_Output_Type; Name : in String;
   ...
   begin
      case Alignment is
         when ARM_Output.Inline | ARM_Output.Float_Left |
           ARM_Output.Float_Right =>
            -- Inside a paragraph:
            Ordinary_Text
              (Output_Object, "[Picture: " & Name & " - " & Descr & "]");
