with ARM_Output, ARM_Contents, Ada.Exceptions, Ada.Characters
  .Handling, Ada.Strings.Fixed, Ada.Text_IO, Ada.Calendar, Ada
...
package body ARM_Index is
   ...
   function Clean
     (Item : in String; Remove_Soft_Hyphens : in Boolean) return String
   is
   ...
   begin
      for I in Item'Range loop
         if Item (I) = '@' then
            ...
            elsif I < Item'Last and then Item (I + 1) = '@' then
               -- Literal '@' command.
               if Remove_Soft_Hyphens then
                  Len            := Len + 1;
                  Result (Len)   := '@';
         ...
         elsif Close_Char /= ' ' and then Close_Char = Item (I) then
   ...
   procedure Add_Reusing_Key
     (Term      : in String; Subterm : in String := "";
      Kind : in Index_Item_Kind_Type := Primary_Term; Clause : in String := "";
      Paragraph : in String               := ""; Key : in Index_Key)
   is
      ...
      CSubterm  : constant String :=
        Clean (Subterm, Remove_Soft_Hyphens => False);
   begin
      Temp_Term.Kind := Kind;
      Ada.Strings.Fixed.Move
        (Target => Temp_Term.Term, Source => CTerm, Drop => Ada.Strings.Error,
         Pad    => ' ');
      Temp_Term.Term_Len := CTerm'Length;
      if Kind /= Primary_Term_and_Subterm
        and then Kind /= Partial_Term_with_Subterm
        and then Kind /= Child_Unit_Parent
        and then Kind /= Declaration_in_Package
        and then Kind /= SubDeclaration_in_Package
        and then Kind /= Subtype_Declaration_in_Package
        and then Kind /= See_Term and then Kind /= See_Also_Term
        and then Kind /= See_Other_Term and then Kind /= See_Also_Other_Term
      then
         if Subterm /= "" then
            Ada.Exceptions.Raise_Exception
              (Not_Valid_Error'Identity,
         ...
         Ada.Strings.Fixed.Move
           (Target => Temp_Term.Subterm, Source => CSubterm,
            Drop   => Ada.Strings.Error, Pad => ' ');
         Temp_Term.Subterm_Len := CSubterm'Length;
      end if;
      if (Kind = See_Other_Term or else Kind = See_Also_Other_Term) then
         if Clause /= "" or else Paragraph /= "" then
            raise Not_Valid_Error with "Clause used in kind without reference";
         else
         ...
         Ada.Strings.Fixed.Move
           (Target => Temp_Term.Clause, Source => Clause,
            Drop   => Ada.Strings.Error, Pad => ' ');
         Temp_Term.Clause_Len := Clause'Length;
         Ada.Strings.Fixed.Move
           (Target => Temp_Term.Paragraph, Source => Paragraph,
            Drop   => Ada.Strings.Error, Pad => ' ');
   ...
   exception
      when Ada.Strings.Length_Error =>
         Ada.Text_IO.Put_Line
           ("**** Index doesn't fit: Term: " & CTerm & " [Subterm: " &
            CSubterm & "]");
   end Add_Reusing_Key;
   ...
   procedure Add
     (Term      : in String; Subterm : in String := "";
      Kind : in Index_Item_Kind_Type := Primary_Term; Clause : in String := "";
      Paragraph : in String               := ""; Key : out Index_Key)
   is
   ...
   is
      ...
      begin
         if Item.Clause_Len > 5 and then Item.Clause (1 .. 5) = "Annex" then
            ...
            if Use_Paragraphs and then Item.Paragraph_Len /= 0 then
               ARM_Output.Index_Reference
                 (Output_Object,
                  Text =>
                    Item.Clause (Item.Clause_Len) & '(' &
                    Item.Paragraph (1 .. Item.Paragraph_Len) & ')',
                  Index_Key     => Item.Key,
                  Clause_Number => Item.Clause (1 .. Item.Clause_Len));
            else
               ARM_Output.Index_Reference
                 (Output_Object, Text => Item.Clause (Item.Clause_Len) & "",
                  Index_Key           => Item.Key,
                  Clause_Number       => Item.Clause (1 .. Item.Clause_Len));
            end if;
            ...
            ARM_Output.Index_Reference
              (Output_Object,
               Text =>
                 Item.Clause (1 .. Item.Clause_Len) & '(' &
                 Item.Paragraph (1 .. Item.Paragraph_Len) & ')',
               Index_Key     => Item.Key,
               Clause_Number => Item.Clause (1 .. Item.Clause_Len));
         else
            ARM_Output.Index_Reference
              (Output_Object, Text => Item.Clause (1 .. Item.Clause_Len),
               Index_Key           => Item.Key,
               Clause_Number       => Item.Clause (1 .. Item.Clause_Len));
         end if;
      end Clause_Ref;
           ...
           (Output_Object,
            Format =>
              (Bold     => False, Italic => True, Font => ARM_Output.Default,
               ...
               Change   => ARM_Output.None, Version | Added_Version => '0',
      ...
      begin
         if A_Soft_Hyphen = 0 then
            ...
            ARM_Output.Ordinary_Text
              (Output_Object, Text (Text'First .. A_Soft_Hyphen - 1));
            ARM_Output.Soft_Hyphen_Break (Output_Object);
            Term_Text
              (Text
                 (A_Soft_Hyphen + 2 ..
                      Text'
                        Last)); -- In case there is more than one soft hyphen.
         end if;
      end Term_Text;
      ...
      begin
         case Item.Kind is
            ...
            when Partial_Term =>
               ARM_Output.Index_Line_Break
                 (Output_Object, Clear_Keep_with_Next => Reset_Keep);
               ARM_Output.Hard_Space (Output_Object);
               ARM_Output.Hard_Space (Output_Object);
               ARM_Output.Hard_Space (Output_Object);
               ARM_Output.Ordinary_Character (Output_Object, '[');
               ...
               declare
                  In_Loc : Natural :=
                    Ada.Strings.Fixed.Index
                      (Item.Subterm (1 .. Item.Subterm_Len), " in ");
               begin
                  if In_Loc = 0 then
                     -- Weird, "in" not found.
                     Term_Text (Item.Subterm (1 .. Item.Subterm_Len));
      ...
      begin
         if Item.Next = null then
            return True;
         elsif To_Lower (Item.Term (1 .. Item.Term_Len)) /=
           To_Lower (Item.Next.Term (1 .. Item.Next.Term_Len))
         then
            ...
            -- be generated.
            return False;
         elsif To_Lower (Item.Subterm (1 .. Item.Subterm_Len)) /=
           To_Lower (Item.Next.Subterm (1 .. Item.Next.Subterm_Len))
         then
   ...
   begin
      Ada.Text_IO.Put_Line
        ("  -- Start index sorting - " & Natural'Image (Term_Count) &
         " items.");
      ...
      declare
         ...
         function "<" (Left, Right : Term_Ptr) return Boolean is
            ...
            begin
               ...
               if Left (Left'First) in '0' .. '9'
                 or else Left (Left'First) in 'A' .. 'Z'
               then
                  if Right (Right'First) in '0' .. '9'
                    or else Right (Right'First) in 'A' .. 'Z'
                  then
                     ...
                     if Left (Left'First) <
                       Character'Val
                         (Character'Pos (Right (Right'First)) +
               ...
               -- %% Warning: This has terrible performance, too much heap use.
                  L : constant String :=
                    Clean (To_Lower (Left), Remove_Soft_Hyphens => True);
                  R : constant String :=
                    Clean (To_Lower (Right), Remove_Soft_Hyphens => True);
               begin
...
--Ada.Text_IO.Put_Line("   Result=Equal");
         ...
         begin
            ...
            case Compare
              (Left.Term (1 .. Left.Term_Len),
               Right.Term (1 .. Right.Term_Len))
            is
               when Less =>
                  return True;
               when Greater =>
            ...
            if Left.Kind = Right.Kind
              or else
              (Left.Kind = Partial_Term_with_Subterm
               and then Right.Kind = Primary_Term_and_Subterm)
              or else
              (Left.Kind = Primary_Term_and_Subterm
               and then Right.Kind = Partial_Term_with_Subterm)
            then
               null; -- Continue to next compare.
            ...
            case Compare
              (Left.Subterm (1 .. Left.Subterm_Len),
               Right.Subterm (1 .. Right.Subterm_Len))
            is
               when Less =>
                  return True;
               when Greater =>
            ...
            if Left.Kind = See_Other_Term
              or else Left.Kind = See_Also_Other_Term
            then
            ...
            if Left.Clause_Number < Right.Clause_Number then
               ...
               if Left.Paragraph_Len <= 1 or else Left.Paragraph (2) = '.'
                 or else Left.Paragraph (2) = '/'
               then
                  -- Single digit number:
                  if Right.Paragraph_Len <= 1 or else Right.Paragraph (2) = '.'
                    or else Right.Paragraph (2) = '/'
                  then
                     -- Single digit number, normal compare:
                     return
                       Left.Paragraph (1 .. Left.Paragraph_Len) <
         ...
         begin
            case ((End_Index - Start_Index) + 1) is
               when 0 | 1 =>
                  null; -- A single element is obviously sorted (trivially).
               when 2 =>
               ...
               -- somewhat sorted.
                     Pivot_Item :=
                       Items ((End_Index - Start_Index) / 2 + Start_Index);
                     loop
                        loop
                           exit when not
                             (Items (Left_Index) < Pivot_Item); -- >=
...
--end loop;
      ...
      while Temp /= null loop
         ...
         if
           (Last /= null
            and then To_Lower (Last.Term (1)) /= To_Lower (Temp.Term (1)))
           or else
           (Last = null and then To_Lower (Temp.Term (1)) in 'a' .. 'z')
         then
            ...
            if To_Lower (Temp.Term (1)) in 'a' .. 'z' then
               ARM_Output.Index_Line_Break
                 (Output_Object, Clear_Keep_with_Next => False);
               ARM_Output.Text_Format
                 (Output_Object,
                  Format =>
                 ...
                 (Output_Object,
                  Format =>
                    (Bold  => False, Italic => False,
                     Font  => ARM_Output.Default, Size => 0,
                     Color => ARM_Output.Default, Change => ARM_Output.None,
         ...
         if Last = null
           or else To_Lower (Last.Term (1 .. Last.Term_Len)) /=
             To_Lower (Temp.Term (1 .. Temp.Term_Len))
         then
            ...
            if Last /= null then
               ...
               if Temp.Kind = Primary_Term then
                  ARM_Output.Start_Paragraph
                    (Output_Object, ARM_Output.Index, Indent => 0,
                     Number => "", No_Breaks => True);
                  Keep_Set := False;
               else -- The item has at least two lines; keep them together.
                  ARM_Output.Start_Paragraph
                    (Output_Object, ARM_Output.Index, Indent => 0,
                     Number => "", No_Breaks => True, Keep_with_Next => True);
                  Keep_Set := True;
               end if;
            end if;
            if Temp.Kind /= Subtype_Declaration_in_Package then
               ...
               declare
                  Of_Loc : Natural :=
                    Ada.Strings.Fixed.Index
                      (Temp.Term (1 .. Temp.Term_Len), " subtype of ");
               begin
                  if Of_Loc = 0 then
           ...
           (Temp.Kind = Primary_Term_and_Subterm
            or else Temp.Kind = Partial_Term_with_Subterm
            or else Temp.Kind = Syntax_NT_Used
            or else Temp.Kind = Child_Unit_Parent
            or else Temp.Kind = Declaration_in_Package
            or else Temp.Kind = SubDeclaration_in_Package
            or else Temp.Kind = Subtype_Declaration_in_Package)
           and then Last.Subterm (1 .. Last.Subterm_Len) /=
             Temp.Subterm (1 .. Temp.Subterm_Len)
         then
            New_Kind (Temp, Reset_Keep => Keep_Set);
            Keep_Set := False;
         elsif
           (Temp.Kind = See_Other_Term or else Temp.Kind = See_Also_Other_Term)
         then
            -- Just add the next reference.
            ARM_Output.Ordinary_Character (Output_Object, ',');
            ARM_Output.Ordinary_Character (Output_Object, ' ');
            ARM_Output.Ordinary_Text
              (Output_Object, Temp.Subterm (1 .. Temp.Subterm_Len));
            -- No clause references here.
         elsif (Temp.Kind = See_Term or else Temp.Kind = See_Also_Term) then
            -- Just add the next reference. We'll just use New_Kind for
            -- this, so we don't get the formats slightly different.
            New_Kind (Temp, Reset_Keep => Keep_Set);
            Keep_Set := False;
         elsif Last.Clause (1 .. Last.Clause_Len) =
           Temp.Clause (1 .. Temp.Clause_Len)
           and then Last.Paragraph (1 .. Last.Paragraph_Len) =
             Temp.Paragraph (1 .. Temp.Paragraph_Len)
         then
