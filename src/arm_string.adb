...
package body ARM_String is
   ...
   procedure Open
     (Input_Object : in out String_Input_Type; Text : in String;
      Text_Name    : in     String)
   is
   ...
   begin
      ...
      if Text'Length > Input_Object.Buffer'Length then
         Ada.Text_IO.Put_Line
           ("  ** Text too long for string input object for " & Text_Name);
      else
      ...
      if Text_Name'Length > Input_Object.Name'Length then
         Input_Object.Name :=
           Text_Name
             (Text_Name'First ..
                  Text_Name'First + Input_Object.Name'Length - 1);
   ...
   procedure Get_Char
     (Input_Object : in out String_Input_Type; Char : out Character)
   is
...
--else Ada.Text_IO.Put_Line ("Get_Char " & Char & ", index=" & Natural'Image(Input_Object.Buffer_Index));
   ...
   begin
      ...
      if Input_Object.Buffer_Index <= Input_Object.Buffer_Len
        and then Input_Object.Buffer (Input_Object.Buffer_Index) = ASCII.LF
      then
         Input_Object.Line_Counter := Input_Object.Line_Counter - 1;
      end if;
   ...
   begin
      ...
      then
         ...
         Result (Result'First .. Result'First + Len - 1) :=
           Input_Object.Buffer
             (Input_Object.Recording_Start .. Input_Object.Buffer_Index);
      end if;
